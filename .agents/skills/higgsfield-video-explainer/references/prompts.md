...
Two elements keep the film consistent: one style-key image attached to every clip, and one identical STYLE descriptor
pasted into every clip prompt. Narrate per block so each voice take maps to exactly one 10-second clip.
...
- In mascot mode, Block 1 greets by gesture with mouth closed, the final block waves a sign-off, and middle blocks use
  the same design.
...
Write one plain line per clip, sized for about 8–9 seconds and normally 20–24 words. Keep every take under roughly 9.5
seconds.
