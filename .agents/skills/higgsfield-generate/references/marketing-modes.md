...
Current mode values for `marketing_studio_video` `--mode`. The live schema is the source of truth, so run
`higgsfield model get marketing_studio_video` if validation fails.
...
The "Hook/setting" column shows whether `--hook_id` and `--setting_id` are valid for that mode. Modes marked ❌ ignore
or reject setup items.
...
- `duration`: integer ≥ 4 seconds. No fixed cap; check `higgsfield model get marketing_studio_video` for the current
  upper bound.
...
- `product_ids`: array of product UUIDs (from `higgsfield marketing-studio products fetch` or `create`). See
...
1. Call `higgsfield marketing-studio products fetch --url <url> --wait` (or use `show_marketing_studio` widget action
   `fetch`).
2. Call `higgsfield generate create marketing_studio_video --url <same url>` — the backend looks up / reuses the entity
   and submits.
