...
publication. See [HTML publication](../../../../apps/legislation-ingestion/docs/regulations/storage-validation.md#canonical-html-publication).

Implementation specification, September 14, 2026. Initial eCFR storage schema and writer are implemented; the full contract
below remains broader than delivered functionality. See [storage validation and boundaries](../../../../apps/legislation-ingestion/docs/regulations/storage-validation.md).
Parent: [implementation plan](../../../../apps/legislation-ingestion/docs/regulations/implementation.md). Applies to direct federal and later licensed state ingestion.
...
Add to C's `src/database/schema/schema.ts` using the normal schema/migration workflow. Table names below are proposed.
...
Annual CFR package identity and printed revision date are independent; content reuse establishes no legal-validity
interval and folder years cannot become legal currency. I owns the
[Title 1 source audit and observation policy](../../../../apps/legislation-ingestion/docs/regulations/annual-source-observations.md).
...
These are persisted publication invariants, not a worker runbook. I owns
[lease renewal, staging, retries and recovery](../../../../apps/legislation-ingestion/docs/regulations/storage-validation.md#transaction-and-recovery-contract).
Stale fences cannot advance source state; publication/outbox commit remains recoverable independently from dispatch.
...
membership. Composite foreign keys enforce code, jurisdiction, owner and edition-parent consistency; see
[PostgreSQL constraints](https://www.postgresql.org/docs/current/ddl-constraints.html).
Store foreign keys/indexes supporting deletion/restriction fan-out deliberately.
...
[shared search projections](../engineering/search-projections.md); projection indexes must not alter the canonical database image.
...
Restricted content must be filtered before ranking/counting/snippet generation. See [state onboarding](../../../../apps/legislation-ingestion/docs/regulations/state-onboarding.md).
