...
Bill and document-section records retain PostgreSQL `tsvector` columns for lexical search. C's
[embedding contract](embeddings.md#storage-contract) owns dedicated foreign-keyed model/dimension/input tables and
dimension-specific HNSW indexes. Older inline vectors are non-authoritative. The September integrity finding means an
old matching hash cannot justify skipping the deferred full rebuild; see
[I's rollout hold](../../../../apps/legislation-ingestion/docs/engineering/embedding-rollout-plan.md).

## Migrations

Drizzle definitions live in C's `src/database/schema`, and the single migration history lives in
`src/database/migrations`. The baseline enables pgvector before vector columns. Preserve migration assets in the core
package and release explicitly through `pnpm --filter legislation-web db:migrate` using the direct administration URL.
No build, W/I/M startup or readiness probe applies migrations. See [database setup](../operations/development.md).
