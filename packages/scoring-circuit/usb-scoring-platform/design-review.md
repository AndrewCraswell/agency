...
Started 8 September 2026. Review the current native KiCad board and the `output/tocap-capacitor-review/` assembly
...
tests 4.1V/20V supplies, reduced capacitors and a resistive-equivalent load step, not the actual isolator startup. The
completed 4.1V step reaches 50ms: raw samples give 5.00159V at 19ms, 4.83892V minimum during the 20–39ms loaded
interval, 5.00058V at 39ms, and 5.16945V maximum during 40–49ms load release. These remain within a 5V +/-5% nominal
screening band. The 20V case is still running; this is not a completed two-case pass, tolerance sweep, actual-load
startup proof or hardware measurement. ADI's `/download/latest/LTspice64.msi` returned signed version 24.0.12, despite
its website advertising a newer version. Retained model SHA256:
