...
| P1       | U19/L2/C39/C41/C43/C67/C68/R108–R110 operating margins — P1 mode mismatch              | U19 remains Burst-only. Candidate footprint and capacitor curves reviewed. Three assumed cable/input-network scenarios pass ngspice; eight-model suite passes. Active PCB/BOM unchanged. Manufacturer-model startup/load-step run active; no result yet. Bypass and suspend margins remain open. | Primary rail instability could prevent USB operation. Candidate IC $12.79 at one / $10.054 at ten; complete delta unpriced.                       | Finish electrical margins, then migrate the complete automatic-mode circuit and local routing; preserve isolation and one USB-C. Recheck startup/suspend budget and assembly outputs.                             |
...
**Manufacturer-model run in progress:** `simulation/primary-regulator-startup.cir` is now the reproducible LTspice
candidate deck. The first run is in `output/primary-regulator-startup.*`; let it finish before starting another. It
tests 4.1V/20V supplies, reduced capacitors and a resistive-equivalent load step, not the actual isolator startup. No
completed result is recorded yet. ADI's `/download/latest/LTspice64.msi` returned signed version 24.0.12, despite its
website advertising a newer version. Retained model SHA256:
