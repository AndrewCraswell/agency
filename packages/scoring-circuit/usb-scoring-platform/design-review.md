...
| P1       | U19/L2/C39/C41/C43/C67/C68/R108–R110 operating margins — P1 mode mismatch              | U19 pad 16 is grounded (Burst-only); the active budget is not validated by the headline 1A rating. LTC3130IMSE-1#PBF is the simpler candidate below; current/capacitance screen passed assumptions, but footprint, derating and transients remain unverified. Not implemented. | Primary rail instability could prevent USB operation. Candidate IC $12.79 at one / $10.054 at ten; complete delta unpriced.                       | Validate and implement the complete automatic-mode replacement circuit, not just its MPN; preserve isolation and one USB-C. Recheck startup/suspend budget, all affected routing and assembly outputs.            |
...
footprint is not interchangeable with U19's DHD. No package overlay or native migration has passed yet.
...
current netlist was freshly exported for this check. Native files/BOM remain LTC3115-1. Direct manufacturer download
failed; the distributor download returned a non-PDF response, so neither was accepted as visual package evidence.
