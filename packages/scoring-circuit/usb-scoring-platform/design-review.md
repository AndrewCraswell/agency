...
| P1       | U19/L2/C39/C41/C43/C67/C68/R108–R110 operating margins — P1 mode mismatch              | U19 remains Burst-only. LTC3130 candidate land pattern is implemented in the local library and checked in KiCad; C39/C43 bias curves reviewed. Current screen remains assumption-dependent. Active PCB/BOM unchanged; VCC bypass, ripple and transients remain open.        | Primary rail instability could prevent USB operation. Candidate IC $12.79 at one / $10.054 at ten; complete delta unpriced.                       | Finish electrical margins, then migrate the complete automatic-mode circuit and local routing; preserve isolation and one USB-C. Recheck startup/suspend budget and assembly outputs.                             |
...
**Simpler candidate, not implemented: LTC3130IMSE-1#PBF.** Automatic Burst/PWM operation and internal compensation avoid
a suspend detector and external compensation network. Its 25V operating ceiling covers nominal 20V PD, but hot-plug/PD
overshoot remains unbounded. Do not treat its 600mA buck rating as a boost guarantee. Using the 660mA minimum
average-current limit and assumed 70% efficiency at 4.1V input gives `0.660 × 4.1 / 5 × 0.70 = 0.37884A`, 26% above the
0.3005A allocation. This is an assumption-dependent screen, not a guaranteed or measured output.

PVIN requires 4.7uF and VIN a separate, Kelvin-connected 1uF bypass. Repurposing C40 from 100nF to 1uF makes direct VBUS
capacitance 7.8uF nominal; at +10% for every listed capacitor it is 8.58uF before IC/parasitic capacitance. Capacitor
DC-bias/temperature derating and cable resonance still require review. The proposed fixed-5V circuit could remove
R90/R91/R108/R109/R110/C67/C68, seven parts, with 22nF bootstraps and EXTVCC fed from the output. Its MSOP-16-EP
footprint is not interchangeable with U19's DHD. The local `LTC3130_MSE` footprint now follows ADI drawing 05-08-1667
Rev F (datasheet page 37, visually inspected): 0.889 x 0.300mm lands on 0.500mm pitch, 3.331mm inner gap, 5.109mm outer
span, and 1.651 x 2.845mm exposed pad. The 0.300mm width is within ADI's 0.305 +/- 0.038mm range and leaves 0.200mm
between adjacent lands. Pin 1 is upper-left in this rotated top view. KiCad loaded all 17 numbered pads and four paste
windows; their coordinates and dimensions were checked. The four exposed-pad paste apertures are a project stencil
choice requiring assembler review, not an ADI specification. No schematic/PCB migration or assembled-pin overlay has
...
loaded successfully in the browser after direct downloads failed.
...
high-voltage buck and low-voltage boost conditions before adding capacitance or a switch. At the candidate's 4V VCC, C41
`C2012X5R1C475K125AC` retains roughly 88% (about 4.1uF typical) on its bias curve. Its nominal value meets the stated
4.7uF bypass value, not a guaranteed effective minimum. Output transient/stability and bypass adequacy still need
completion before migration.
...
**LTC3114IDHC-1#PBF is no longer the preferred path.** The following screened constraint explains why not to implement
that candidate's input reservoir directly on VBUS:
