...
resistors. All seven 3.3k sense pull-downs now have ground returns. Five comparator inputs (LEFT_A, LEFT_C, RIGHT_B,
RIGHT_C and PISTE) reach their pull-downs. LEFT_B and RIGHT_A still need local escape routing around the existing
controller wiring and bypass capacitors. The remaining 44 unrouted items concern the conductor/sense interface.
...
export and connected-pin transfer checks succeeded. DRC reports **44 unrouted items** and **four other findings**, all
...
In1.Cu signal trace or moved part. LEFT_B and RIGHT_A remain boxed in by local routing and require revised escapes;
conductor/clamp connections remain unfinished. This is routing only, not protection or sensing qualification. Schematic,
...
but failed four scoring tests: mutation and command-ledger timeouts, a canonical-corpus failure and a 29-versus-28
scenario-count assertion (769 scoring tests passed). The run stopped before every other package completed. These tests
