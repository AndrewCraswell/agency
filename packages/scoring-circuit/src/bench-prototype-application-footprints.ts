...
const bp123ActiveSeeds = [
  ["U_APP_SUPERVISOR", "Texas Instruments", "TPS389033DSER", null, "single ESP32 brownout supervisor"],
  ["U_APP_WDOG", "Texas Instruments", "TPS3431SDRBR", null, "single ESP32 health watchdog"],
  [
  ...
  ["C_APP_WDOG_BYPASS", "KEMET", "C0603C104K3RACTU", "0603", "watchdog local bypass capacitor"],
  ["R_APP_WDOG_TIMEOUT", "Yageo", "RC0603FR-0710KL", "0603", "watchdog timeout selection"],
  ["R_APP_WDI_PULLUP", "Yageo", "RC0603FR-07100KL", "0603", "watchdog input default"],
  ["R_ESP_EN_PULLUP", "Yageo", "RC0603FR-0710KL", "0603", "ESP32 enable/reset pull-up"],
  ["C_ESP_EN_DELAY", "TDK", "C1608X5R1A105K080AC", "0603", "ESP32 enable/reset delay capacitor"]
] as const
...
function currentBp140ReferenceSet(): string[] {
  return [
    ...
    .filter((reference) => !["R_W5500_RESET_PULLUP", "U_APP_RESET_FANOUT"].includes(reference))
    .sort()
}

const bp123ActiveReferenceIds = bp123ActiveSeeds.map(([reference]) => reference).sort()
...
const records = [
  ...
  ...bp123ActiveSeeds.map(([reference, manufacturer, mpn, packageName, role]) =>
    selected({
      reference,
      section: "reset-watchdog",
      manufacturer,
      mpn,
      package: packageName,
      sourceContract: "BP-123",
      sourceUrl: retainedSourceFor(reference)?.url ?? null,
      manufacturerDrawing: retainedSourceFor(reference),
      role
    } as Seed & { readonly role: string })
...
const yageo100kReferences = ["R_W5500_INT_BIAS", "R_APP_WDI_PULLUP", "R_DISPLAY_ENABLE_GATE_PD"] as const

const bp033SharedC0603References = [
  "C_APP_REG_IN_HF",
  "C_APP_REG_BOOT",
...
const projectFootprintMappings = [
  ...
  ...yageo100kReferences.map((reference) => ({
    reference,
...
const definition = {
  ...
  omittedPeripherals: [
    dnp(
      "U_FRAM",
      "CY15B104Q-LHXIT",
      "ESP-IDF encrypted NVS supplies bounded persistence without a second SPI journal device."
    ),
  ...
  closureRules: [
    ...
    "Keep U_FRAM, U_AUDIO, and every omitted peripheral DNP. Do not create a land pattern, route, or bodge connection for an omitted peripheral.",
...
export function validateBenchPrototypeApplicationFootprints(value: unknown): true {
  ...
  const bp123RecordReferenceIds = contract.records
    .filter((record) => record.sourceContract === "BP-123")
  ...
  if (
    ...
    contract.projectFootprintMappings[27]?.reference !== "U_W5500" ||
    contract.projectFootprintMappings[27]?.artifactKind !== "bp033-w5500-project-footprint" ||
    contract.projectFootprintMappings[27]?.artworkModule !== "src/bp033-w5500-project-footprint.tsx" ||
    contract.projectFootprintMappings[27]?.reviewDocument !== "docs/bench-prototype-application-footprints.md" ||
    contract.projectFootprintMappings[27]?.sourceArtifactPath !== "docs/evidence/bp-033/wiznet-w5500-datasheet.pdf" ||
    contract.projectFootprintMappings[27]?.sourceSha256 !==
      "7B826B808084CCD986BCC22904C00A07A508EF42FB93D079FE7150A4C4F1A63D" ||
    contract.projectFootprintMappings[27]?.reviewState !== "root-reviewed-review-input" ||
    contract.projectFootprintMappings[27]?.reviewer !== "root-final-reviewer" ||
    contract.projectFootprintMappings[27]?.reviewedAt !== "2026-08-25" ||
    contract.projectFootprintMappings[27]?.fabricationRelease !== "deny" ||
    ...
    contract.projectFootprintMappings[28]?.reviewState !== "root-reviewed-review-input" ||
    contract.projectFootprintMappings[28]?.reviewer !== "root-final-reviewer" ||
    contract.projectFootprintMappings[28]?.reviewedAt !== "2026-08-25" ||
    contract.projectFootprintMappings[28]?.fabricationRelease !== "deny" ||
    !bp033SharedC0603References.every((reference, offset) => {
      const mapping = contract.projectFootprintMappings[29 + offset]
      return (
        mapping?.reference === reference &&
    ...
    !bp033Keystone5001TestPointEvidenceCandidate.sourceBinding.exactReferences.every((reference, offset) => {
      const mapping = contract.projectFootprintMappings[35 + offset]
      return (
        mapping?.reference === reference &&
    ...
    !bp033TdkC2012x7s1a226m125ac0805ReviewCandidate.rootIntegrationHandoff.requiredRows.every((row, offset) => {
      const mapping = contract.projectFootprintMappings[38 + offset]
      return (
    ...
    contract.projectFootprintMappings[43]?.reference !== "Q_DISPLAY_ENABLE" ||
    contract.projectFootprintMappings[43]?.artifactKind !== bp033DisplayBufferBss138akaReferenceBinding.artifactKind ||
    contract.projectFootprintMappings[43]?.artworkModule !==
      "src/bp033-display-buffer-bss138aka-reference-binding.tsx" ||
    contract.projectFootprintMappings[43]?.reviewDocument !==
      "docs/bp-033-display-buffer-bss138aka-reference-binding-review.md" ||
    contract.projectFootprintMappings[43]?.sourceArtifactPath !==
      "docs/evidence/bp-032/nexperia-bss138aka-datasheet.pdf" ||
    contract.projectFootprintMappings[43]?.sourceSha256 !==
      bp033DisplayBufferBss138akaReferenceBinding.retainedManufacturerFacts.sourceSha256 ||
    contract.projectFootprintMappings[43]?.reviewState !== "root-reviewed-review-input" ||
    contract.projectFootprintMappings[43]?.reviewer !== "root-final-reviewer" ||
    contract.projectFootprintMappings[43]?.reviewedAt !== "2026-08-25" ||
    contract.projectFootprintMappings[43]?.fabricationRelease !== "deny" ||
    !bp033Sn74ahct245pwrTssop20Footprint.references.every((reference, offset) => {
      const mapping = contract.projectFootprintMappings[44 + offset]
      return (
        mapping?.reference === reference &&
    ...
    !yageo100kReferences.every((reference, offset) => {
      const mapping = contract.projectFootprintMappings[46 + offset]
      return (
        mapping?.reference === reference &&
    ...
    !bp033ApplicationRegulatorSupportMappings.every((reviewed, offset) => {
      const mapping = contract.projectFootprintMappings[51 + offset]
      return (
    ...
    !bp033DisplayLimiterMappings.every(([reference, sourceFile, sourceSha256], offset) => {
      const mapping = contract.projectFootprintMappings[56 + offset]
      return (
        mapping?.reference === reference &&
    ...
    bp123RecordReferenceIds.length !== bp123ActiveReferenceIds.length ||
    bp123RecordReferenceIds.some((reference, index) => reference !== bp123ActiveReferenceIds[index]) ||
    contract.records.some((record) =>
      ...
      ].includes(record.reference)
    ) ||
    contract.projectFootprintMappings.some((mapping) =>
      [
        "R_W5500_RESET_PULLUP",
        "U_APP_RESET_FANOUT",
        "R_BUFFER_A_ENABLE_PULLUP",
        ...
        "Q_DISPLAY_BUFFER_A_ENABLE",
        "Q_DISPLAY_BUFFER_B_ENABLE"
      ].includes(mapping.reference)
    ) ||
    contract.bp140SelectionBlockedReferences.length !== 0 ||
    contract.bp140DnpReferences.length !== 0 ||
    contract.omittedPeripherals.length !== 6 ||
    contract.omittedPeripherals.some((record) => record.population !== "DNP") ||
    !contract.omittedPeripherals.some((record) => record.reference === "U_FRAM") ||
    contract.records.some((record) =>
      ["U_FRAM", "R_FRAM_WP_PULLUP", "R_FRAM_HOLD_PULLUP", "C_FRAM_BYPASS"].includes(record.reference)
    ) ||
