...
describe("BP-033 application footprint closure ledger", () => {
  ...
  it("maps all six active 10k support resistors to the reviewed Yageo evidence", () => {
    const mappings = benchPrototypeApplicationFootprints.projectFootprintMappings.filter(
      (mapping) => mapping.artifactKind === "bp033-yageo-10k-bridge-footprint"
    )
    expect(mappings).toHaveLength(6)
    expect(new Set(mappings.map((mapping) => mapping.reference)).size).toBe(6)
  ...
  it("maps the remaining W5500 support evidence without release credit", () => {
    const reviewed = benchPrototypeApplicationFootprints.projectFootprintMappings.filter(
      (mapping) => mapping.artifactKind === "bp033-w5500-crystal-support-exact-mpn-evidence"
    )
    expect(reviewed).toHaveLength(9)
    expect(reviewed.map((mapping) => mapping.reference)).toEqual(
      expect.arrayContaining(["Y_W5500", "C_W5500_XI", "C_W5500_XO", "FB_W5500_AVDD"])
    )
    expect(
      reviewed.every(
        (mapping) =>
  ...
  it("keeps audio and every other omitted peripheral out of the board", () => {
    ...
    expect(benchPrototypeApplicationFootprints.records.map((record) => record.reference)).not.toEqual(
      expect.arrayContaining(["U_FRAM", "R_FRAM_WP_PULLUP", "R_FRAM_HOLD_PULLUP", "C_FRAM_BYPASS"])
    )
  })
  ...
  it("reconciles the active BP-123 BOM references and removes the old reset fanout", () => {
    expect(
    ...
    expect(benchPrototypeApplicationFootprints.records.map((record) => record.reference)).not.toEqual(
      expect.arrayContaining(["R_W5500_RESET_PULLUP", "U_APP_RESET_FANOUT"])
    )
    expect(
      benchPrototypeApplicationFootprints.projectFootprintMappings.map((mapping) => mapping.reference)
    ).not.toEqual(expect.arrayContaining(["R_W5500_RESET_PULLUP", "U_APP_RESET_FANOUT"]))
  })
  ...
  it("links the six active KEMET 100 nF rows to the shared review-only evidence", () => {
    const references = [
      "C_APP_REG_IN_HF",
      "C_APP_REG_BOOT",
  ...
  it("links the shared BP-144 display-enable MOSFET to retained BSS138AKA evidence", () => {
    expect(benchPrototypeApplicationFootprints.projectFootprintMappings).toContainEqual({
  ...
  it("links the three active 100 kOhm resistors to exact Yageo evidence", () => {
    for (const reference of ["R_W5500_INT_BIAS", "R_APP_WDI_PULLUP", "R_DISPLAY_ENABLE_GATE_PD"]) {
      expect(benchPrototypeApplicationFootprints.projectFootprintMappings).toContainEqual({
        reference,
  ...
  it("links all seven BP-033 display-limiter rows to exact primary evidence", () => {
    const expected = [
      [
        "R_DISPLAY_ILM",
    ...
    for (const [reference, sourceFile, sourceSha256] of expected) {
      expect(benchPrototypeApplicationFootprints.projectFootprintMappings).toContainEqual({
        reference,
        ...
        sourceArtifactPath: `docs/evidence/bp-033/${sourceFile}`,
        sourceSha256,
        reviewState: "root-reviewed-review-input",
        reviewer: "root-final-reviewer",
        reviewedAt: "2026-08-25",
