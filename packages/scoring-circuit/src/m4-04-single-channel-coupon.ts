...
const ercNets = [
  ...
  {
    net: "V5_ANALOG",
    source: "J_UPSTREAM_5V.pin1 from the protected V5 branch",
    sinks: ["U_NEGATIVE_RAIL.pin2", "U_REF.pin2", "U_OVP_BUFFER.pin7", "C_NEG_IN.pin1", "C_BUFFER_POS.pin1"],
    rule: "The protected 5-volt branch directly supplies the positive analog rail; no isolated converter is populated."
  },
  {
    net: "V5_NEG",
  ...
  {
    net: "APP_3V3",
    source: "J_UPSTREAM_3V3.pin1 from the LMR43620 application rail",
    sinks: ["U_SOURCE_SWITCH.pin13", "U_SAR.pin2", "U_SAR.pin10", "C_MUX.pin1", "C_SAR_AVDD.pin1", "C_SAR_DVDD.pin1"],
    rule: "The switch and SAR use the sole regulated 3.3-volt application rail; no coupon-local LDO is populated."
  },
  {
    net: "SCORING_SGND",
    source: "quiet analog region with one reviewed connection to APP_GND",
    sinks: ["J_FIXTURE.pin2", "J_GUARDED_FORCE.pin2", "U_ESD.pin3", "U_ESD.pin8", "U_SAR.pin5"],
    rule: "The reserved fixture pin three is not assigned as a second return, and no isolation boundary is claimed."
  },
  {
    net: "SPI",
...
const definition = {
  ...
  schematic: {
    ...
    normalPower:
      "USB-C PD remains the apparatus normal input; the coupon accepts only the protected V5 and APP_3V3 branch outputs and does not duplicate USB-C, VBUS, CC, or PD circuitry."
  },
  erc: {
