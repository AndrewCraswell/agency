import type { ReactElement } from "react"

type PlainRecord = Record<PropertyKey, unknown>

function isPlainRecord(value: unknown): value is PlainRecord {
  return (
    value !== null &&
    typeof value === "object" &&
    !Array.isArray(value) &&
    Object.getPrototypeOf(value) === Object.prototype
  )
}

function deepFreeze<T>(value: T, seen = new WeakSet<object>()): T {
  if (value === null || typeof value !== "object") return value
  if (seen.has(value)) throw new RangeError("BP-031 TDK evidence cannot contain cycles or aliases")
  seen.add(value)
  for (const key of Reflect.ownKeys(value)) {
    const descriptor = Object.getOwnPropertyDescriptor(value, key)
    if (descriptor === undefined || !("value" in descriptor)) {
      throw new RangeError("BP-031 TDK evidence may contain only data properties")
    }
    deepFreeze(descriptor.value, seen)
  }
  return Object.freeze(value)
}

function sameDataGraph(actual: unknown, expected: unknown, seen = new WeakMap<object, object>()): boolean {
  if (Object.is(actual, expected)) return true
  if (actual === null || expected === null || typeof actual !== "object" || typeof expected !== "object") return false
  if (seen.has(actual)) return seen.get(actual) === expected
  seen.set(actual, expected)

  const actualArray = Array.isArray(actual)
  const expectedArray = Array.isArray(expected)
  if (actualArray !== expectedArray) return false
  if (actualArray) {
    if (
      !Array.isArray(actual) ||
      !Array.isArray(expected) ||
      Object.getPrototypeOf(actual) !== Array.prototype ||
      Object.getPrototypeOf(expected) !== Array.prototype ||
      actual.length !== expected.length
    ) {
      return false
    }
    const actualKeys = Reflect.ownKeys(actual)
    const expectedKeys = Reflect.ownKeys(expected)
    if (
      actualKeys.length !== expectedKeys.length ||
      actualKeys.some((key) => typeof key === "symbol" || !expectedKeys.includes(key))
    ) {
      return false
    }
    return expected.every((entry, index) => {
      const descriptor = Object.getOwnPropertyDescriptor(actual, String(index))
      return (
        descriptor !== undefined &&
        "value" in descriptor &&
        descriptor.enumerable &&
        sameDataGraph(descriptor.value, entry, seen)
      )
    })
  }

  if (!isPlainRecord(actual) || !isPlainRecord(expected)) return false
  const actualKeys = Reflect.ownKeys(actual)
  const expectedKeys = Reflect.ownKeys(expected)
  if (
    actualKeys.length !== expectedKeys.length ||
    actualKeys.some((key) => typeof key === "symbol" || !expectedKeys.includes(key))
  ) {
    return false
  }
  return expectedKeys.every((key) => {
    const actualDescriptor = Object.getOwnPropertyDescriptor(actual, key)
    const expectedDescriptor = Object.getOwnPropertyDescriptor(expected, key)
    return Boolean(
      actualDescriptor &&
      expectedDescriptor &&
      "value" in actualDescriptor &&
      "value" in expectedDescriptor &&
      actualDescriptor.enumerable === expectedDescriptor.enumerable &&
      sameDataGraph(actualDescriptor.value, expectedDescriptor.value, seen)
    )
  })
}

const projectCopperPadLengthMm = 0.7
const projectCopperPadWidthMm = 0.7
const projectCopperPadGapMm = 0.7
const projectOverallLandSpanMm = Number((projectCopperPadGapMm + 2 * projectCopperPadLengthMm).toFixed(3))
const projectCopperPadCenterXMm = Number(((projectCopperPadGapMm + projectCopperPadLengthMm) / 2).toFixed(3))
const projectSolderMaskMarginMm = 0.05
const projectPasteReductionPerEdgeMm = 0.05
const projectSolderMaskOpeningLengthMm = Number((projectCopperPadLengthMm + 2 * projectSolderMaskMarginMm).toFixed(3))
const projectSolderMaskOpeningWidthMm = Number((projectCopperPadWidthMm + 2 * projectSolderMaskMarginMm).toFixed(3))
const projectPasteOpeningLengthMm = Number((projectCopperPadLengthMm - 2 * projectPasteReductionPerEdgeMm).toFixed(3))
const projectPasteOpeningWidthMm = Number((projectCopperPadWidthMm - 2 * projectPasteReductionPerEdgeMm).toFixed(3))
const projectCourtyardLengthMm = 2.4
const projectCourtyardWidthMm = 1.3
const projectCourtyardClearanceMm = 0.15
const artworkSha256 = "D1DB1A503674A4E51CFB8E529C93CD02ACF6C30D025C284FD08431E239D945EC"

const affectedReferences = [
  "C_REF_IN_1",
  "C_REF_IN_2",
  "C_REF_IN_3",
  "C_REF_IN_4",
  "C_REF_IN_5",
  "C_REF_IN_6",
  "C_REF_IN_7"
] as const
const primaryProductPageUrl =
  "https://product.tdk.com/en/search/capacitor/ceramic/mlcc/info?part_no=CGA3E3X7R1H105K080AB"

/**
 * BP-031 project land-pattern candidate for the exact TDK capacitor used by
 * the seven reference-input bypasses. This isolated artifact is not imported
 * by a board model and does not grant fabrication or release authority.
 */
const evidenceDefinition = {
  artifactKind: "bp031-tdk-cga3e3x7r1h105k080ab-project-footprint",
  workUnit: "BP-031",
  manufacturer: "TDK Corporation",
  manufacturerPartNumber: "CGA3E3X7R1H105K080AB",
  sourceContract: "BP-101",
  role: "reference input bypass",
  geometryAuthority: "project-review-input-not-manufacturer-specification",
  sourceBinding: {
    canonicalSourcePath: "packages/scoring-circuit/src/one-channel-analog-readiness.ts",
    canonicalSourceReference: "C_REF_IN",
    replicatedReferencePrefix: "C_REF_IN_",
    manufacturer: "TDK",
    manufacturerPartNumber: "CGA3E3X7R1H105K080AB",
    package: "0603",
    exactOrderableSourceId: "tdk-exact-part-detail",
    landPatternSourceId: "tdk-exact-part-detail",
    landPatternApplicability:
      "TDK exact-orderable report PA/PB/PC reflow ranges apply as manufacturer guidance; the associated image is reference-only, not released CAD"
  },
  affectedReferences,
  primaryProductPageUrl,
  sourceControl: {
    basisCommit: "f85f5c35937bc36ee52fc8b5799890150a3adf59",
    upstreamSources: [
      {
        path: "packages/scoring-circuit/src/one-channel-analog-readiness.ts",
        sha256: "496d8727b33209c03b31f2b1f203397c7cab364f40d00edf2ec8b1bdf227e55d"
      }
    ]
  },
  sources: [
    {
      id: "tdk-exact-part-detail",
      authority: "tdk-product-report-mirrored-by-farnell",
      applicability: "exact-orderable-identity-package-electrical-and-land-guidance",
      artifactPath: "packages/scoring-circuit/docs/evidence/m4-04/tdk-cga3e3x7r1h105k080ab-detail.pdf",
      sourceUrl: "https://www.farnell.com/datasheets/4491451.pdf",
      manufacturerProductPageUrl: primaryProductPageUrl,
      reviewedPages: [1, 2, 3],
      sha256: "8692A2973DD875110C6F3FE3EB0A688454C0A155DCC8FCFAF3130F6862EC316F",
      scope:
        "Exact CGA3E3X7R1H105K080AB product report. Page 1 names the orderable, package, electrical rating, operating range, and PA/PB/PC reflow guidance; page 2 retains the DC-bias reference graph; page 3 retains the associated land image as reference material, not project CAD."
    },
    {
      id: "tdk-automotive-catalog",
      authority: "manufacturer-primary",
      applicability: "manufacturer-family-package-and-electrical-context",
      artifactPath: "packages/scoring-circuit/docs/evidence/m4-04/tdk-mlcc-automotive-general-zh.pdf",
      sourceUrl:
        "https://product.tdk.cn/system/files/dam/doc/product/capacitor/ceramic/mlcc/catalog/mlcc_automotive_general_zh.pdf",
      manufacturerProductPageUrl: primaryProductPageUrl,
      sha256: "E6F5803E89514DC61813BF2C96414D784005274730A8AF43A5EF54EBD546DBFF",
      scope:
        "TDK automotive MLCC catalog. It identifies CGA3 as 1608 and EIA 0603, records the CGA3 body dimensions, and lists the exact 1 uF X7R orderable."
    },
    {
      id: "tdk-virtual-component-library-parts-list",
      authority: "manufacturer-primary",
      applicability: "manufacturer-exact-orderable-identity-only",
      artifactPath: "packages/scoring-circuit/docs/evidence/m4-04/tdk-mlcc-virtual-component-library-parts-list.pdf",
      sourceUrl: "https://product.tdk.cn/system/files/dam/tvcl/partslist_mlcc_en.pdf",
      manufacturerProductPageUrl: primaryProductPageUrl,
      sha256: "B71416318033D9E0E50E4386A758FE58E133805F289B0200DFDE983B4FDA6DA4",
      scope:
        "TDK virtual-component-library parts list. It confirms the exact orderable under the CGA3 automotive general family; it is identity/model evidence and does not supply project geometry."
    }
  ],
  manufacturerCad: {
    state: "not-acquired",
    artifactPath: null,
    authority: "deny",
    note: "Manufacturer CAD was not acquired or retained for this candidate."
  },
  package: {
    designation: "CGA3 (1608 / EIA 0603)",
    lengthMm: { nominal: 1.6, plus: 0.2, minus: 0.1 },
    widthMm: { nominal: 0.8, plus: 0.2, minus: 0.1 },
    thicknessMm: { nominal: 0.8, plus: 0.2, minus: 0.1 },
    terminalWidthMm: { minimum: 0.2 },
    terminalSpacingMm: { minimum: 0.3 }
  },
  manufacturerLandPattern: {
    designation: "TDK CGA3 recommended land pattern",
    sourceId: "tdk-exact-part-detail",
    sourceScope:
      "TDK exact-orderable report page 1 PA/PB/PC ranges; page 3 associated image is reference-only, not CAD",
    applicability:
      "manufacturer-published exact-orderable land guidance, not an exact-orderable CAD object or released footprint",
    flow: {
      paGapMm: { minimum: 0.7, maximum: 1.0 },
      pbPadLengthMm: { minimum: 0.8, maximum: 1.0 },
      pcPadWidthMm: { minimum: 0.6, maximum: 0.8 }
    },
    reflow: {
      paGapMm: { minimum: 0.6, maximum: 0.8 },
      pbPadLengthMm: { minimum: 0.6, maximum: 0.8 },
      pcPadWidthMm: { minimum: 0.6, maximum: 0.8 }
    }
  },
  projectSelection: {
    solderingMethod: "reflow",
    rationale: "Midpoint selection within the TDK CGA3 PA, PB, and PC reflow ranges.",
    manufacturerParameterSelectionMm: { paGap: 0.7, pbPadLength: 0.7, pcPadWidth: 0.7 },
    copperPad: { lengthMm: projectCopperPadLengthMm, widthMm: projectCopperPadWidthMm },
    overallLandSpanMm: projectOverallLandSpanMm,
    derivedCopperPadGapMm: projectCopperPadGapMm,
    derivedCopperPadCenterXMm: projectCopperPadCenterXMm,
    solderMask: {
      openingLengthMm: projectSolderMaskOpeningLengthMm,
      openingWidthMm: projectSolderMaskOpeningWidthMm,
      marginPerEdgeMm: projectSolderMaskMarginMm,
      derivation: "0.7 mm copper dimension + 2 x 0.05 mm project mask margin",
      status: "project-input"
    },
    paste: {
      openingLengthMm: projectPasteOpeningLengthMm,
      openingWidthMm: projectPasteOpeningWidthMm,
      reductionPerEdgeMm: projectPasteReductionPerEdgeMm,
      derivation: "0.7 mm copper dimension - 2 x 0.05 mm project paste reduction",
      status: "project-input"
    },
    courtyard: {
      lengthMm: projectCourtyardLengthMm,
      widthMm: projectCourtyardWidthMm,
      minimumClearanceMm: projectCourtyardClearanceMm,
      derivation:
        "max(2.1 mm land span, 1.8 mm maximum package length) + 2 x 0.15 mm; max(1.0 mm package width, 0.7 mm pad width) + 2 x 0.15 mm",
      status: "project-review-input"
    }
  },
  terminals: [
    { pad: "1", terminal: "A", polarity: "non-polar", xMm: -projectCopperPadCenterXMm, yMm: 0 },
    { pad: "2", terminal: "B", polarity: "non-polar", xMm: projectCopperPadCenterXMm, yMm: 0 }
  ],
  stressOrientationReview: {
    state: "pending-review",
    polarity: "non-polar",
    pinOne: "not-applicable",
    ratedVoltageVdc: 50,
    nominalApplicationRailVdc: 5,
    operatingTemperatureC: { minimum: -55, maximum: 125 },
    dcBiasEvidence: "page-2-reference-graph-retained-not-guaranteed",
    note: "The ceramic capacitor has no polarity or pin-one requirement. Rotation is functionally equivalent, but rail transient, effective capacitance at the application bias, placement, and assembly-clearance review remain open."
  },
  dcBiasReview: {
    sourceId: "tdk-exact-part-detail",
    reviewedPage: 2,
    status: "reference-graph-retained-not-guaranteed",
    ratedVoltageVdc: 50,
    nominalApplicationRailVdc: 5,
    graphVoltageRangeVdc: { minimum: 0, maximum: 50 },
    disposition:
      "The retained DC Bias Characteristic graph shows effective capacitance declining with applied DC voltage; it does not publish a guaranteed minimum capacitance at the 5 V application rail.",
    designLimit:
      "Do not release the nominal 1 uF value as the biased value; validate effective capacitance, transient response, and reference-input charge-bucket behavior at the actual rail and temperature."
  },
  placementReview: {
    sourceId: "tdk-exact-part-detail",
    reviewedPages: [1, 3],
    packageMaximumMm: { length: 1.8, width: 1.0, thickness: 1.0 },
    projectCourtyardMm: { length: projectCourtyardLengthMm, width: projectCourtyardWidthMm },
    boardPlacementStatus: "not-reviewed",
    placementLimits:
      "No board-edge, neighboring-component, keepout, assembly-clearance, or board-fit limit is published or accepted by this candidate."
  },
  orientation: {
    state: "pending-review",
    assemblyRotationDeg: null,
    datum: "local two-terminal axis",
    note: "No pin-one or polarity orientation is asserted by this project candidate."
  },
  artwork: {
    state: "generated-project-review-only",
    representation: "canonical-rendered-footprint-soup-geometry",
    generator: "tscircuit",
    generatorVersion: "0.0.2271",
    sha256: artworkSha256,
    authority: "deny"
  },
  releaseState: "deny",
  fabricationAuthority: "deny",
  accepted: false
} as const

const expectedEvidence = deepFreeze(structuredClone(evidenceDefinition))
export const benchPrototypeTdkCga3ProjectFootprintGeometry = deepFreeze(evidenceDefinition)

function isExpectedEvidence(value: unknown): value is typeof evidenceDefinition {
  return sameDataGraph(value, expectedEvidence)
}

/** Empty output means the bounded TDK candidate remains exact, review-only, and denied. */
export function validateBenchPrototypeTdkCga3ProjectFootprint(
  candidate: unknown = benchPrototypeTdkCga3ProjectFootprintGeometry
): readonly string[] {
  if (!isExpectedEvidence(candidate)) {
    return ["BP-031 TDK CGA3 identity, bias/placement evidence, artwork, or deny state drifted"]
  }
  const evidence = candidate
  const errors: string[] = []
  const exactSource = evidence.sources.find((source) => source.id === "tdk-exact-part-detail")
  if (
    evidence.workUnit !== "BP-031" ||
    evidence.manufacturer !== "TDK Corporation" ||
    evidence.manufacturerPartNumber !== "CGA3E3X7R1H105K080AB" ||
    evidence.sourceContract !== "BP-101" ||
    evidence.role !== "reference input bypass" ||
    JSON.stringify(evidence.affectedReferences) !==
      JSON.stringify(["C_REF_IN_1", "C_REF_IN_2", "C_REF_IN_3", "C_REF_IN_4", "C_REF_IN_5", "C_REF_IN_6", "C_REF_IN_7"])
  ) {
    errors.push("TDK exact orderable identity or seven-reference scope drifted")
  }
  if (
    evidence.sourceBinding.canonicalSourceReference !== "C_REF_IN" ||
    evidence.sourceBinding.replicatedReferencePrefix !== "C_REF_IN_" ||
    evidence.sourceBinding.manufacturer !== "TDK" ||
    evidence.sourceBinding.manufacturerPartNumber !== "CGA3E3X7R1H105K080AB" ||
    evidence.sourceBinding.package !== "0603" ||
    evidence.sourceBinding.exactOrderableSourceId !== "tdk-exact-part-detail" ||
    evidence.sourceBinding.landPatternSourceId !== "tdk-exact-part-detail" ||
    !evidence.sourceBinding.landPatternApplicability.includes("reference-only")
  ) {
    errors.push("TDK exact-source and land-guidance binding drifted")
  }
  if (
    evidence.sourceControl.basisCommit !== "f85f5c35937bc36ee52fc8b5799890150a3adf59" ||
    evidence.sourceControl.upstreamSources[0]?.path !==
      "packages/scoring-circuit/src/one-channel-analog-readiness.ts" ||
    evidence.sourceControl.upstreamSources[0]?.sha256 !==
      "496d8727b33209c03b31f2b1f203397c7cab364f40d00edf2ec8b1bdf227e55d"
  ) {
    errors.push("TDK upstream source-control hashes drifted")
  }
  if (
    exactSource === undefined ||
    exactSource.applicability !== "exact-orderable-identity-package-electrical-and-land-guidance" ||
    exactSource.manufacturerProductPageUrl !== evidence.primaryProductPageUrl ||
    JSON.stringify(exactSource.reviewedPages) !== JSON.stringify([1, 2, 3]) ||
    !/^[0-9A-F]{64}$/u.test(exactSource.sha256)
  ) {
    errors.push("TDK exact retained source identity or hash drifted")
  }
  if (
    evidence.manufacturerLandPattern.sourceId !== "tdk-exact-part-detail" ||
    !evidence.manufacturerLandPattern.sourceScope.includes("page 1") ||
    !evidence.manufacturerLandPattern.sourceScope.includes("page 3") ||
    !evidence.manufacturerLandPattern.applicability.includes("not an exact-orderable CAD") ||
    evidence.manufacturerLandPattern.reflow.paGapMm.minimum !== 0.6 ||
    evidence.manufacturerLandPattern.reflow.paGapMm.maximum !== 0.8 ||
    evidence.manufacturerLandPattern.reflow.pbPadLengthMm.minimum !== 0.6 ||
    evidence.manufacturerLandPattern.reflow.pbPadLengthMm.maximum !== 0.8 ||
    evidence.manufacturerLandPattern.reflow.pcPadWidthMm.minimum !== 0.6 ||
    evidence.manufacturerLandPattern.reflow.pcPadWidthMm.maximum !== 0.8
  ) {
    errors.push("TDK reflow land ranges or CAD applicability drifted")
  }
  if (
    evidence.manufacturerLandPattern.flow.paGapMm.minimum !== 0.7 ||
    evidence.manufacturerLandPattern.flow.paGapMm.maximum !== 1.0 ||
    evidence.manufacturerLandPattern.flow.pbPadLengthMm.minimum !== 0.8 ||
    evidence.manufacturerLandPattern.flow.pbPadLengthMm.maximum !== 1.0 ||
    evidence.manufacturerLandPattern.flow.pcPadWidthMm.minimum !== 0.6 ||
    evidence.manufacturerLandPattern.flow.pcPadWidthMm.maximum !== 0.8
  ) {
    errors.push("TDK flow-solder land ranges drifted")
  }
  if (
    evidence.dcBiasReview.sourceId !== "tdk-exact-part-detail" ||
    evidence.dcBiasReview.reviewedPage !== 2 ||
    evidence.dcBiasReview.status !== "reference-graph-retained-not-guaranteed" ||
    evidence.dcBiasReview.ratedVoltageVdc !== 50 ||
    evidence.dcBiasReview.nominalApplicationRailVdc !== 5 ||
    evidence.dcBiasReview.graphVoltageRangeVdc.minimum !== 0 ||
    evidence.dcBiasReview.graphVoltageRangeVdc.maximum !== 50 ||
    evidence.stressOrientationReview.dcBiasEvidence !== "page-2-reference-graph-retained-not-guaranteed"
  ) {
    errors.push("TDK DC-bias evidence or application limit drifted")
  }
  if (
    evidence.placementReview.sourceId !== "tdk-exact-part-detail" ||
    JSON.stringify(evidence.placementReview.reviewedPages) !== JSON.stringify([1, 3]) ||
    evidence.placementReview.packageMaximumMm.length !== 1.8 ||
    evidence.placementReview.packageMaximumMm.width !== 1.0 ||
    evidence.placementReview.packageMaximumMm.thickness !== 1.0 ||
    evidence.placementReview.boardPlacementStatus !== "not-reviewed" ||
    evidence.projectSelection.courtyard.lengthMm !== 2.4 ||
    evidence.projectSelection.courtyard.widthMm !== 1.3
  ) {
    errors.push("TDK placement limits or review boundary drifted")
  }
  if (
    evidence.stressOrientationReview.state !== "pending-review" ||
    evidence.stressOrientationReview.polarity !== "non-polar" ||
    evidence.stressOrientationReview.pinOne !== "not-applicable" ||
    evidence.orientation.state !== "pending-review" ||
    evidence.orientation.assemblyRotationDeg !== null
  ) {
    errors.push("TDK orientation and stress review must remain pending")
  }
  if (
    evidence.manufacturerCad.state !== "not-acquired" ||
    evidence.manufacturerCad.artifactPath !== null ||
    evidence.manufacturerCad.authority !== "deny" ||
    evidence.artwork.state !== "generated-project-review-only" ||
    evidence.artwork.generator !== "tscircuit" ||
    evidence.artwork.generatorVersion !== "0.0.2271" ||
    evidence.artwork.sha256 !== artworkSha256 ||
    evidence.artwork.authority !== "deny" ||
    evidence.releaseState !== "deny" ||
    evidence.fabricationAuthority !== "deny" ||
    evidence.accepted
  ) {
    errors.push("TDK CAD, artwork, release, and fabrication authority must remain denied")
  }
  return errors
}

const projectFootprint = (
  <footprint name="BP031_TDK_CGA3E3X7R1H105K080AB_PROJECT_FOOTPRINT" originalLayer="top">
    <smtpad
      name="1"
      pcbX={-projectCopperPadCenterXMm}
      pcbY={0}
      shape="rect"
      solderMaskMargin={`${projectSolderMaskMarginMm}mm`}
      solderPasteMargin={`-${projectPasteReductionPerEdgeMm}mm`}
      width={`${projectCopperPadWidthMm}mm`}
      height={`${projectCopperPadLengthMm}mm`}
      portHints={["1", "A", "non-polar"]}
    />
    <smtpad
      name="2"
      pcbX={projectCopperPadCenterXMm}
      pcbY={0}
      shape="rect"
      solderMaskMargin={`${projectSolderMaskMarginMm}mm`}
      solderPasteMargin={`-${projectPasteReductionPerEdgeMm}mm`}
      width={`${projectCopperPadWidthMm}mm`}
      height={`${projectCopperPadLengthMm}mm`}
      portHints={["2", "B", "non-polar"]}
    />
    {/* This review courtyard is project geometry, not manufacturer CAD. */}
    <courtyardrect
      pcbX={0}
      pcbY={0}
      width={`${projectCourtyardLengthMm}mm`}
      height={`${projectCourtyardWidthMm}mm`}
      strokeWidth="0.05mm"
    />
  </footprint>
)

export interface BenchPrototypeTdkCga3ProjectFootprintProps {
  readonly pcbRotation?: number
  readonly pcbX?: number
  readonly pcbY?: number
}

/** Isolated tscircuit component for the BP-031 exact-capacitor review. */
export function BenchPrototypeTdkCga3ProjectFootprint({
  pcbRotation,
  pcbX,
  pcbY
}: BenchPrototypeTdkCga3ProjectFootprintProps = {}): ReactElement {
  return (
    <chip
      name="C_BP031_TDK_CGA3E3X7R1H105K080AB"
      manufacturerPartNumber="CGA3E3X7R1H105K080AB"
      pinLabels={{ pin1: "A", pin2: "B" }}
      footprint={projectFootprint}
      pcbRotation={pcbRotation}
      pcbX={pcbX}
      pcbY={pcbY}
    />
  )
}

export default BenchPrototypeTdkCga3ProjectFootprint
