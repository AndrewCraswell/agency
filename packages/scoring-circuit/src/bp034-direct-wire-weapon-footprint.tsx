...
/**
 * Isolated, project-derived footprint for BP-034's owner-approved temporary
 * direct-wire option. It is not a manufacturer CAD pattern and is deliberately
 * not imported into a board model.
 */
export const bp034DirectWireWeaponFootprint = {
  ...
  review: {
    state: "root-approved-pre-order-interface",
  ...
  boardImport: {
    state: "deny",
    rule: "Isolated review footprint only; no scoring-board import or placement is authorized."
  },
  physicalEvidence: {
    state: "open",
...
export function validateBp034DirectWireWeaponFootprint(
  ...
  if (
    ...
    value.review.state !== "root-approved-pre-order-interface" ||
    ...
    value.boardImport.state !== "deny" ||
    value.physicalEvidence.state !== "open" ||
    value.productionSocket.state !== "open" ||
    value.fabricationAuthority !== "deny" ||
    value.releaseState !== "deny"
  ) {
    throw new RangeError("BP-034 footprint must remain a denied, isolated USB-C-PD-preserving prototype artifact")
  }
  if (
...
/** Isolated tscircuit review component; it must not be instantiated on a board. */
