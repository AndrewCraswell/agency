...
function upstreamHash(path: string) {
  const relativePath = path.replace("packages/scoring-circuit/", "../")
  return createHash("sha256")
...
describe("BP-031 TI TPS7A2033PDBVR DBV0005A SOT-23-5 candidate footprint", () => {
  it("binds the exact orderable, package, retained TI source, and denied CAD disposition", () => {
    ...
    expect(oneChannelAnalogExperimentBom.find((part) => part.reference === "U_3V3")).toMatchObject({
      manufacturer: "Texas Instruments",
      mpn: "TPS7A2033PDBVR",
      package: "DBV SOT-23-5"
    })
    ...
    for (const upstream of bp031Tps7a2033PdbvrDbv0005aCandidateFootprint.sourceControl.upstreamSources) {
      expect(upstreamHash(upstream.path)).toBe(upstream.sha256)
    }
