...
The retained models cover the earlier carrier and selected circuits from the frozen combined board. They are not a
complete simulation of the active virtual box or the planned standalone HUB75 board. Run the ngspice suite with:
...
- `primary-regulator-startup.cir` is a separate **LTspice-only combined-board experiment**, not one of the ngspice acceptance
  models. It uses ADI's proprietary `LTC3130-1.sub`; that model is not redistributed here. Copy the deck to an output
  directory and run `LTspice.exe -b <deck-path> -I<model-directory>` (no space after `-I`). Install LTspice and its
  proprietary model outside the repository; extracted installers are not project source. It compares 4.1V and 20V
...
- The combined board's LTC3130 input-network model checks three explicitly assumed scenarios: a 4.75V source with
...
- The native STM32 acquisition design has a pair/reset sensing model and a seven-conductor scan model. The latter
...
The virtual box's SN6505 transformer supply and TPS62162 regulator are not covered by the combined board's LTC3130
experiments. Reconcile actual component values and conditions before applying any result to another board.
