...
## ESP32-only BP-123 and BP-144 reconciliation

The footprint ledger follows the simplified P0 BOM. The active BP-123 records
are exactly U_APP_SUPERVISOR, U_APP_WDOG,
C_APP_SUPERVISOR_CT_AND_BYPASS (quantity two), C_APP_WDOG_BYPASS,
R_APP_WDOG_TIMEOUT, R_APP_WDI_PULLUP, R_ESP_EN_PULLUP, and C_ESP_EN_DELAY.
The previous R_W5500_RESET_PULLUP and U_APP_RESET_FANOUT references are not
selected in the ESP32-only prototype and are intentionally absent.

BP-144 is represented by the exact active BOM reference set:
U_DISPLAY_BUFFER_A, U_DISPLAY_BUFFER_B, R_HUB75_SIGNAL_DEFAULTS (quantity
16), R_HUB75_PANEL_OE_PULLUP, C_HUB75_BUFFER_BYPASS (quantity two),
Q_DISPLAY_ENABLE, R_DISPLAY_ENABLE_PULLUP_AND_GATE (quantity two), and
R_DISPLAY_ENABLE_GATE_PD. The old per-buffer enable and per-signal pull
reference names are not retained. Shared logical references preserve the BOM
quantity without inventing duplicate footprint identities.
...
F-RAM, RTC, secure element, audio amplifier, speaker, and external antenna are
explicitly DNP. F-RAM is represented only in `omittedPeripherals`; it has no
active ledger record, footprint mapping, support resistor mapping, or bypass
capacitor mapping. ESP-IDF encrypted NVS is the bounded P0 persistence path.
In particular, audio must remain DNP because GPIO35 is the selected
encrypted-IR RMT input. The TSOP38438 and its support network are included,
but its optical aperture, front-panel coupon, electrical timing, range, flood,
and power-off tests remain BP-146 gates.
