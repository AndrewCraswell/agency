...
| `U_USB_PORT_PROTECT` | TI `TPD4S201TRGRRQ1` | Protects CC1, CC2, D+, and D- against IEC ESD and short-to-20 V VBUS. Connector CC1/2 enter its `C_CC1/C_CC2`; its `RPD_G1/G2` return to those own nodes for dead-battery attach; its protected `CC1/CC2` route to TPS25730A `CC1/CC2`. The 330 pF capacitors are on those protected controller nodes. `/FLT` goes to `FAULT_IN_N` with a 10 kOhm LDO pullup. `VPWR` comes from the PD LDO; VBIAS is 100 nF, 50 V X7R. |
...
| `U_BUCK_BOOST` | TI `TPS55288RPMR` | Takes only eFuse-protected negotiated input and generates V5. V3_3, isolated scoring power, display, audio, and all system rails derive downstream. No USB VBUS connection bypasses the PD path or reaches a rail. |
...
USB-C necessarily presents 5 V before a PD contract. `TPS25730ADREFR` is strapped to 20 V minimum and 20 V maximum; its documented `EnableSinkAfterContract` behavior combined with automatic disable on capability mismatch keeps PPHV off when the source cannot meet the 20 V, 3 A requirement. The controller's VBUS-powered dead-battery LDO runs the CC controller and `TPD4S201` before V3_3 exists; once V3_3 is present it powers `VIN_3V3` without backfeeding VBUS. Therefore the eFuse and TPS55288 never receive the initial 5 V contract, a non-PD source, or an inadequate PD source.
...
the 47 uF to 100 uF required range and it has no MLCC DC-bias loss. The model
The model also includes 10 uF VIN_3V3, 4.7 uF 50 V raw-VBUS, and 330 pF
...
- [TPD4S201-Q1 product page](https://www.ti.com/product/TPD4S201-Q1) and [datasheet](https://www.ti.com/lit/ds/symlink/tpd4s201-q1.pdf): four low-speed channels for CC and USB2 D+/D-, 28 V short-to-VBUS tolerance, and IEC ESD protection.
