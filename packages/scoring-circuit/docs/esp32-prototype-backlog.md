...
| `P0-07` | done | Place the complete PCB. | `P0-06` | Commit `fb52a79` closes placement for all 250 populated components on the 360 mm by 200 mm bench board. The strict placement gate reports zero inter-component footprint, pad-clearance, courtyard, or placement failures and zero automatically packed components. All 17 focused prototype suites pass, 75 tests total; package types, focused lint, format, commit hooks, and root browser review of PCB, schematic, 3D, and external-I/O views pass. Commit `62acaf7` subsequently enables the strict routing gate and establishes the partial routing baseline now owned by `P0-08`; component placement remains closed. |
| `P0-08` | active | Route and review the PCB. | `P0-07` | Commit `62acaf7` establishes deterministic fail-closed routing and explicit USB/power and Ethernet/HUB75 placement. Commit `38a5ecb` explicitly places the acquisition island; `34bb24e` maps all 12 direct-wire electrical holes to PCB ports; `8a8eecd` adds the real inner-layer `APP_GND` plane and fanout phase. Commit `5198191` applies reviewed widths only to the retained power trunks, and `34d1ab3` opens a terminal escape corridor beneath the ESP32 antenna reserve without cutting the ground plane. The latest strict build routes 449 of 708 source connections and stops on 229 routing/DRC errors: 139 route failures, 41 disconnected ports, 41 missing traces, two pad-to-trace clearances, and six via clearances. Forced ESP32 routes were rejected after they regressed the build and are not retained. The provisional four-layer supplier candidate is documented, but exact USB/Ethernet pair geometry remains a manufacturing-input gate because the installed pair primitive cannot model the required protection, termination, and bias branches. Remaining local routes and final DRC clearance are active. |
...
The active milestone is `P0-08`: placement, the ground plane, reviewed power widths, and the ESP32 terminal escape
corridor are committed. The next handoff is constrained local routing and clearance of every remaining design-rule
violation and unrouted connection before manufacturing output begins; stack-up-dependent pair geometry is confirmed
with the selected supplier during `P0-09` rather than guessed in the router.
