...
| Add red and green scoring lights | active | USB-C power is complete. Next, add only the minimum directly visible red and green scoring indicators needed for bench scoring tests. |
...
| Order and assemble the board | active | The current board has 34 modeled source components, including the exact Adafruit USB-C PD and Pololu regulator assemblies added in `8deff2e`. Exact manufacturer STEP geometry is used for every populated item and no fabricated JSCAD boxes remain. Do not order yet: red/green indicators, sound, and repeater-light interfaces are still being added. |
