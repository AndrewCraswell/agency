...
| ID | Status | Latest state | Deliverable | Dependencies | Done when |
| --- | --- | --- | --- | --- | --- |
| `BP-320` | in-progress | The obsolete 738-line dual-MCU model is replaced in the working tree by the canonical empty board, hierarchy, global nets, and four mounting holes; focused tests and generated desktop/mobile preview pass. | Maintain one canonical clean-sheet circuit source. | `BP-000`, `BP-040` | Source, title/revision, hierarchy, global nets, provisional outline, and no-obsolete-circuit test are committed. |
...
| ID | Status | Latest state | Deliverable | Dependencies | Done when |
| --- | --- | --- | --- | --- | --- |
| `BP-521` | in-progress | Target-neutral C HAL for explicit microsecond time, seven-channel ADC observations, safe-inactive outputs, IR edges, and opaque bounded Ethernet is delivered; native coverage reports 100 percent lines/functions and 89.77 percent branches for the new HAL. Correct-worktree reconciliation is active. | Freeze the ESP32 adapter boundary. | `BP-000` | C interfaces contain no ESP-IDF, wall clock, display, network-policy, remote-command, or scoring-rule dependency and meet coverage gates. |
