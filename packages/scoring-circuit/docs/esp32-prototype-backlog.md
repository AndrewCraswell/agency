...
| `P0-06` | active | Reconcile BOM and footprints and clear schematic errors. | `P0-05` | Commit `818df85` root-approves the exact ESP32-S3-WROOM-1-N16R2 placement footprint from retained official DXF, STEP, and datasheet evidence; no WROOM-1U substitution remains. Acquisition, USB/power, digital, output/display/IR, physical-BOM, and integrated-inventory slices remain under implementation or root revision. The old 256 source-component count is not an acceptance target: the final physical count will exclude retired DNP/carrier artifacts and the two explicit non-BOM weapon/piste board features. |
