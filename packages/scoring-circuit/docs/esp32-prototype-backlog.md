...
| Add two repeater-light interfaces | done | Commit `9a1c318` adds two identical transmit-only RS-422/FPA outputs without adding another Ethernet controller or switch. One TI AM26LV31EIPWR drives both NorComp 182-009-113R161 right-angle male DB9 connectors from ESP32 GPIO43/UART TX. Each port uses the documented pins 3 (Tx-), 4 (Tx+), and 6/7 (ground). Exact manufacturer DB9 and KiCad TSSOP STEP geometry, editable KiCad files, Gerbers, drills, BOM, and placement files are committed. Two focused test files with 12 tests, package typecheck, lint, formatting, routing/export, and desktop/mobile browser inspection passed. All 129 connections route with zero unresolved nets and zero routing/DRC errors. Full-repository verification again stopped only on the unrelated `@repo/shopify-emails` coverage thresholds. |
| Order and assemble the board | active | The committed board now has all requested prototype interfaces and 42 modeled source components: seven scoring conductors, Ethernet, encrypted IR, WS2812 display, USB-C PD power, red/green indicators, sound, and two FPA repeater outputs. Exact STEP geometry is used for every populated item and no fabricated JSCAD boxes remain. The remaining work is final order-package inspection, procurement, assembly, and electrical bring-up; it is no longer waiting on another circuit block. |
