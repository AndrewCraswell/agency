...
| `P0-06` | active | Reconcile BOM and footprints and clear schematic errors. | `P0-05` | Commits `818df85`, `02803b1`, and `012b409` approve the WROOM-1, non-BOM weapon/piste landings, and complete acquisition footprint set. Commit `a4eeec8` approves the canonical Ethernet/HUB75 footprints and removes the repeated-render timeout defect. Commit `c6dfb1c` approves the exact TI supervisor/watchdog footprints and replaces six recovery DNP placeholders with physical non-BOM test pads. Commit `f0812c4` approves the canonical USB-C PD, protection, eFuse, 5 V, and 3.3 V footprints, removes retired link carriers, and routes USB data and power rails directly. Output/display/IR and the final physical-BOM/integrated-inventory gate remain under implementation or root review. The live schematic has 250 source components with zero source errors, duplicate references, placeholders, or DNP production components; the old 256 count is retired. |
| `P0-07` | waiting | Place the complete PCB. | `P0-06` | Root-reviewed readiness audit found the first placement corrections: move `J_WEAPON_DIRECT` inward from its board-edge crossing, rotate or relocate the inward-facing IR receiver, propagate the requested `U_APP` coordinates, move Ethernet outside the ESP32 antenna keepout, separate the co-located white LED and buzzer, and give USB-C, display, HUB75, RJ45, analog, and power islands explicit cable-facing placement. Integrated rendering currently also reports three pack-solver exhaustion errors. Begin these corrections immediately after `P0-06` closes. |
