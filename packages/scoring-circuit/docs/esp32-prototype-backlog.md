...
| Add USB-C power | done | Commit `381df31` replaces the loose-wire Adafruit 5991 arrangement with an Adafruit 5807 HUSB238 breakout fixed at 20 V. Its VOUT and GND pins solder directly through the carrier to the socketed Pololu D36V50F5 regulator; the terminal block is DNP and there are no power jumpers for the fabricator to install. The exact vendor STEP model, editable KiCad files, BOM, placement, Gerbers, and drills are committed. The complete board routes all 145 connections with zero unresolved nets and zero routing/DRC errors. |
...
| Add two repeater-light interfaces | done | Commit `381df31` removes the incorrect RS-422/FPA DB9 implementation and adds two independent FA-05 DATA-LINE outputs. Each TE 5520250-2 right-angle 6P4C RJ14 socket has its own 4N32M optocoupler, 82 ohm loop resistor, 680 kohm base resistor, and protection diode; only the UART input is shared, so the two repeater-supplied loops cannot backfeed each other. The exported contact numbers are the connector's physical pins 2 through 5. Exact manufacturer connector CAD, editable KiCad files, Gerbers, drills, BOM, and placement files are committed. The complete board routes all 145 connections with zero unresolved nets and zero routing/DRC errors. Physical validation with an actual FA-05 repeater remains part of bring-up, not a PCB dependency. |
| Order and assemble the board | active | Commit `381df31` leaves a 58-component, two-layer prototype with all requested interfaces: seven scoring conductors, Ethernet, encrypted IR, WS2812 display, direct-solder USB-C PD power, four scoring lamps, sound, and two FA-05 DATA-LINE outputs. All 58 placed components have rendered CAD bodies; 52 have explicit manufacturer numbers, while the remaining six are standard DevKitC sockets and direct-wire/display headers. All 145 electrical connections are routed with zero unresolved nets and zero routing/DRC errors, and the fabrication files are regenerated. Remaining work is order-package inspection, procurement, assembly, and electrical bring-up. |
