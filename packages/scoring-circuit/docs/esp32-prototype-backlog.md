...
| --- | --- | --- | --- |
| Simplified carrier definition | active | None | The prototype boundary is rewritten around ESP32-S3, WIZ850io Ethernet, an STUSB4500 PD module, a 5 V regulator module, and the retained custom scoring front end. Commit and root review remain. |
...
| Minimal carrier schematic | ready | Simplified carrier definition | Reuse the verified acquisition, HUB75, IR, output, and direct-landing circuits. Replace the discrete USB/power and Ethernet blocks with module sockets, remove the external supervisor/watchdog, and produce one readable schematic with zero connectivity errors. |
| Prototype PCB and order files | waiting | Minimal carrier schematic | Place the modules and custom front end on a four-layer carrier no larger than 200 mm by 140 mm. Route in a conventional PCB editor, clear its DRC, review Gerbers/drills/BOM/centroid, and order a small batch. Do not make completion depend on the tscircuit autorouter. |
