...
| `P0-06` | done | Reconcile BOM and footprints and clear schematic errors. | `P0-05` | Root-approved commits `818df85`, `02803b1`, `012b409`, `a4eeec8`, and `c6dfb1c` reconcile the WROOM-1, direct landings, acquisition, Ethernet/HUB75, and ESP32-support families. Commits `f0812c4` and `8cd35af` approve USB/power and output/display/IR footprints with retained manufacturer evidence; commit `f0aa9f7` enforces the integrated 250-reference BOM inventory. All 12 clean-sheet P0 suites pass, 53 tests total; the integrated inventory adds 3 passing tests; package type-check, focused lint, and format pass. The schematic has zero errors, duplicate references, placeholder MPNs, or DNP production components. Whole-board placement is intentionally owned by `P0-07`. |
| `P0-07` | active | Place the complete PCB. | `P0-06` | Dependency `P0-06` is closed. Start with the root-reviewed corrections: move `J_WEAPON_DIRECT` inward from its board-edge crossing, rotate or relocate the inward-facing IR receiver, propagate the requested `U_APP` coordinates, move Ethernet outside the ESP32 antenna keepout, separate the co-located white LED and buzzer, and give USB-C, display, HUB75, RJ45, analog, and power islands explicit cable-facing placement. Replace auto-packing with deliberate placement, then run edge, keepout, courtyard, and overlap checks. |
