...
| `P0-06` | active | Reconcile BOM and footprints and clear schematic errors. | `P0-05` | Commit `818df85` root-approves the exact ESP32-S3-WROOM-1-N16R2 placement footprint from retained official DXF, STEP, and datasheet evidence; no WROOM-1U substitution remains. Commit `02803b1` removes fake MPNs from the owner-approved weapon/piste solder landings and classifies them as the two explicit non-BOM board features. Acquisition, USB/power, digital, output/display/IR, physical-BOM, and integrated-inventory slices remain under implementation or root revision. The old 256 source-component count is not an acceptance target. |
