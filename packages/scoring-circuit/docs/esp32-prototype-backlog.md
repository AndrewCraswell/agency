...
| `P0-08` | active | Route and review the PCB. | `P0-07` | Commit `62acaf7` establishes deterministic fail-closed routing, fixes direct-landing port identity and the ESP32 exposed-pad/via model, and explicitly relays out the USB/power and Ethernet/HUB75 islands. Commit `38a5ecb` replaces the acquisition auto-grid with explicit conductor, mux, sense-buffer, converter, reference, and phase-control placement; root review passes the focused and integrated 250-component PCB suites, package types, lint, and format. The latest strict build routes 450 of 708 source traces and stops on 248 routing/DRC errors: 152 route failures, 40 disconnected ports, 46 missing traces, one pad-to-trace clearance, and nine via clearances. The analog relayout reduces its diagnostic records from 70 to 58 and reduces total routing/DRC errors from the committed baseline by 11. Four-layer planes/rules, critical-signal constraints, remaining local routing, and final DRC clearance remain active. |
...
The active milestone is `P0-08`: the deterministic routing baseline is committed, and the next handoff is explicit
analog-island placement followed by four-layer planes/rules, critical-signal routing, and clearance of every remaining
design-rule violation and unrouted connection before manufacturing output begins.
