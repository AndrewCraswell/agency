...
| `BP-050` | in-progress | USB-C/PD, eFuse, 5 V, and 3.3 V are exact. Commit `ee4d841` freezes the 4 A display branch, TPS259474A limiter, 6.3 A fuse, service disconnect, measurement link, panel pigtail, safe-off procedure, and current arithmetic; root review, 38 focused/related tests, package types, lint, and format pass. The analog/reference rail budget and one reconciled total remain. | Freeze the complete minimal power tree. | `BP-020` | One exact diagram and BOM cover USB-C PD, protection, eFuse, 5 V, 3.3 V, display branch, analog rails, startup, peak, fault, thermal, and measurement links without double counting. |
...
| `BP-104` | done | Commit `4fdd403` places the P0 interface on the canonical board: six 1.3 mm finished-hole weapon landings at 3.81 mm pitch, six separate probe holes, four 3.2 mm strain anchors, and a separate piste landing/probe/anchor. The production socket remains intentionally out of P0 scope. Root review, 11 focused tests, package types, lint, format, build, and browser verification pass. | Maintain the weapon-cable board interface. | `BP-020` | Six weapon landings plus piste, labels, finished holes, pitch, probes, strain relief, and de-energized continuity procedure are exact enough to place. |
...
| `BP-120` | in-progress | Commit `d07f096` selects `ESP32-S3-WROOM-1-N16R2`; commit `ecef0b0` reconciles the active pad-allocation consumers to that module. The 16 MB flash, 2 MB PSRAM, integrated antenna, RF clearance, and pad ownership are exact. Bypass, EN/BOOT, watchdog/reset, native USB/UART recovery, and unused-pad reconciliation remain. | Freeze ESP32 module, pins, support, reset, and recovery. | `BP-020` | Exact module, RF keepout, every pad owner, bypass, EN/BOOT, watchdog/reset, native USB, UART recovery, and unused-pad policy agree. |
| `BP-126` | done | Commit `ecef0b0` places the exact TSOP38438 and its 100 ohm supply filter, 100 nF bypass, 100 ohm output limiter, 10 kohm reset-safe pull-up, GPIO35 probe point, and front-edge optical keepout on the canonical board. Existing bounded RMT, authenticated handoff, replay/flood, and no-hit-authority contracts remain the firmware boundary. Root review, 38 focused tests, package types, lint, format, build, and browser verification pass. | Maintain the encrypted-IR hardware and adapter contract. | `BP-020` | Receiver/filter/protection/test point, optical placement, GPIO/RMT bounds, reset/flood behavior, and authenticated no-hit-authority handoff agree. |
