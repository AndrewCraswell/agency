...
| `P0-08` | active | Route and review the PCB. | `P0-07` | Commit `62acaf7` establishes deterministic fail-closed routing and explicit USB/power and Ethernet/HUB75 placement. Commit `38a5ecb` explicitly places the acquisition island; `34bb24e` maps all 12 direct-wire electrical holes to PCB ports; `8a8eecd` adds the real inner-layer `APP_GND` plane and fanout phase. Root review passes 11 focused/integrated PCB tests, package types, lint, and format. The latest strict build routes 456 of 708 source traces and stops on 236 routing/DRC errors: 146 route failures, 40 disconnected ports, 40 missing traces, one pad-to-trace clearance, and nine via clearances. This is six more routed connections and 12 fewer errors than the preceding checkpoint. Critical-signal constraints, remaining local routes, power widths, stack-up-dependent USB/Ethernet geometry, and final DRC clearance remain active. |
