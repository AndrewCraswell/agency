...
| Remove unused board circuitry | done | Commit `9ac2f11` records a root review of every source component, source port, and electrical connection. The duplicate WIZ850io chip-select pull-up, unnecessary TSOP38438 output pull-up, optional Pololu VRP/enable/power-good carrier positions, and redundant enable trace were removed. Both pins in each Pololu input/output power pair are now connected. The generated-product test allows only eleven spare DevKitC socket positions, the WIZ850io NC, and two 4N32M NC pins to remain unconnected. The routed build reports 144 of 144 connections, zero unresolved connections, and zero routing/DRC errors; 13 focused tests, types, lint, formatting, unused-code analysis, netlist inspection, and browser review of PCB, schematic, 3D, and external-I/O views passed. |
| Order and assemble the board | active | Commits `381df31`, `36f90a1`, `842faab`, `8353782`, and `9ac2f11` leave a 56-component, two-layer prototype with all requested interfaces: seven scoring conductors, Ethernet, encrypted IR, WS2812 display, direct-solder USB-C PD power, four scoring lamps, sound, and two FA-05 DATA-LINE outputs. The complete DevKitC carrier is aligned to both 22-pin sockets, the USB-C daughterboard is seated on its plated power pads, and both upright TE 5520250-2 repeater housings sit above and overhang the rear board edge with only their through-hole hardware below the PCB. All 56 placed components have rendered CAD bodies; 50 have explicit manufacturer numbers, while the remaining six are standard DevKitC sockets and direct-wire/display headers. All 144 electrical connections are routed with zero unresolved nets and zero routing/DRC errors. Root review, package tests, types, lint, formatting, unused-code analysis, routed build, netlist inspection, and browser review passed. Remaining work is to procure the parts and board, assemble it, and complete electrical bring-up. |
