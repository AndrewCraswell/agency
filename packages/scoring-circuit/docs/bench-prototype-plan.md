...
| `BP-035` | blocked | Root review synchronized the evaluator with BP-034's exact USB-C `USB2CC1M` and Ethernet `N201-003-BL` selections: `selection-blocked` is now zero. The current order candidate remains `DENY` with 672 explicit blockers across unresolved identity/package/population, 32 sample-evidence, 254 footprint-evidence, reference reconciliation, duplicate, and drift categories; BP-031 through BP-034 still own those closures. | Converge the lane BOMs into an order-candidate BOM. | `BP-031`, `BP-032`, `BP-033`, `BP-034` | Every proposed populated row is exact and footprint-approved; no proposed populated `TBD` remains; every omitted option is explicit DNP; lane netlists and reference sets are ready for schematic integration. |
...
| `BP-146` | blocked | Root-reviewed Vishay source geometry now drives hash-bound 1:1 overlays and a source-controlled tscircuit project footprint. The runtime footprint renders the exact 2.54 mm pin pitch, OUT/GND/VS order, 1.10 mm finished drills, geometrically circular 2.20 mm copper pads, 2.30 mm mask openings, zero paste, and package/lens review datums; focused rendering tests pass. Manufacturer CAD, footprint acceptance, board placement, optical authority, and fabrication remain denied. Closure still requires accepted optical layout and a measured panel coupon, plus handheld range, angle, latency, flood, reset, and power-off results. | Select and freeze the encrypted IR remote receiver path and optical test interface. | `BP-126`, `BP-142`, `BP-145` | Exact receiver/demodulator or decoder, wavelength/carrier, supply/filter/protection, connector or optical window assumptions, test points, footprint, current budget, numeric range/angle/light/latency targets, and stuck/noise/power-off behavior are reviewed. |
