...
| `BP-032` | in-progress | All 51 canonical references now have root-reviewed evidence and zero references remain not-started. Root-reviewed commits `591a8d1` and `fe702aa` retain and link the exact Murata `NXE1S0505MC` package, five-land guidance, pin map, isolation-domain boundary, project-review artwork, and corrected page-6 orientation to `U_ISO_POWER`. Root visual review caught and corrected the candidate's initial upper-left/lower-left pin-one contradiction; the accepted evidence now records pin 1 lower-left and pin 14 upper-left in the +Y-up top view. Exact manufacturer CAD, board placement, physical isolation/creepage, thermal behavior, schematic acceptance, release, and fabrication remain denied. The remaining work is promotion review across the 51 mapped candidates, preserving downstream physical gates under BP-300/BP-502/BP-507. Candidate verification passes 6 files/57 tests and canonical integration passes 4 files/38 tests, with package types, lint, format, and hooks clean. | Close processor, isolation, reset, clock, and debug footprints. | `BP-030`, `BP-122`, `BP-123`, `BP-124`, `BP-125` | Independent evidence record for every populated lane-C reference. |
| `BP-033` | in-progress | All dependencies are complete at their pre-order design boundaries. Root-reviewed commits `40fe73c` and `d9b3ed0` extend and link the shared exact KEMET `C0603C104K3RACTU` source/project geometry to six application-regulator, HUB75-buffer, IR-supply, and F-RAM bypass references. Root strengthened the validator to reject full descriptor and sibling-alias drift and refreshed one stale BP-031 closure digest. Current lane state is 59 reviewed-unapproved and 42 not-started references. Earlier root-reviewed mappings cover USB-C/PD/protection, branch fuses, TVS and source selection, both eFuse roles, eight W5500 bypass capacitors, four measurement links, 26 Yageo 10 kOhm rows, W5500, encrypted IR, and the HUB75 overlay. The largest delivered families awaiting root review are five TDK 22 uF rows and three Keystone test points; parallel work covers Yageo 100 kOhm, display MOSFETs, and display buffers. CAD, placement, mechanical/assembly, optical/powered behavior, acceptance, release, and fabrication remain denied; panel receipt and powered behavior belong to BP-502/BP-507. Canonical verification passes 3 files/53 tests with package types, lint, format, and hooks clean. | Close power, Ethernet, application, HUB75, display, and encrypted-IR receiver footprints. | `BP-030`, `BP-050`, `BP-141`, `BP-142`, `BP-143`, `BP-144`, `BP-145`, `BP-146` | Independent evidence record for every populated lane-D reference, including the BP-146 receiver path; audio remains explicit DNP with no host or I2C stub. |
...
| `BP-035` | blocked | BP-031, BP-032, and BP-033 are incomplete dependencies; BP-034 is complete. After root-reviewed shared-capacitor integration commit `d9b3ed0`, BP-031 is 113 reviewed-unapproved/0 not-started, BP-032 is 51/0, and BP-033 is 59/42, with zero approved lane rows. The current evaluator remains `DENY` with 333 blockers across 290 references: 22 unresolved MPN, 36 unresolved package, 34 unresolved population, 206 footprint-evidence, 7 missing-lane-reference, 8 missing-baseline-reference, 8 package-drift, and 12 population-drift; `selection-blocked` is zero. Reviewed-unapproved evidence intentionally does not grant footprint approval. | Converge the lane BOMs into an order-candidate BOM. | `BP-031`, `BP-032`, `BP-033`, `BP-034` | Every proposed populated row is exact and footprint-approved; no proposed populated `TBD` remains; every omitted option is explicit DNP; lane netlists and reference sets are ready for schematic integration. |
