...
| `BP-030` | done | Delivered: Freeze the footprint-evidence method and review template. | Freeze the footprint-evidence method and review template. | `BP-020` | Per-reference template binds exact MPN/package, manufacturer drawing/CAD, artwork digest, orientation, reviewer, and disposition. It does not claim closure before selections exist. |
| `BP-031` | blocked | Root-reviewed commits through `7529e98` plus `c723c8c` add review-only candidates for the populated analog/reference families including all seven `ADS8881IDGS` SAR converters. Root visual review verified TI pages 6-7 and 55-57 for the pin map and DGS0010A geometry, then required page 50 to bind the exact active `ADS8881IDGS` VSSOP(DGS)-10 orderable before acceptance. The integrated 25-test ADS8881/ledger/fabrication set plus package types, lint, format, source hashes, rendered geometry, and diff checks pass. Every candidate remains CAD-unacquired, orientation/process-pending, `accepted: false`, and fabrication-denied. BP-031 remains blocked only by incomplete dependency `BP-104`; remaining populated-footprint evidence, orientation, schematic reconciliation, and fabrication gates stay open. | Close analog, protection, reference, and weapon-fixture footprints. | `BP-030`, `BP-103`, `BP-104` | Independent evidence record for every populated lane-B reference; unresolved options remain explicit DNP. |
