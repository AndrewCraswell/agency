...
| `BP-146` | blocked | Exact Vishay source bytes are retained and hash-verified, and a fail-closed project-footprint/optical-coupon procedure now freezes every required drill, pad, mask, courtyard, orientation, 1:1 overlay, aperture, 3 mm project keepout, calibration, photo, and physical-gate evidence field without claiming manufacturer CAD. Closure still requires the generated project footprint and overlays, a measured panel coupon, and handheld range, angle, latency, flood, reset, and power-off results. | Select and freeze the encrypted IR remote receiver path and optical test interface. | `BP-126`, `BP-142`, `BP-145` | Exact receiver/demodulator or decoder, wavelength/carrier, supply/filter/protection, connector or optical window assumptions, test points, footprint, current budget, numeric range/angle/light/latency targets, and stuck/noise/power-off behavior are reviewed. |
