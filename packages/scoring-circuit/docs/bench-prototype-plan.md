...
| `BP-031` | blocked | Root-reviewed commits `74d10b4`, `c4efd08`, and `6dd2d41` add review-only candidates for the seven exact TDK `CGA3E3X7R1H105K080AB` references, the `ADA4177-1ARZ` R-8 amplifier, and all seven KEMET `T521B106M025ATE100` reference-output capacitors. Root visually verified the ADI and KEMET manufacturer drawings; the KEMET review confirms the 1411/3528 B-case package, terminal dimensions, and polarized end views, while explicitly treating its copper, mask, paste, courtyard, and local cathode/anode datum as project inputs pending independent assembly-orientation review. The latest 33 focused/dependent KEMET tests plus the prior TDK/ADA suites, package types, lint, format, retained-source hashes, rendered geometry, and diff checks pass. Every candidate remains CAD-unacquired, orientation/stress-pending, `accepted: false`, and fabrication-denied. BP-031 remains blocked only by incomplete dependency `BP-104`; remaining populated-footprint evidence, orientation, schematic reconciliation, and fabrication gates stay open. | Close analog, protection, reference, and weapon-fixture footprints. | `BP-030`, `BP-103`, `BP-104` | Independent evidence record for every populated lane-B reference; unresolved options remain explicit DNP. |
| `BP-032` | blocked | Root-reviewed commit `482e0ed` reconciles all 14 BP-125 processor-support references to exact selections, eliminating the eight formerly unresolved capacitor roles while retaining exact TI reset/watchdog and corrected `NXE1S0505MC` evidence. BP-032 remains blocked by incomplete dependencies `BP-123` and `BP-125`; remaining primary-source retention, manufacturer-CAD dispositions, project footprints, artwork, orientation, schematic sign-off, and fabrication evidence stay denied. | Close processor, isolation, reset, clock, and debug footprints. | `BP-030`, `BP-122`, `BP-123`, `BP-124`, `BP-125` | Independent evidence record for every populated lane-C reference. |
...
| `BP-104` | in-progress | Dependencies `BP-010` and `BP-103` are complete, so this task is not blocked. Root-reviewed commit `5339ec7` retains and hash-binds official Molex drawing evidence for `43045-1200`, `43025-1200`, `43030-0007`, and `44242-0005`, plus exact-MPN CAD-preview PDFs for the first, second, and fourth parts. The unlisted `43030-0007` CAD-pattern probe is conservatively recorded only as HTTP 404 with no claim that alternate Molex CAD is unavailable. Root verification passed 42 focused/dependent tests, package types, lint, format, all seven PDF hashes, exact-MPN PDF text checks, and diff checks. Fixture pinout, NC positions, continuity thresholds, and fail-closed intake are frozen; remaining work is received-part fit, labels, seven crimp/retention records, strain relief, rejected miswires, and calibrated continuity. | Freeze fixture-harness pinout and NC positions. | `BP-010`, `BP-103` | Drawing, continuity map, mating parts, labels, strain relief, and miswire test. |
