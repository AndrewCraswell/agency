...
| `BP-031` | in-progress | All dependencies are complete. Root-reviewed commit `e15ed5f` binds exact KEMET `C0603C104K3RACTU` evidence to `C_REF_REG_HF_1..7`; current lane state is 84 reviewed-unapproved and 29 not-started references. Earlier root-reviewed evidence covers the ADC, switch, ESD, buffer, reference, resistor, and capacitor families already mapped in the canonical closure. The remaining 29 references are seven `R_SOURCE` rows, seven `C_REF_REG` rows, seven `R_REF_SAR` rows, seven `C_REF` rows, and the fixture connector. CAD, placement, artwork acceptance, release, fabrication, and explicit DNP closure remain denied until their own review gates finish; physical fixture construction and continuity belong to BP-105/BP-502. The integrated C0603 evidence/closure/processor set passes 4 files/65 tests with package types, lint, and format clean. | Close analog, protection, reference, and weapon-fixture footprints. | `BP-030`, `BP-103`, `BP-104` | Independent evidence record for every populated lane-B reference; unresolved options remain explicit DNP. |
| `BP-032` | in-progress | All dependencies are complete. Root-reviewed commit `f09798d` links the already retained exact TI `SN74LVC2G07DCKR`, Nexperia `BSS138AKA`, and Samtec `FTSH-105-01-L-DV-007-K` evidence to three reset/fanout rows and `J_STM_SWD`. Together with shared-capacitor commit `e15ed5f`, current lane state is 29 reviewed-unapproved and 22 not-started references. Remaining work is exact evidence and canonical-reference closure for 12 Murata support capacitors, two supervisors, two watchdogs, two isolators, the STM32, ESP32 module, isolated converter, and ESP32 service header. CAD, placement, process acceptance, release, and fabrication remain denied; processor schematic sign-off belongs to BP-300 and physical mating/continuity to BP-502. The reset/header integration passes 4 files/34 tests with package types, lint, format, and hooks clean. | Close processor, isolation, reset, clock, and debug footprints. | `BP-030`, `BP-122`, `BP-123`, `BP-124`, `BP-125` | Independent evidence record for every populated lane-C reference. |
...
| `BP-035` | blocked | BP-031, BP-032, and BP-033 are incomplete dependencies; BP-034 is complete. After root-reviewed commits `e15ed5f` and `f09798d`, BP-031 is 84 reviewed-unapproved/29 not-started, BP-032 is 29/22, and BP-033 is 51/50, with zero approved lane rows. The current evaluator remains `DENY` with 390 blockers across 290 references: 22 unresolved MPN, 36 unresolved package, 34 unresolved population, 263 footprint-evidence, 7 missing-lane-reference, 8 missing-baseline-reference, 8 package-drift, and 12 population-drift; `selection-blocked` is zero. Reviewed-unapproved evidence intentionally does not grant footprint approval. | Converge the lane BOMs into an order-candidate BOM. | `BP-031`, `BP-032`, `BP-033`, `BP-034` | Every proposed populated row is exact and footprint-approved; no proposed populated `TBD` remains; every omitted option is explicit DNP; lane netlists and reference sets are ready for schematic integration. |
