...
| `BP-031` | blocked | Root-reviewed commits `74d10b4`, `c4efd08`, `6dd2d41`, and `da0f9a1` add review-only candidates for the seven exact TDK `CGA3E3X7R1H105K080AB` references, `ADA4177-1ARZ`, all seven KEMET `T521B106M025ATE100` capacitors, and all seven `TMUX1112PWR` source switches. Root visually verified the TI TMUX pin map, PW0016A outline, board-layout/mask example, and stencil pages; review corrected the page mapping before acceptance. The integrated 58-test footprint/ledger/fabrication set, package types, lint, format, source hashes, rendered geometry, and diff checks pass. Every candidate remains CAD-unacquired, orientation/process-pending, `accepted: false`, and fabrication-denied. BP-031 remains blocked only by incomplete dependency `BP-104`; remaining populated-footprint evidence, orientation, schematic reconciliation, and fabrication gates stay open. | Close analog, protection, reference, and weapon-fixture footprints. | `BP-030`, `BP-103`, `BP-104` | Independent evidence record for every populated lane-B reference; unresolved options remain explicit DNP. |
...
| `BP-104` | in-progress | Dependencies `BP-010` and `BP-103` are complete, so this task is not blocked. Root-reviewed commits `5339ec7` and `d195d47` retain official Molex evidence and correct the physical-evidence contract: `43045-1200`, `43025-1200`, and `44242-0005` require exact retained CAD, while loose crimp terminal `43030-0007` requires its exact retained drawing plus the explicit `not-acquired-pattern-probe-returned-404` disposition with null CAD fields instead of an impossible CAD artifact. Root verification passed 50 dependent tests, package types, lint, format, seven PDF hashes, and diff checks. Fixture pinout, NC positions, continuity thresholds, and fail-closed intake are frozen; remaining work is received-part fit, labels, seven crimp/retention records, strain relief, rejected miswires, and calibrated continuity. | Freeze fixture-harness pinout and NC positions. | `BP-010`, `BP-103` | Drawing, continuity map, mating parts, labels, strain relief, and miswire test. |
