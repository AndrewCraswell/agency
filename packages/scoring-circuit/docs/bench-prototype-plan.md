...
| `BP-020` | done | Root rebaselined and verified the executable BOM around one ESP32, external AFE, W5500 Ethernet, TSOP38438 IR, HUB75, USB-C PD, native-USB/test-pad recovery, and serialized primary outputs. STM32, processor isolation, isolated power/link, both populated service headers, alternate power input/selector, F-RAM, RTC, secure element, audio, speaker, and external antenna remain explicit DNP rows. Focused BOM/architecture/service/connector tests and package type-check pass in the root integration commit. | Rebaseline the ESP32-only prototype BOM separately from production. | `BP-000` | Every P0 reference is explicitly selected, `TBD`, or DNP; removed references are recorded as superseded or DNP rather than silently omitted; selected rows include MPN, lifecycle, quantity, source, and package. |
...
| `BP-050` | blocked | The selected USB-C PD, protection, eFuse, 20 V-to-5 V conversion, USB2 service, branch protection, and measurement evidence remain reusable. Remove `J_LAB_INJECTION`, `7101SYZQE`, and their dedicated harness from populated P0; retain labeled rail test pads/removable links under a USB-disconnected procedure. Recalculate the ESP32/AFE and display loads after BP-020. | Freeze the simplified USB-C PD power, protection, conversion, and branch measurement path. | `BP-020` | Exact USB-C/PD/protection/eFuse/V5 parts and support values, 20 V/3 A contract behavior, USB2 routing, branch protection/current links, display disconnect, no populated alternate input, and current-limited test-pad procedure. |
...
| `BP-121` | blocked | The previous ESP32 application-only allocation is invalid because ESP32 now owns acquisition, C17 execution, and primary outputs. Reallocate the exact module only after BP-127 fixes the external ADC cadence and required peripheral instances. GPIO35 remains reserved for IR unless the complete IR path is deliberately redesigned. | Reconcile the sole ESP32 module pad and peripheral allocation. | `BP-127` | Exact module-pad and peripheral-instance table covering external ADC SPI/GDMA and CONVST, W5500, HUB75, USB2, IR RMT, recovery, serialized primary outputs, watchdog, straps, RF keepout, and safe-state ownership with no double allocation. |
...
| `BP-123` | blocked | The existing reset/watchdog evidence is reusable only in part. Rewrite it for one ESP32 authority: one EN/reset tree, hardware-safe excitation and output disables, supervisor/watchdog health aggregation, fresh-baseline recovery, and no cross-domain reset fanout. | Close the ESP32 supervisor/watchdog/reset and output-safe-state network. | `BP-121` | Exact MPN/value schematic fragment, reset truth/timing review, clean generated ERC evidence, fail-closed capture protocol, and proof that only fresh acquisition/core/reference/output health can service the watchdog. |
...
| `BP-127` | blocked | Root approved the ESP32-S3-only P0 direction. It waits for the simplified BP-050 power contract. The old allocation frees eleven GPIOs by removing isolated SPI, dual heartbeats, optional I2C, F-RAM chip-select, and audio reservations; P0 needs three dedicated ADS8881 signals plus a shared-SPI output latch. Continuous seven-channel sampling replaces per-channel comparator GPIOs only if the timing experiment proves it. | Prove ESP32-S3-only P0 scoring feasibility and safety boundary. | `BP-050`, `BP-100`, `BP-101`, `BP-102` | Exact ADS8881 SPI/GDMA and CONVST interface, monotonic sampling schedule, scan/timestamp error budget, bounded queue/error model, preliminary pin/resource map, hardware-safe serialized primary outputs, watchdog contract, rail budget, and a one-cell falsification experiment under simultaneous Ethernet, HUB75, IR, USB, radio, and flash/cache stress. No paper result receives seven-channel or fabrication credit. |
...
| `BP-142` | blocked | The selected LMR43620 evidence remains reusable, but the 3.3 V load changed from an application domain to the sole scoring processor plus retained peripherals. Recalculate startup, transient, RF burst, ADC/reference interaction, branch current, and thermal margin after BP-127; remove the isolated scoring branch. | Freeze the single ESP32/AFE 3.3 V implementation from the 5 V rail. | `BP-050`, `BP-127` | Exact regulator/support BOM plus loaded startup, transient, RF, acquisition, current, and thermal calculation with separately measurable ESP32 and analog branches. |
...
| `BP-144` | blocked | The two-buffer HUB75 topology and fail-blank behavior remain selected, but its ESP32 pins and reset-enable source must be rebound after BP-121/BP-123. | Reconcile the two-buffer reset-safe HUB75 path with the sole ESP32. | `BP-121`, `BP-123`, `BP-143` | Schematic truth table proves black/high-impedance defaults; all 13 signals, pulls, pins, and reset-enable paths are present. |
| `BP-145` | blocked | P0 removes F-RAM, RTC, secure element, audio, speaker, and unused optional expansion. Encrypted remote keys/counters use eFuse-backed identity and encrypted NVS with a bounded persistence/wear contract; no flash write may interfere with scoring acquisition. Final confirmation waits on BP-121/BP-142. | Freeze the intentionally minimal P0 peripheral population. | `BP-121`, `BP-142` | Explicit DNP/removal table for F-RAM, RTC, secure element, audio, speaker, and unused expansion; retained eFuse/NVS key, counter, journal, write-timing, wear, and recovery policy. |
