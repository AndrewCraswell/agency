...
| `BP-032` | in-progress | All dependencies are complete. Root-reviewed commits `782070c` and `227ae6c` retain and link exact TI `ISO7762FDWR` and `ISO7721FDR` package, land-pattern, pin-direction, and orientation evidence to `U_ISO_MAIN` and `U_ISO_AUX`. Together with the earlier support-capacitor, reset, service-header, shared-capacitor, supervisor, and watchdog evidence, current lane state is 47 reviewed-unapproved and 4 not-started references. Remaining work is exact evidence and canonical-reference closure for the STM32, ESP32 module, isolated converter, and ESP32 service header. CAD, placement, physical-isolation acceptance, process acceptance, release, and fabrication remain denied; processor schematic sign-off belongs to BP-300 and physical mating/continuity to BP-502. Candidate verification passes 3 files/15 tests and canonical integration passes 5 files/36 tests, with package types, lint, format, and hooks clean. | Close processor, isolation, reset, clock, and debug footprints. | `BP-030`, `BP-122`, `BP-123`, `BP-124`, `BP-125` | Independent evidence record for every populated lane-C reference. |
...
| `BP-035` | blocked | BP-031, BP-032, and BP-033 are incomplete dependencies; BP-034 is complete. After root-reviewed isolator integration commit `227ae6c`, BP-031 is 113 reviewed-unapproved/0 not-started, BP-032 is 47/4, and BP-033 is 53/48, with zero approved lane rows. The current evaluator remains `DENY` with 343 blockers across 290 references: 22 unresolved MPN, 36 unresolved package, 34 unresolved population, 216 footprint-evidence, 7 missing-lane-reference, 8 missing-baseline-reference, 8 package-drift, and 12 population-drift; `selection-blocked` is zero. Reviewed-unapproved evidence intentionally does not grant footprint approval. | Converge the lane BOMs into an order-candidate BOM. | `BP-031`, `BP-032`, `BP-033`, `BP-034` | Every proposed populated row is exact and footprint-approved; no proposed populated `TBD` remains; every omitted option is explicit DNP; lane netlists and reference sets are ready for schematic integration. |
