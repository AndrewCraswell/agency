...
| `BP-020` | done | Root rebaselined the executable BOM around one ESP32, external AFE, W5500 Ethernet, TSOP38438 IR, HUB75, USB-C PD, native-USB/test-pad recovery, and direct hardware-safe primary outputs. The component-by-component audit removed the unnecessary serialized output latch, NXE1S0505 analog isolation converter and support, and coupon-local TPS7A2033 network. Aggregate USB-support and application-regulator placeholders were replaced with their exact physical rows: the required PD straps/capacitors/USB pair and all ten LMR43620 rail references. STM32, processor isolation, isolated power/link, populated service headers, alternate power input/selector, F-RAM, RTC, secure element, audio, speaker, and external antenna remain explicit DNP rows. Commits `bb02424` and `fe91804` leave the ADA4177 on common-ground `V5_ANALOG`/`V5_NEG` and reuse the sole LMR43620 `APP_3V3`; the latest 58 BOM/rail/footprint tests plus package types pass. | Rebaseline the ESP32-only prototype BOM separately from production. | `BP-000` | Every P0 reference is explicitly selected, `TBD`, or DNP; removed references are recorded as superseded or DNP rather than silently omitted; selected rows include MPN, lifecycle, quantity, source, and package. |
...
| `BP-050` | backlog | Root reviewed the simplified USB-C-only contract and integrated its eleven exact PD strap resistors, seven exact local/CC capacitors, and matched 22 ohm native-USB pair into the BOM. The exact eight-part power chain, TPS56A37 support, display limiter, de-energized measurement procedure, and provisional ESP32-only load screen remain. Remaining task work is explicit rather than dependency-blocked: close the AFE/radio/output rail budget, qualify or replace TVS2200 against the TPS25730A limit, and collect physical PD/inrush/load-step/cable-drop/trip/thermal/shutdown evidence. The focused P0 power/BOM suite passes. | Freeze the simplified USB-C PD power, protection, conversion, and branch measurement path. | `BP-020` | Exact USB-C/PD/protection/eFuse/V5 parts and support values, 20 V/3 A contract behavior, USB2 routing, branch protection/current links, display disconnect, no populated alternate input, and current-limited test-pad procedure. |
