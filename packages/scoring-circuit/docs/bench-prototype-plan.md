...
| `BP-033` | in-progress | All dependencies are complete at their pre-order design boundaries. Root-reviewed commits `25d5932` and `cc427a6` retain TDK's exact `C2012X7S1A226M125AC` page capture, separate family land guidance from project geometry, and link `C_DISPLAY_IN`, `C_DISPLAY_OUT`, and `C_APP_REG_OUT_A/B/C`. Current lane state is 67 reviewed-unapproved and 34 not-started references. Earlier root-reviewed mappings cover USB-C/PD/protection, branch fuses, TVS and source selection, both eFuse roles, eight W5500 bypass capacitors, four measurement links, 26 Yageo 10 kOhm rows, W5500, encrypted IR, HUB75, six shared KEMET 100 nF rows, and three Keystone test points. The largest delivered families awaiting root review are two display MOSFETs and two display buffers; a narrow Yageo 100 kOhm candidate is also queued. Manufacturer CAD, project CAD import, board placement, geometry/orientation acceptance, release, and fabrication remain denied; panel receipt and powered behavior belong to BP-502/BP-507. Canonical TDK verification passes 3 files/40 tests with package types, lint, format, and hooks clean. | Close power, Ethernet, application, HUB75, display, and encrypted-IR receiver footprints. | `BP-030`, `BP-050`, `BP-141`, `BP-142`, `BP-143`, `BP-144`, `BP-145`, `BP-146` | Independent evidence record for every populated lane-D reference, including the BP-146 receiver path; audio remains explicit DNP with no host or I2C stub. |
...
| `BP-035` | blocked | BP-031, BP-032, and BP-033 are incomplete dependencies; BP-034 is complete. After root-reviewed TDK integration commit `cc427a6`, BP-031 is 113 reviewed-unapproved/0 not-started, BP-032 is 51/0, and BP-033 is 67/34, with zero approved lane rows. The current evaluator remains `DENY` with 325 blockers across 290 references: 22 unresolved MPN, 36 unresolved package, 34 unresolved population, 198 footprint-evidence, 7 missing-lane-reference, 8 missing-baseline-reference, 8 package-drift, and 12 population-drift; `selection-blocked` is zero. Reviewed-unapproved evidence intentionally does not grant footprint approval. | Converge the lane BOMs into an order-candidate BOM. | `BP-031`, `BP-032`, `BP-033`, `BP-034` | Every proposed populated row is exact and footprint-approved; no proposed populated `TBD` remains; every omitted option is explicit DNP; lane netlists and reference sets are ready for schematic integration. |
