...
| `BP-033` | in-progress | All dependencies are complete at their pre-order design boundaries. Root-reviewed commits `9e2312a` and `4aa1bc6` retain and visually verify the exact active TI `SN74AHCT245PWR` orderable, its 20-pin PW/TSSOP pin map and mechanical outline, and link `U_DISPLAY_BUFFER_A/B`. Current lane state is 71 reviewed-unapproved and 30 not-started references. Earlier root-reviewed mappings cover USB-C/PD/protection, branch fuses, TVS and source selection, both eFuse roles, eight W5500 bypass capacitors, four measurement links, 26 Yageo 10 kOhm rows, W5500, encrypted IR, HUB75, six shared KEMET 100 nF rows, three Keystone test points, five TDK 22 uF rows, and two BSS138 display-enable MOSFETs. A narrow Yageo 100 kOhm candidate is queued; the remaining families are partitioned into non-overlapping display-limiter, application-regulator, W5500-support, reset, IR, F-RAM, and connector slices. TI publishes no PCB land pattern in the retained PDF, so copper/mask/paste/courtyard, CAD, orientation overlay, placement, schematic integration, release, and fabrication remain denied. Canonical buffer verification passes 3 files/44 tests with package types, lint, format, and hooks clean. | Close power, Ethernet, application, HUB75, display, and encrypted-IR receiver footprints. | `BP-030`, `BP-050`, `BP-141`, `BP-142`, `BP-143`, `BP-144`, `BP-145`, `BP-146` | Independent evidence record for every populated lane-D reference, including the BP-146 receiver path; audio remains explicit DNP with no host or I2C stub. |
...
| `BP-035` | blocked | BP-031, BP-032, and BP-033 are incomplete dependencies; BP-034 is complete. After root-reviewed display-buffer integration commit `4aa1bc6`, BP-031 is 113 reviewed-unapproved/0 not-started, BP-032 is 51/0, and BP-033 is 71/30, with zero approved lane rows. The current evaluator remains `DENY` with 321 blockers across 290 references: 22 unresolved MPN, 36 unresolved package, 34 unresolved population, 194 footprint-evidence, 7 missing-lane-reference, 8 missing-baseline-reference, 8 package-drift, and 12 population-drift; `selection-blocked` is zero. Reviewed-unapproved evidence intentionally does not grant footprint approval. | Converge the lane BOMs into an order-candidate BOM. | `BP-031`, `BP-032`, `BP-033`, `BP-034` | Every proposed populated row is exact and footprint-approved; no proposed populated `TBD` remains; every omitted option is explicit DNP; lane netlists and reference sets are ready for schematic integration. |
