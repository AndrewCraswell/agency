...
| `BP-031` | in-progress | All dependencies are complete. Root-reviewed commit `bd3c29c` retains official Vishay document 20043 for the exact CRCW-HP series, accepts the 0603/1206 HP body, terminal, recommended-reflow, and non-polar orientation evidence, and maps all 28 replicated `R_ESD`, `R_SOURCE_PD`, `R_SAR`, and `R_FAULT_GUARD` references while keeping exact-orderable CAD, board fit, release, and fabrication denied. Current lane state is 56 reviewed-unapproved and 57 not-started references. The five-file integration set passed 71 tests; after evidence deduplication the nine-file dependent set passed 108 tests with package types clean. Root-reviewed commit `0d8820d` corrects the canonical `REF5025AQDRQ1` package from the erroneous VSON-8 description to TI's exact D SOIC-8, 5.0 mm by 3.9 mm body, 1.27 mm pitch identity; the official addendum, D0008A outline, and 22 focused tests were independently verified, while the seven-reference footprint mapping remains the next review step. Root-reviewed commit `f331abb` maps the exact ADI `ADA4177-1ARZ` R-8 evidence to all seven `U_OVP_BUFFER_1..7` references. Root visually verified the Rev. E ordering guide, independent R-8 outline, lower-left pin-one datum, seven-reference mapping, and deny-state integrity. ADI `90-0096` remains an S8 family land-pattern input rather than exact ADA4177 geometry, so project geometry, CAD, board fit, release, and fabrication remain denied. The reconciled ADA4177/ADS8881/TMUX/TPD4 and dependent evidence set passes 7 files/69 tests with package types, lint, format, and diff checks clean. Earlier root-reviewed commits `90c69e3`, `1ba9dba`, and `4208421` cover all seven ADS8881 ADCs, TMUX1112 switches, and TPD4 ESD devices; earlier evidence covers KEMET, Vishay, Murata, Molex, negative-rail, and 3.3 V regulator inputs. Remaining work is the rest of lane-B footprint/orientation acceptance, including exact ADA4177 geometry or an explicit approved family equivalence, canonical-reference reconciliation, DC-bias/placement review, and explicit DNP closure; physical fixture construction and continuity are owned by BP-105/BP-502. | Close analog, protection, reference, and weapon-fixture footprints. | `BP-030`, `BP-103`, `BP-104` | Independent evidence record for every populated lane-B reference; unresolved options remain explicit DNP. |
