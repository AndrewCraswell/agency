...
| `BP-034` | backlog | USB-C and Ethernet cable samples are exact, source-backed selections and the five upstream connector contracts now fail closed on identity drift. The owner-validated OK Fencing weapon cable is accepted as the external three-socket interface; next freeze the individual socket identity and custom panel/short-harness mounting, then collect receipt, orientation, strain-relief, and continuity evidence after `BP-104` and `BP-143`. | Complete pre-order connector sample, mate, fit, pinout, and continuity review. | `BP-050`, `BP-104`, `BP-124`, `BP-141`, `BP-143` | Received sample identities, mating-part checks, orientation/photos, mechanical fit, harness pinout, strain relief, and continuity archive. No automated scoring fixture is required yet. |
...
| `BP-123` | blocked | Implemented exact supervisor, watchdog, and reset-network contract retained; blocked on cold-start, brownout, watchdog, manual-reset, cross-domain, and power-off physical captures plus downstream schematic integration. | Close both supervisor/watchdog/reset networks. | `BP-120`, `BP-121` | Exact MPN/value schematic and cold-start, brownout, watchdog, manual-reset, cross-domain, and power-off tests. |
...
| `BP-146` | blocked | Root-reviewed exact Vishay source bytes are retained and hash-verified for the TSOP384 datasheet, Minicast window guidance, and leaded assembly instructions. Datasheet-only 38 kHz AGC4, supply/current, 30 m laboratory-source distance, half-angle, and delay-rule characteristics are now executable without receiving prototype credit. Closure still requires an accepted board land pattern/CAD/drill/pad/orientation review, accepted optical layout and physical panel coupon, and handheld range, angle, latency, flood, reset, and power-off measurements. | Select and freeze the encrypted IR remote receiver path and optical test interface. | `BP-126`, `BP-142`, `BP-145` | Exact receiver/demodulator or decoder, wavelength/carrier, supply/filter/protection, connector or optical window assumptions, test points, footprint, current budget, numeric range/angle/light/latency targets, and stuck/noise/power-off behavior are reviewed. |
