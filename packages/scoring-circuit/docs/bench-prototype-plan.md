...
| `BP-123` | in-progress | Dependencies `BP-120` and `BP-121` are complete. Root-reviewed commits `4a60c5f` and `d89647f` add the fail-closed 11-net schematic preflight and a renderable review-only fragment containing the exact TPS389033DSER supervisors, TPS3431SDRBR watchdogs, SN74LVC2G07DCKR fanout, BSS138AKA reset sinks, and selected passives. Root verified the physical TI pin maps, every canonical endpoint set is connected and mutually disjoint, a synthetic cross-net short fails, and all BP-300/ERC/fabrication/physical authority remains false. Fifteen focused reset/fragment tests, package types, lint, and format pass. Next work is canonical BP-300 schematic source/PDF/ERC submission and review, followed by assembled-prototype cold-start, brownout, watchdog, manual-reset, cross-domain, and power-off captures. | Close both supervisor/watchdog/reset networks. | `BP-120`, `BP-121` | Exact MPN/value schematic and cold-start, brownout, watchdog, manual-reset, cross-domain, and power-off tests. |
