...
| `BP-103` | in-progress | Active against root-approved BP-100/BP-101/BP-102 working artifacts: close exact seven-cell/shared-resource identities, daisy-chain and STM32 pad maps, and explicit denied integration-evidence gates. | Replicate the approved cell across seven named conductors. | `BP-100`, `BP-101`, `BP-102` | Channel-by-channel schematic and net-map review; no generic repeated block hides pin swaps. |
...
| `BP-146` | blocked | Root-approved partial Vishay package drawing, pin/lead geometry, source digests, and optical-window formula evidence is ready to commit. Closure remains blocked by an accepted land pattern/CAD/drill, accepted board optical keepout, and physical range, angle, latency, flood, reset, and power-off evidence; commit is also blocked by the unrelated legislation package/lock mismatch. | Select and freeze the encrypted IR remote receiver path and optical test interface. | `BP-126`, `BP-142`, `BP-145` | Exact receiver/demodulator or decoder, wavelength/carrier, supply/filter/protection, connector or optical window assumptions, test points, footprint, current budget, numeric range/angle/light/latency targets, and stuck/noise/power-off behavior are reviewed. |
