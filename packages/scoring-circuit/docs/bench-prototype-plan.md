...
| `BP-125` | blocked | Root review selected Yageo `RC0603FR-0710KL` for both STM32 BOOT0 pulldown and ESP32 BOOT_N pullup, hash-bound the exact manufacturer product specification, and preserved package geometry as unclaimed. Twelve support references, processor schematic sign-off, CAD, artwork, and fabrication evidence remain unresolved. | Define oscillator, decoupling, boot straps, and unused-pin policy. | `BP-120`, `BP-121` | Data-sheet checklist and schematic sign-off for both processors, including GPIO35 `IR_RX`/`RMT_RX`, GPIO36/37 reserved NC, GPIO33/GPIO34 module-unexposed, and BP-121 `irReceiver` provenance. |
