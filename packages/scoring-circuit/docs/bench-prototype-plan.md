...
| `BP-125` | in-progress | Dependencies `BP-120` and `BP-121` are complete. Root-reviewed commit `482e0ed` resolves all eight formerly unselected capacitor roles to five exact Murata GCM identities, binds retained ESP32/Murata primary bytes and DC-bias responses, corrects official Murata provenance, and reconciles all 14 BP-125 support references into BP-032 while keeping footprint authority denied. Two focused suites pass 13 tests with package types, lint, format, nine evidence hashes, and diff checks clean. Next work is retaining the remaining primary-source bytes, project land-pattern/paste/mask/courtyard/orientation review, and independent STM32/ESP32 schematic sign-off. | Define oscillator, decoupling, boot straps, and unused-pin policy. | `BP-120`, `BP-121` | Data-sheet checklist and schematic sign-off for both processors, including GPIO35 `IR_RX`/`RMT_RX`, GPIO36/37 reserved NC, GPIO33/GPIO34 module-unexposed, and BP-121 `irReceiver` provenance. |
