...
| `BP-034` | blocked | Active implementation has delivered a prototype-only direct-wire A/B/C landing contract and root-requested validator revisions are complete, but this work unit remains blocked by incomplete dependencies `BP-104` and `BP-143`. The owner-validated OK Fencing weapon cable remains accepted. After those dependencies close, root review can complete the direct-wire contract and the received-sample, mounting, mate/unmate, continuity, retention, and strain evidence. | Complete pre-order connector sample, mate, fit, pinout, and continuity review. | `BP-050`, `BP-104`, `BP-124`, `BP-141`, `BP-143` | Received sample identities, mating-part checks, orientation/photos, mechanical fit, harness pinout, strain relief, and continuity archive. No automated scoring fixture is required yet. |
...
| `BP-123` | in-progress | Dependencies `BP-120` and `BP-121` are complete, so this task is not blocked. The exact supervisor/watchdog/reset contract and fail-closed six-class physical-evidence intake are implemented. The next handoff is schematic integration followed by real cold-start, brownout, watchdog, manual-reset, cross-domain, and power-off captures from an assembled prototype; authority remains denied until those task-level acceptance records exist. | Close both supervisor/watchdog/reset networks. | `BP-120`, `BP-121` | Exact MPN/value schematic and cold-start, brownout, watchdog, manual-reset, cross-domain, and power-off tests. |
