...
| `BP-031` | blocked | The unsupported `ADA4177-1BRZ` identity is corrected across the analog chain to manufacturer-listed `ADA4177-1ARZ`; its Rev. E primary source and R SOIC-8 package are identified but not yet hash-acquired. The 112 repeated-cell rows still share 13 hash-bound M4-04 sources through 91 links, 21 rows lack hash-bound source records, and all 113 records remain DNP-unresolved. Closure still requires source bytes, manufacturer CAD, artwork, orientation, schematic reconciliation, and BP-104 physical evidence. | Close analog, protection, reference, and weapon-fixture footprints. | `BP-030`, `BP-103`, `BP-104` | Independent evidence record for every populated lane-B reference; unresolved options remain explicit DNP. |
| `BP-032` | blocked | Root review corrected `NXE1S0505MC` to its source-backed 14-position/five-land package with four functional connections and reconciled the four exact `GCM188R71H104KA57D` STM32 VDD bypass selections from BP-125. Eight processor-support references remain explicitly unresolved, and all project-footprint, CAD, artwork, orientation, schematic, and fabrication gates stay denied. | Close processor, isolation, reset, clock, and debug footprints. | `BP-030`, `BP-122`, `BP-123`, `BP-124`, `BP-125` | Independent evidence record for every populated lane-C reference. |
| `BP-033` | blocked | Root review hash-bound seven retained manufacturer PDFs to eight exact BP-050/BP-142 references: the USB-C PD/protection path, both TPS259474 eFuse uses, source-selector diode, and application inductor. All remaining populated lane-D sources plus manufacturer review, CAD, artwork, orientation, layout, and fabrication evidence stay denied; BP-143/BP-146 physical gates also remain open. | Close power, Ethernet, application, HUB75, display, and encrypted-IR receiver footprints. | `BP-030`, `BP-050`, `BP-141`, `BP-142`, `BP-143`, `BP-144`, `BP-145`, `BP-146` | Independent evidence record for every populated lane-D reference, including the BP-146 receiver path; audio remains explicit DNP with no host or I2C stub. |
| `BP-034` | backlog | The owner-validated OK Fencing weapon cable is accepted as compatible and is not a prototype blocker. A fail-closed pair contract now requires independently accepted left and right A/B/C socket interfaces and permits either mechanically retained carrier-to-PCB mounting or separately mounted sockets wired to labeled PCB landing pads with strain relief. Owner photos are retained as hash-bound, non-dimensional references; production keeps a later replaceable keyed harness requirement. Closure still requires exact socket identity, received samples, per-side mounting/insulation, mate/unmate, continuity/isolation/negative-test, retention, and strain evidence after `BP-104` and `BP-143`. | Complete pre-order connector sample, mate, fit, pinout, and continuity review. | `BP-050`, `BP-104`, `BP-124`, `BP-141`, `BP-143` | Received sample identities, mating-part checks, orientation/photos, mechanical fit, harness pinout, strain relief, and continuity archive. No automated scoring fixture is required yet. |
| `BP-035` | blocked | Blocked: The fail-closed convergence evaluator is delivered, but its current result is `DENY` until BP-031 through BP-034 close all footprint, sample, and populated-TBD gaps. | Converge the lane BOMs into an order-candidate BOM. | `BP-031`, `BP-032`, `BP-033`, `BP-034` | Every proposed populated row is exact and footprint-approved; no proposed populated `TBD` remains; every omitted option is explicit DNP; lane netlists and reference sets are ready for schematic integration. |
...
| `BP-104` | blocked | Fixture pinout, NC positions, continuity thresholds, and the fail-closed physical-evidence intake are frozen. Official Molex drawing endpoints and exact material-table scopes are now recorded for `43045-1200`, `43025-1200`, `43030-0007`, and `44242-0005`, but retained bytes/hashes remain unavailable. Closure still requires retained drawing/CAD artifacts, received exact parts, sample fit and labels, seven crimp/retention records, strain relief, rejected-miswire captures, and calibrated continuity evidence. | Freeze fixture-harness pinout and NC positions. | `BP-010`, `BP-103` | Drawing, continuity map, mating parts, labels, strain relief, and miswire test. |
...
| `BP-123` | blocked | The exact supervisor/watchdog/reset contract now includes a fail-closed six-class physical-evidence intake with calibrated instrument, procedure, injected-input, immutable artifact, prototype-identity, signal, and typed metric validation. The canonical intake remains empty and every authority remains denied. Closure still requires real cold-start, brownout, watchdog, manual-reset, cross-domain, and power-off captures from one assembled prototype plus downstream schematic integration. | Close both supervisor/watchdog/reset networks. | `BP-120`, `BP-121` | Exact MPN/value schematic and cold-start, brownout, watchdog, manual-reset, cross-domain, and power-off tests. |
...
| `BP-125` | blocked | Root review selected Murata `GCM188R71H104KA57D` for the four STM32 VDD bypass references and hash-bound the exact manufacturer sheet, request context, and four raw temperature-characteristic responses without making a numeric DC-bias claim. This extends the earlier exact Yageo boot-resistor selections; remaining processor-support MPNs, processor schematic sign-off, CAD, artwork, and fabrication evidence stay unresolved. | Define oscillator, decoupling, boot straps, and unused-pin policy. | `BP-120`, `BP-121` | Data-sheet checklist and schematic sign-off for both processors, including GPIO35 `IR_RX`/`RMT_RX`, GPIO36/37 reserved NC, GPIO33/GPIO34 module-unexposed, and BP-121 `irReceiver` provenance. |
...
| `BP-143` | blocked | Exact Adafruit panel/signal/power-cable, Samtec header/footprint-print, and JST power-mate source bytes are retained and hash-bound; the reviewed ordered source set now fails closed on omission or identity drift. Closure still requires purchased-panel receipt, de-energized continuity, mating/orientation, current, cable-drop, connector-temperature, and physical-fit evidence. | Freeze HUB75 connector and panel power mating parts. | `BP-020` | Exact 16-pin signal and power connector MPNs, cable pinout, current rating, and purchased-panel continuity record. |
...
| `BP-146` | blocked | Root-reviewed Vishay source geometry now drives hash-bound 1:1 project-footprint and assembly overlays. The project candidate uses a 1.10 mm finished drill, 2.20 mm pad, 2.30 mm mask opening, 0.55 mm annular ring, and a verified 0.1898 mm worst-case diametral clearance over the 0.8602 mm maximum lead diagonal; manufacturer CAD, board-CAD consumption, and fabrication remain denied. Closure still requires the source-controlled PCB footprint review, accepted optical layout and measured panel coupon, plus handheld range, angle, latency, flood, reset, and power-off results. | Select and freeze the encrypted IR remote receiver path and optical test interface. | `BP-126`, `BP-142`, `BP-145` | Exact receiver/demodulator or decoder, wavelength/carrier, supply/filter/protection, connector or optical window assumptions, test points, footprint, current budget, numeric range/angle/light/latency targets, and stuck/noise/power-off behavior are reviewed. |
