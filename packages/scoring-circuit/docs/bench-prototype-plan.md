...
| `BP-031` | blocked | Root-reviewed commits `0679719` and `ae0a7b9` add exact review-only evidence for the seven KEMET `C0603C104K3RACTU` reference-bypass capacitors and all four selected Vishay CRCW resistor families (`R_ESD`, `R_SAR`, `R_SOURCE_PD`, and `R_FAULT_GUARD`). The retained exact-part/project identity records, Vishay series PDF, upstream ledgers, package-specific reflow geometry, non-polar orientation, rendered artwork hashes, and deny-by-default authority are fail-closed; integrated verification passes 35 shared-capacitor tests and 54 Vishay/dependent tests. Earlier reviewed commits cover the other populated analog/reference families including all seven exact `C0603C102J5GACTU` SAR filter capacitors. Every candidate remains CAD-unacquired where noted, orientation/process-pending, `accepted: false`, and fabrication-denied. BP-031 remains blocked only by incomplete dependency `BP-104`; remaining populated-footprint evidence, orientation, schematic reconciliation, and fabrication gates stay open. | Close analog, protection, reference, and weapon-fixture footprints. | `BP-030`, `BP-103`, `BP-104` | Independent evidence record for every populated lane-B reference; unresolved options remain explicit DNP. |
