...
| `BP-032` | blocked | Root-reviewed commits `8b3c33a`, `43418f2`, and `a437df5` add exact review-only candidates for `ESP32-S3-WROOM-1U-N16R2`, `TPS389033DSER`, `TPS3431SDRBR`, and the keyed, pin-7-omitted `FTSH-105-01-L-DV-007-K` STM32 SWD header with its exact FFSD mate. The committed Espressif, TI, and Samtec evidence bytes match their recorded hashes; root verification passes 9 ESP32/TI tests and 24 FTSH/BP-124 tests. Earlier reviewed commits cover the STM32, both digital isolators, isolated-power module, `SN74LVC2G07DCKR`, and both `BSS138AKA` transistors. Every candidate remains `accepted: false` and fabrication-denied. BP-032 remains blocked only by incomplete dependencies `BP-123` and `BP-125`; remaining lane-C footprints, manufacturer CAD where unavailable, independent orientation/process review, schematic sign-off, physical mating/continuity evidence, and fabrication approval remain open. | Close processor, isolation, reset, clock, and debug footprints. | `BP-030`, `BP-122`, `BP-123`, `BP-124`, `BP-125` | Independent evidence record for every populated lane-C reference. |
