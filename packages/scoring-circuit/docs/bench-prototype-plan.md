...
| `BP-146` | in-progress | Dependencies `BP-126`, `BP-142`, and `BP-145` are complete, so this task is not blocked. Root-reviewed correction `0963d35` replaces the invalid 6.95 mm board-depth interpretation with the Vishay drawing's 4.8 mm board-plane body projection and 1.2 mm back-edge-to-lead-row offset, adds separate rectangular/circular assembly obstructions, and emits an 8.38 mm by 5.90 mm courtyard covering body and copper with 0.55 mm clearance. A source-controlled deterministic generator reproduces both hash-bound overlays; source/blob/SVG hashes match. Twenty-four dependent tests, package types, lint, and format pass. The corrected project footprint remains review-only and `accepted: false`; next work is explicit manufacturer-CAD disposition and footprint release decision, board placement and optical layout, then measured range, angle, latency, flood, reset, and power-off evidence. Fabrication and optical authority remain denied. | Select and freeze the encrypted IR remote receiver path and optical test interface. | `BP-126`, `BP-142`, `BP-145` | Exact receiver/demodulator or decoder, wavelength/carrier, supply/filter/protection, connector or optical window assumptions, test points, footprint, current budget, numeric range/angle/light/latency targets, and stuck/noise/power-off behavior are reviewed. |
