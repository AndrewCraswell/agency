...
| `BP-031` | blocked | Root-reviewed commits through `c723c8c` plus `f4a6d5f` add review-only candidates for the populated analog/reference families including all seven exact `C0603C102J5GACTU` SAR filter capacitors. Root visual review verified KEMET kit pages 1-2 for exact 1000 pF, 5%, 50 V C0G identity and C1091 pages 9-10 for 0603 dimensions and density-B copper/courtyard guidance, while mask and paste remain explicit project inputs. The integrated 34-test KEMET/ledger/fabrication set plus package types, lint, format, source hashes, rendered geometry, and diff checks pass. Every candidate remains CAD-unacquired, orientation/process-pending, `accepted: false`, and fabrication-denied. BP-031 remains blocked only by incomplete dependency `BP-104`; remaining populated-footprint evidence, orientation, schematic reconciliation, and fabrication gates stay open. | Close analog, protection, reference, and weapon-fixture footprints. | `BP-030`, `BP-103`, `BP-104` | Independent evidence record for every populated lane-B reference; unresolved options remain explicit DNP. |
| `BP-032` | blocked | Root-reviewed commits `8b3c33a` and `43418f2` add exact review-only candidates for `ESP32-S3-WROOM-1U-N16R2`, `TPS389033DSER`, and `TPS3431SDRBR`, retaining and hash-binding Espressif PDF/DXF/STEP evidence and TI datasheet evidence. Root verification caught and corrected Git text normalization of the official DXF/STEP files; the committed bytes now match the recorded manufacturer hashes and all 9 focused tests pass. Package types, targeted lint/format, rendered geometry, and fail-closed identity/authority checks also pass. Earlier reviewed commits cover the STM32, both digital isolators, isolated-power module, `SN74LVC2G07DCKR`, and both `BSS138AKA` transistors. Every candidate remains `accepted: false` and fabrication-denied. BP-032 remains blocked only by incomplete dependencies `BP-123` and `BP-125`; the debug header and other remaining lane-C footprints, independent orientation/process review, schematic sign-off, and fabrication evidence remain open. | Close processor, isolation, reset, clock, and debug footprints. | `BP-030`, `BP-122`, `BP-123`, `BP-124`, `BP-125` | Independent evidence record for every populated lane-C reference. |
