...
| `BP-146` | blocked | Root-reviewed Vishay geometry now drives hash-bound overlays and a source-controlled tscircuit footprint. Commit `1b83ceb` binds the exact artifact path/exports, Git blob SHA-1, SHA-256, TSOP38438 identity, OUT/GND/VS order, and full rendered project geometry into canonical evidence so source or geometry drift fails closed; 30 focused tests plus package types, lint, and format pass. Manufacturer CAD, footprint acceptance, board placement, optical authority, and fabrication remain denied. Closure still requires accepted optical layout and a measured panel coupon, plus handheld range, angle, latency, flood, reset, and power-off results. | Select and freeze the encrypted IR remote receiver path and optical test interface. | `BP-126`, `BP-142`, `BP-145` | Exact receiver/demodulator or decoder, wavelength/carrier, supply/filter/protection, connector or optical window assumptions, test points, footprint, current budget, numeric range/angle/light/latency targets, and stuck/noise/power-off behavior are reviewed. |
