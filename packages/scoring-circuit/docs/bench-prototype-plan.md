...
**Status legend:** `backlog` dependencies complete but work not started; `in-progress` active work; `review` committed evidence awaiting explicit independent approval; `blocked` waiting on a named prerequisite; `done` implementation, verification, independent approval, and commit recorded.
...
| `BP-031` | in-progress | All 113 canonical references have root-reviewed evidence and zero remain not-started. Root approval now promotes three bounded mapping families covering 42 references: `C_SAR_1..7`, `C_REF_IN_1..7`, and the 28 Vishay CRCW resistor rows. Current lane state is 42 approved and 71 reviewed-unapproved. Exact manufacturer CAD, DC-bias performance, stress/placement, board fit, release, and fabrication remain denied. The remaining work is promotion review across the other 71 mapped candidates and closure of final BP-031 authority gates. | Close analog, protection, reference, and weapon-fixture footprints. | `BP-030`, `BP-103`, `BP-104` | Independent evidence record for every populated lane-B reference; unresolved options remain explicit DNP. |
...
| `BP-033` | in-progress | All dependencies are complete at their pre-order design boundaries. At `148fd7f`, the canonical display-limiter mapping is committed. Root review now reuses the retained BP-032 exact SN74LVC2G07DCKR evidence for `U_APP_RESET_FANOUT` instead of creating a duplicate BP-033 artifact. Current lane state is 89 reviewed-unapproved and 12 not-started references. Earlier root-reviewed mappings cover USB-C/PD/protection, branch fuses, TVS and source selection, both eFuse roles, eight W5500 bypass capacitors, four measurement links, 26 Yageo 10 kOhm rows, three Yageo 100 kOhm rows, two encrypted-IR resistors, five application-regulator rows, W5500, encrypted IR, HUB75, six shared KEMET 100 nF rows, three Keystone test points, five TDK 22 uF rows, two BSS138 display-enable MOSFETs, both display buffers, and the reset fanout. The remaining families are W5500 support, F-RAM, and connectors. Commodity-passive closure uses shared package-family evidence and a compact table-driven ledger; bespoke validators remain only for unusual packages and mechanical boundaries. Manufacturer CAD authority, placement, switching-loop/thermal review, schematic integration, release, and fabrication remain denied. | Close power, Ethernet, application, HUB75, display, and encrypted-IR receiver footprints. | `BP-030`, `BP-050`, `BP-141`, `BP-142`, `BP-143`, `BP-144`, `BP-145`, `BP-146` | Independent evidence record for every populated lane-D reference, including the BP-146 receiver path; audio remains explicit DNP with no host or I2C stub. |
...
| `BP-401` | blocked | Blocked on `BP-400`: outline, stack-up, placement, and mechanical review are incomplete. | Route analog, isolation, clocks, USB-C PD and USB2 service, UART recovery, Ethernet, HUB75, and high-current branches. | `BP-400` | Routed source with reviewed return paths, USB/Ethernet differential constraints, PD/power layout rules, isolation clearance, and no unrouted nets. |
...
| ID | Status | Latest state | Work unit | Dependencies | Acceptance evidence |
| --- | --- | --- | --- | --- | --- |
| `BP-500` | blocked | Blocked on `BP-300`: board-support configurations require the canonical schematic. | Build STM32 and ESP32 board-support configurations. | `BP-120`, `BP-121`, `BP-126`, `BP-300` | Pin ownership tests, encrypted IR receiver/decoder support, build artifacts, firmware digests, and hardware revision binding. |
| `BP-501` | blocked | Blocked on `BP-105`, `BP-302`, and `BP-500`: fixture, test-point plan, and firmware support are incomplete. | Implement fixture-safe manufacturing self-test. | `BP-105`, `BP-302`, `BP-500` | Test enumerates rails, reference, resets, isolation, seven lines, lamps, buzzer, encrypted IR receiver/decoder, Ethernet, and display; failures cannot be reported as passes. |
...
| `BP-509` | blocked | Blocked on `BP-505`, `BP-506`, and `BP-508`: processor and analog bring-up evidence is incomplete. | Execute all-channel rules and interaction corpus. | `BP-505`, `BP-506`, `BP-508` | Foil, epee, sabre, simultaneous-event, shorts, grounds, lockout, timing-boundary, replay, every encrypted IR short/held command and state guard, loaded/fresh bout state, duplicate/replay rejection, and failure-case results. |
