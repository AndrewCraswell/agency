...
| `BP-032` | in-progress | All dependencies are complete. Root-reviewed commits `0a6b4fe` and `b25d15f` retain and link the exact ST `STM32G474RET3TR` LQFP64 package, recommended-land, full BP-120 allocation, pin-one, and rendered project-footprint evidence to `U_SCORING`. Together with the earlier isolator, support-capacitor, reset, service-header, shared-capacitor, supervisor, and watchdog evidence, current lane state is 48 reviewed-unapproved and 3 not-started references. Remaining work is exact evidence and canonical-reference closure for the ESP32 module, isolated converter, and ESP32 service header. CAD, placement, schematic acceptance, process acceptance, release, and fabrication remain denied; processor schematic sign-off belongs to BP-300 and physical mating/continuity to BP-502. Candidate verification passes 3 files/14 tests and canonical integration passes 5 files/36 tests, with package types, lint, format, and hooks clean. | Close processor, isolation, reset, clock, and debug footprints. | `BP-030`, `BP-122`, `BP-123`, `BP-124`, `BP-125` | Independent evidence record for every populated lane-C reference. |
...
| `BP-035` | blocked | BP-031, BP-032, and BP-033 are incomplete dependencies; BP-034 is complete. After root-reviewed STM32 integration commit `b25d15f`, BP-031 is 113 reviewed-unapproved/0 not-started, BP-032 is 48/3, and BP-033 is 53/48, with zero approved lane rows. The current evaluator remains `DENY` with 342 blockers across 290 references: 22 unresolved MPN, 36 unresolved package, 34 unresolved population, 215 footprint-evidence, 7 missing-lane-reference, 8 missing-baseline-reference, 8 package-drift, and 12 population-drift; `selection-blocked` is zero. Reviewed-unapproved evidence intentionally does not grant footprint approval. | Converge the lane BOMs into an order-candidate BOM. | `BP-031`, `BP-032`, `BP-033`, `BP-034` | Every proposed populated row is exact and footprint-approved; no proposed populated `TBD` remains; every omitted option is explicit DNP; lane netlists and reference sets are ready for schematic integration. |
