...
| `BP-031` | blocked | Root-reviewed commits `74d10b4`, `c4efd08`, `6dd2d41`, `da0f9a1`, and `a6dd41e` add review-only candidates for the exact TDK bypass capacitors, ADA4177 buffers, KEMET T521 reservoirs, TMUX1112 switches, and all seven `TPD4E05U06DQAR` ESD shunts. Root visual review corrected the TPD4 citation from the unrelated TPD1 page to page 4, corrected pin one from the submitted lower-left claim to TI's top-left datum, and rebound exact orderable evidence to pages 20 and 37 before acceptance. The integrated 25-test TPD/ledger/fabrication set plus package types, lint, format, source hashes, rendered geometry, and diff checks pass. Every candidate remains CAD-unacquired, orientation/process-pending, `accepted: false`, and fabrication-denied. BP-031 remains blocked only by incomplete dependency `BP-104`; remaining populated-footprint evidence, orientation, schematic reconciliation, and fabrication gates stay open. | Close analog, protection, reference, and weapon-fixture footprints. | `BP-030`, `BP-103`, `BP-104` | Independent evidence record for every populated lane-B reference; unresolved options remain explicit DNP. |
| `BP-032` | blocked | Root-reviewed commits `482e0ed` and `03d047c` reconcile all 14 BP-125 processor-support references to exact selections and add the exact `STM32G474RET3TR` LQFP64 project-footprint candidate. Root visually verified ST DS12288 Rev 6 Figures 62-64 and Table 124: 64-pad LQFP identity, 0.5 mm pitch, 0.3 mm by 1.2 mm recommended copper, 12.7 mm outer span, counter-clockwise top-view numbering, pin-one datum, temperature grade, and tape-and-reel order code. Commit `584e465` removes an obsolete evidence-directory exclusivity assertion while retaining exact hash checks for the bounded reset-source ledger. The integrated 58-test footprint/ledger/fabrication set, package types, lint, format, retained-source hash, and diff checks pass. BP-032 remains blocked by incomplete dependencies `BP-123` and `BP-125`; manufacturer CAD, independent orientation/process review, remaining lane-C footprints, schematic sign-off, and fabrication evidence stay denied. | Close processor, isolation, reset, clock, and debug footprints. | `BP-030`, `BP-122`, `BP-123`, `BP-124`, `BP-125` | Independent evidence record for every populated lane-C reference. |
