...
| `BP-031` | in-progress | All dependencies are complete. Root-reviewed commit `4208421` maps the exact TI `TPD4E05U06DQAR` review candidate to all seven `U_ESD_1..7` references, hash-binds the retained Rev. O source and rendered artwork, records the no-CAD disposition, and preserves every geometry/orientation/release/fabrication denial. Root review accepts the identity/source/reference mapping only; the artifact does not receive footprint approval. The changed closure digest is reconciled through every retained BP-031 dependent source, validator, test, and document. Eight affected files/72 tests, package types, lint, format, hooks, and diff checks pass; a full 832-test package run had 830 passes with only the pre-existing BP-125 hash mismatch and BP-123 timeout, neither in this slice. Earlier reviewed commits cover KEMET, Vishay, Murata, Molex, negative-rail, and 3.3 V regulator evidence. Remaining work is lane-B footprint/orientation acceptance, canonical reference reconciliation, DC-bias/placement review, and explicit DNP closure; physical fixture construction and continuity are owned by BP-105/BP-502. | Close analog, protection, reference, and weapon-fixture footprints. | `BP-030`, `BP-103`, `BP-104` | Independent evidence record for every populated lane-B reference; unresolved options remain explicit DNP. |
| `BP-032` | in-progress | All dependencies are complete. Root-reviewed commit `0799050` adds exact Yageo `RC0603FR-0710KL` and `RC0603FR-07100KL` source/candidate evidence and reconciles all 16 reset, watchdog, boot, and support references into the lane-C ledger. Root visually verified both retained one-page official sheets and accepts their orderable identities, hashes, package envelopes, reference mapping, and deny-state integrity; because the sheets publish no land pattern, the project copper/mask/paste/courtyard and assembly-stress assumptions remain explicitly unapproved. Four focused/dependent files/37 tests, package types, lint, format, hooks, and diff checks pass. Earlier commits cover the processors, isolators, isolated power, reset devices/fanout, and keyed debug header. Next work is the remaining exact `C1608X5R1A105K080AC` support-capacitor candidate plus independent geometry/orientation/process and canonical-reference closure. Processor schematic sign-off remains BP-300; physical mating/continuity remains BP-502. | Close processor, isolation, reset, clock, and debug footprints. | `BP-030`, `BP-122`, `BP-123`, `BP-124`, `BP-125` | Independent evidence record for every populated lane-C reference. |
