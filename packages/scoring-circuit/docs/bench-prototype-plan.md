...
| `BP-031` | blocked | Root-reviewed commit `0679719` adds exact review-only footprint evidence for all seven `C_REF_REG_HF_1..7` instances of KEMET `C0603C104K3RACTU`, shared with BP-032. The retained exact-part PDF, canonical source/reference hashes, non-polar orientation, project-only copper/mask/paste/courtyard geometry, and deny-by-default authority are fail-closed; the integrated focused/dependent set passes 35 tests. Earlier reviewed commits cover the other populated analog/reference families including all seven exact `C0603C102J5GACTU` SAR filter capacitors. Every candidate remains CAD-unacquired where noted, orientation/process-pending, `accepted: false`, and fabrication-denied. BP-031 remains blocked only by incomplete dependency `BP-104`; remaining populated-footprint evidence, orientation, schematic reconciliation, and fabrication gates stay open. | Close analog, protection, reference, and weapon-fixture footprints. | `BP-030`, `BP-103`, `BP-104` | Independent evidence record for every populated lane-B reference; unresolved options remain explicit DNP. |
