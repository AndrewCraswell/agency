...
| `BP-031` | blocked | Active evidence work is resolving the exact Panasonic `ERA3AEB2491V` source and adding its manufacturer-derived project land pattern in separate isolated units. The unsupported `ADA4177-1BRZ` identity is already corrected across the analog chain to manufacturer-listed `ADA4177-1ARZ`. The current ledger remains fabrication-denied: unresolved source records, manufacturer CAD, accepted project footprints, artwork, orientation, schematic reconciliation, and BP-104 physical evidence still prevent closure. | Close analog, protection, reference, and weapon-fixture footprints. | `BP-030`, `BP-103`, `BP-104` | Independent evidence record for every populated lane-B reference; unresolved options remain explicit DNP. |
...
| `BP-034` | in-progress | A prototype-only direct-wire A/B/C landing contract for both sides has been delivered in an isolated worktree and is undergoing root revision. It specifies exact nets, pads, test points, labels, NC exclusions, de-energized rework, clamp-only strain loading, continuity/isolation thresholds, and miswire rejection while keeping USB-C PD unchanged. Root review requires the validator to fail closed on every authority-bearing contract field before approval. The owner-validated OK Fencing weapon cable remains accepted; production socket selection and all received-sample, mounting, mate/unmate, continuity, retention, and strain evidence remain open. | Complete pre-order connector sample, mate, fit, pinout, and continuity review. | `BP-050`, `BP-104`, `BP-124`, `BP-141`, `BP-143` | Received sample identities, mating-part checks, orientation/photos, mechanical fit, harness pinout, strain relief, and continuity archive. No automated scoring fixture is required yet. |
...
| `BP-125` | blocked | Root review selected Murata `GCM188R71H104KA57D` for the four STM32 VDD bypass references and hash-bound its manufacturer evidence. A read-only audit has now confirmed the exact value, tolerance, voltage, dielectric, temperature range, and package/body geometry for the five proposed Murata capacitor identities and found only series-level manufacturer land guidance, not exact-MPN manufacturer CAD. The delivered selection unit still awaits root diff review; one retained `GCM21BR71E225KA73L` archive URL must be rebound to the official Murata source. Partner CAD remains candidate-only. Processor schematic sign-off, project paste/mask/courtyard/orientation/placement review, and fabrication evidence still block closure. | Define oscillator, decoupling, boot straps, and unused-pin policy. | `BP-120`, `BP-121` | Data-sheet checklist and schematic sign-off for both processors, including GPIO35 `IR_RX`/`RMT_RX`, GPIO36/37 reserved NC, GPIO33/GPIO34 module-unexposed, and BP-121 `irReceiver` provenance. |
