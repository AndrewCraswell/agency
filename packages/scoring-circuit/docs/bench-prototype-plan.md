...
| `BP-031` | blocked | Root-reviewed commits `0679719`, `ae0a7b9`, and `8e6d06e` add exact review-only evidence for the seven KEMET `C0603C104K3RACTU` bypass capacitors, all four selected Vishay CRCW resistor families, and all seven Murata `GRM21BR71A106KE51L` reference reservoirs. The retained exact-part/project identities, manufacturer sources, upstream ledgers, package-specific reflow geometry, non-polar orientation, rendered artwork hashes, and deny-by-default authority are fail-closed; integrated verification passes 35 shared-capacitor, 54 Vishay/dependent, and 43 Murata/dependent tests. Earlier reviewed commits cover the other populated analog/reference families including all seven exact `C0603C102J5GACTU` SAR filter capacitors. Every candidate remains CAD-unacquired where noted, orientation/process-pending, `accepted: false`, and fabrication-denied. BP-031 remains blocked only by incomplete dependency `BP-104`; remaining populated-footprint evidence, orientation, schematic reconciliation, DC-bias/placement review, and fabrication gates stay open. | Close analog, protection, reference, and weapon-fixture footprints. | `BP-030`, `BP-103`, `BP-104` | Independent evidence record for every populated lane-B reference; unresolved options remain explicit DNP. |
| `BP-032` | blocked | Root-reviewed commit `0679719` adds exact review-only evidence for the eight BP-123 reset/supervisor/watchdog `C0603C104K3RACTU` references, sharing the same hash-bound exact-part and project-geometry record with BP-031; the integrated focused/dependent set passes 35 tests. Commits `8b3c33a`, `43418f2`, and `a437df5` cover the ESP32 module, both TI supervisor/watchdog packages, and keyed STM32 SWD header, while earlier reviewed commits cover the STM32, isolators, isolated power, reset fanout, and reset transistors. Every candidate remains `accepted: false` and fabrication-denied. BP-032 remains blocked only by incomplete dependencies `BP-123` and `BP-125`; remaining lane-C footprints, manufacturer CAD where unavailable, independent orientation/process review, schematic sign-off, physical mating/continuity evidence, and fabrication approval remain open. | Close processor, isolation, reset, clock, and debug footprints. | `BP-030`, `BP-122`, `BP-123`, `BP-124`, `BP-125` | Independent evidence record for every populated lane-C reference. |
| `BP-033` | blocked | Root-reviewed commits `59ea3bc` and `75e9d28` add the exact Samtec `TST-108-04-G-D-RA` HUB75 pin-map/orientation overlay and a review-only WIZnet `W5500` post-July-2021 LQFP-48 project-footprint candidate. Root visually verified WIZnet Figures 26/27: 7 mm body, 9 mm lead-tip span, 0.5 mm pitch, 48-lead counter-clockwise top-view sequence, pin-one corner marker, and no exposed thermal pad. The strengthened test hash-checks the retained official PDF and verifies all 48 rendered copper and paste apertures, mask, courtyard, pin sequence, and geometry digest. Twenty dependent tests, package types, lint, format, source hashes, and diff checks pass. Both artifacts remain project review inputs with CAD, orientation, board-fit, and fabrication authority denied. BP-033 remains blocked by incomplete dependencies `BP-143` and `BP-146`, plus remaining lane-D footprint and physical-evidence gates. | Close power, Ethernet, application, HUB75, display, and encrypted-IR receiver footprints. | `BP-030`, `BP-050`, `BP-141`, `BP-142`, `BP-143`, `BP-144`, `BP-145`, `BP-146` | Independent evidence record for every populated lane-D reference, including the BP-146 receiver path; audio remains explicit DNP with no host or I2C stub. |
...
| `BP-035` | blocked | Root reran the current evaluator after BP-031/BP-033 evidence integration; it remains `DENY` with 650 blockers: 22 unresolved MPN, 36 unresolved package, 34 unresolved population, 32 sample-evidence, 265 footprint-evidence, 232 missing-baseline-reference, 2 duplicate-reference, 9 missing-lane-reference, 6 package-drift, and 12 population-drift. `selection-blocked` remains zero. The 16-blocker net reduction reflects eight newly exact identities moving from selection gaps into footprint review. Dependencies `BP-031`, `BP-032`, `BP-033`, and `BP-034` remain incomplete and own these closures; rerun after every lane change. | Converge the lane BOMs into an order-candidate BOM. | `BP-031`, `BP-032`, `BP-033`, `BP-034` | Every proposed populated row is exact and footprint-approved; no proposed populated `TBD` remains; every omitted option is explicit DNP; lane netlists and reference sets are ready for schematic integration. |
...
| `BP-104` | in-progress | Dependencies `BP-010` and `BP-103` are complete, so this task is not blocked. Root-reviewed commits `5339ec7` and `d195d47` retain official Molex evidence and correct the physical-evidence contract. Commits `81beeed` and `97915a5` add a strict received-part and physical-evidence intake, require `root-final-reviewer` across every section and artifact, and prove the complete acceptance path including the explicit null-CAD disposition for loose terminal `43030-0007`. Root verification passed 25 intake/harness tests, package types, lint, format, and diff checks. Fixture pinout, source bindings, continuity thresholds, and intake workflow are frozen; remaining work is real received-part fit, labels, seven crimp/retention records, strain relief, rejected miswires, and calibrated continuity. | Freeze fixture-harness pinout and NC positions. | `BP-010`, `BP-103` | Drawing, continuity map, mating parts, labels, strain relief, and miswire test. |
...
| `BP-123` | in-progress | Dependencies `BP-120` and `BP-121` are complete. Root-reviewed commits `3ad25e2` and `4b7e4b3` provide the deterministic reset/watchdog schematic review fragment and a fail-closed physical-capture intake for all six required cold-start, brownout, watchdog, manual-reset, cross-domain, and power-off classes. The intake binds one prototype identity, canonical metrics and limits, unique hashed waveforms, calibrated instruments, real UTC timestamps, procedures, setups, injected profiles, and an independent reviewer artifact; root review added normalized-invalid-date rejection plus operator/reviewer and artifact-collision guards. Fourteen focused reset/intake tests, package types, lint, format, and diff checks pass. Physical acceptance remains impossible in code and all authority stays denied. Next work is canonical BP-300 schematic/PDF/ERC integration and real assembled-prototype captures. | Close both supervisor/watchdog/reset networks. | `BP-120`, `BP-121` | Exact MPN/value schematic and cold-start, brownout, watchdog, manual-reset, cross-domain, and power-off tests. |
...
| `BP-125` | in-progress | Dependencies `BP-120` and `BP-121` are complete. Root-reviewed commits `6a028d7` and `b180c91` add three source-bound, review-only Murata footprint candidates for all five selected GCM capacitor MPNs and retain the exact ST DS12288 Rev. 6 manufacturer PDF for `STM32G474RET3TR`. The Murata review verifies actual rendered copper, mask, paste, and courtyard geometry; the ST review visually verifies the LQFP64 pinout, rail and decoupling scheme, clock limits, reset state, unused-input guidance, and ordering-code identity, and corrects the unused-input citation from page 107 to page 108. Twenty-one focused/dependent tests, package types, lint, format, retained evidence hashes, the exact upstream selection hash, and diff checks pass. Exact-MPN capacitor CAD, processor footprints, clock-selection evidence, orientation/stress review, and independent STM32/ESP32 schematic sign-off remain open; all fabrication authority stays denied. | Define oscillator, decoupling, boot straps, and unused-pin policy. | `BP-120`, `BP-121` | Data-sheet checklist and schematic sign-off for both processors, including GPIO35 `IR_RX`/`RMT_RX`, GPIO36/37 reserved NC, GPIO33/GPIO34 module-unexposed, and BP-121 `irReceiver` provenance. |
...
| `BP-143` | in-progress | Dependency `BP-020` is complete. Root-reviewed commit `25754b3` adds an empty-by-default, deeply immutable physical-evidence intake binding exact panel/cable/prototype identities, real canonical timestamps, calibrated continuity/current/voltage/thermal instruments, unique artifact hashes, all 16 signal pins, both power branches, mating/orientation, current, cable drop, connector temperature, and fit. Synthetic adversarial tests keep all authority denied; 2 focused tests plus package types, lint, format, and diff checks pass. Next work requires the actual purchased panel/cables and measured submission. | Freeze HUB75 connector and panel power mating parts. | `BP-020` | Exact 16-pin signal and power connector MPNs, cable pinout, current rating, and purchased-panel continuity record. |
| `BP-144` | blocked | The reset-safe two-buffer HUB75 contract is implemented, but incomplete dependencies `BP-123` and `BP-143` still prevent closure. The remaining handoff is integrated reset/watchdog schematic evidence plus purchased-panel continuity and power evidence; schematic, power-off, layout, bench, and fabrication authority remain denied. | Implement the two-buffer reset-safe HUB75 path. | `BP-121`, `BP-123`, `BP-143` | Schematic truth table proves black/high-impedance defaults; all 13 signals and pulls are present. |
...
| `BP-146` | in-progress | Dependencies `BP-126`, `BP-142`, and `BP-145` are complete, so this task is not blocked. Root-reviewed commit `5232ad2` hash-binds a retained Vishay product-page snapshot proving the official ECAD path leaves Vishay for Ultra Librarian; no external model was downloaded or credited as manufacturer CAD. The corrected drawing-derived footprint is explicitly `candidate-unapproved`, `accepted: false`, unreleased, and fabrication-denied. Root independently confirmed the official ECAD notice and passed 25 dependent tests, package types, lint, format, snapshot hash `BEAE68A5...169F6`, and diff checks. Remaining root-release inputs are 1:1 board-CAD orientation/courtyard review, fabricator capability and DRC, final board outputs, panel/coupon evidence, and immutable range, angle, latency, flood, reset, and power-off measurements. | Select and freeze the encrypted IR remote receiver path and optical test interface. | `BP-126`, `BP-142`, `BP-145` | Exact receiver/demodulator or decoder, wavelength/carrier, supply/filter/protection, connector or optical window assumptions, test points, footprint, current budget, numeric range/angle/light/latency targets, and stuck/noise/power-off behavior are reviewed. |
