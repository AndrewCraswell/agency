...
| `BP-033` | in-progress | All dependencies are complete at their pre-order design boundaries. At `148fd7f`, the canonical display-limiter mapping is committed. Current lane state is 88 reviewed-unapproved and 13 not-started references. Earlier root-reviewed mappings cover USB-C/PD/protection, branch fuses, TVS and source selection, both eFuse roles, eight W5500 bypass capacitors, four measurement links, 26 Yageo 10 kOhm rows, three Yageo 100 kOhm rows, two encrypted-IR resistors, five application-regulator rows, W5500, encrypted IR, HUB75, six shared KEMET 100 nF rows, three Keystone test points, five TDK 22 uF rows, two BSS138 display-enable MOSFETs, and both display buffers. The remaining families are W5500 support, reset, F-RAM, and connectors. Commodity-passive closure will now use shared package-family evidence and a compact table-driven ledger; bespoke validators remain only for unusual packages and mechanical boundaries. Manufacturer CAD authority, placement, switching-loop/thermal review, schematic integration, release, and fabrication remain denied. Canonical verification passes 3 files/46 tests with package types, lint, and format clean. | Close power, Ethernet, application, HUB75, display, and encrypted-IR receiver footprints. | `BP-030`, `BP-050`, `BP-141`, `BP-142`, `BP-143`, `BP-144`, `BP-145`, `BP-146` | Independent evidence record for every populated lane-D reference, including the BP-146 receiver path; audio remains explicit DNP with no host or I2C stub. |
