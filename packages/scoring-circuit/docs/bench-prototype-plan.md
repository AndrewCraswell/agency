...
| `BP-125` | in-progress | Dependencies `BP-120` and `BP-121` are complete. Root-reviewed commit `6a028d7` adds three source-bound, review-only Murata footprint candidates for all five selected GCM capacitor MPNs. The review corrected the land-guide pad axes, then verified actual rendered copper extents, inner gaps, spans, mask/paste apertures, and copper/body courtyard clearances for 0603, 0805, and 1210. Fourteen focused/dependent tests, package types, lint, format, retained evidence hashes, the exact upstream selection hash, and diff checks pass. Exact-MPN CAD remains unacquired, orientation and stress review remain pending, and every candidate stays `accepted: false` and fabrication-denied. Next work is retaining the STM32 primary source, remaining processor-support footprints, and independent STM32/ESP32 schematic sign-off. | Define oscillator, decoupling, boot straps, and unused-pin policy. | `BP-120`, `BP-121` | Data-sheet checklist and schematic sign-off for both processors, including GPIO35 `IR_RX`/`RMT_RX`, GPIO36/37 reserved NC, GPIO33/GPIO34 module-unexposed, and BP-121 `irReceiver` provenance. |
