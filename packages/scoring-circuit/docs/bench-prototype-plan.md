...
| `BP-031` | in-progress | All dependencies are complete. Root-reviewed commits `0ae5999` and `389b566` retain the exact KEMET `C0603C102J5GACTU` identity and applicable KEMET 0603 family land guidance, distinguish the 0.70 mm terminal separation from the 0.80 mm project pad gap, bind rendered artwork and non-polar orientation, and map all seven `C_SAR_1..7` references while keeping exact-orderable CAD, board fit, release, and fabrication denied. Current lane state is 70 reviewed-unapproved and 43 not-started references; the candidate, closure, and convergence set passes 3 files/56 tests with package types, lint, and format clean. Root-reviewed commits `38e654a` and `0a11e9e` retain the exact TI `REF5025AQDRQ1` datasheet and D0008A SOIC-8 drawing, bind the rendered artwork and pin-one orientation, harden the candidate against hidden/getter/symbol drift, and map all seven `U_REF_1..7` references while keeping CAD, board fit, release, and fabrication denied. Root-reviewed commit `bd3c29c` retains official Vishay document 20043 for the exact CRCW-HP series, accepts the 0603/1206 HP body, terminal, recommended-reflow, and non-polar orientation evidence, and maps all 28 replicated `R_ESD`, `R_SOURCE_PD`, `R_SAR`, and `R_FAULT_GUARD` references while keeping exact-orderable CAD, board fit, release, and fabrication denied. Root-reviewed commit `f331abb` maps the exact ADI `ADA4177-1ARZ` R-8 evidence to all seven `U_OVP_BUFFER_1..7` references. Root visually verified the Rev. E ordering guide, independent R-8 outline, lower-left pin-one datum, seven-reference mapping, and deny-state integrity. ADI `90-0096` remains an S8 family land-pattern input rather than exact ADA4177 geometry, so project geometry, CAD, board fit, release, and fabrication remain denied. Earlier root-reviewed commits `90c69e3`, `1ba9dba`, and `4208421` cover all seven ADS8881 ADCs, TMUX1112 switches, and TPD4 ESD devices; earlier evidence covers Vishay, Murata, Molex, negative-rail, and 3.3 V regulator inputs. Remaining work is the rest of lane-B footprint/orientation acceptance, including the seven-reference reference-input capacitor family, source resistors, canonical-reference reconciliation, DC-bias/placement review, and explicit DNP closure; physical fixture construction and continuity are owned by BP-105/BP-502. | Close analog, protection, reference, and weapon-fixture footprints. | `BP-030`, `BP-103`, `BP-104` | Independent evidence record for every populated lane-B reference; unresolved options remain explicit DNP. |
