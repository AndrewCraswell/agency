...
The board-plane datum is explicit: the front lens datum is `(2.5, 0)` mm and
the optical axis points toward negative `y`. In drawing `6.550-5263.01-4`, the
front plan projection is a radius-2.0 mm circle centred at `(2.5, 2.0)` mm;
the 5.0 mm-wide rear body block spans `y=2.0` through the back edge at
`y=4.8`. The lead row is `y=3.6` mm, derived from the nominal 1.2 mm
body-back-edge dimension, not from the 6.95 mm package height. That height is
vertical and is deliberately excluded from board-plane depth.

The tscircuit artifact emits separate top-layer rectangular body and
circular-front assembly-obstruction keepouts. Its explicit `8.38 mm` by
`5.90 mm` project courtyard is the 0.55 mm clearance envelope around both the
nominal body projection and the 2.20 mm copper-pad extents: `x=-1.65` through
`6.73` mm and `y=-0.55` through `5.35` mm. These are review inputs, not a
Vishay land pattern or fabrication approval. The blue body/lens outline in
both overlays remains a deterministic projection of the retained manufacturer
drawing dimensions, not manufacturer CAD.

The retained project artwork is generated by the deterministic SVG overlay
generator version `2.1.0` in
[`bp146-overlay-generator.ts`](../src/bp146-overlay-generator.ts). Running
`pnpm --filter @repo/scoring-circuit exec tsx src/bp146-overlay-generator.ts`
regenerates the two hash-bound 1:1 artifacts:
...
`07273F80E0F622108F272238C6C0A39B54658B98` and SHA-256
`56925A88005421305B161D56537235FC3BE4D8B79EE66767CFFC9803FC75295C`; a source
