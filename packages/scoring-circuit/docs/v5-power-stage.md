...
The selected replacement is TI `TPS56A37RPAR`, a fixed-frequency 500 kHz synchronous buck with integrated 19.4 mOhm high-side and 8.5 mOhm low-side MOSFETs. It is specified for 4.5 V to 28 V input and 10 A continuous output. No external power MOSFETs are used. The topology exactly follows the component values of TI's 5 V, 10 A EVM application, except that V5 is Kelvin-sensed after the telemetry shunt. This is a selected design direction, not a transferable production layout.
...
| Output MLCC bank | 2 x Murata `GRM32ER71E226KE15L` | 22 uF, 25 V, X7R, 1210, after the V5 shunt. The EVM identifies 35 uF effective total at 5 V; release must prove at least that effective value over tolerance and temperature. |
...
| V5 current telemetry | Bourns `CRE2512-FZ-R002E-3` | 2 mOhm, 1 percent, 3 W current shunt with Kelvin connections to `INA238AIDGSR`. It is a selected 2512 part with land-pattern/CAD, Kelvin escape, temperature-coefficient, and calibration gates recorded in `src/part-readiness.ts`. At 9.01 A it dissipates 0.162 W nominal. |
...
The V5 telemetry shunt is full rail and sits after the buck. Its I-squared-R loss must therefore be supplied by the buck and eFuse, rather than hidden in the downstream allocation: 0.128 W at 39.95 W post-shunt continuous load and 0.162 W at 45.05 W post-shunt peak load. At the rail budget's 85 percent buck-efficiency floor, the continuous 39.95 W post-shunt load is 40.08 W before the shunt and requires 2.36 A at 20 V. The 45.05 W, 100 ms post-shunt peak is 45.21 W before the shunt and requires 2.66 A, exceeding the 2.40 A worst-low limit by about 0.26 A. This is long relative to the eFuse's 2 ms blanking interval, so it cannot be described as protected peak operation.

The exact eFuse limit is **40.73 W before the shunt**, which becomes **40.60 W**, or **8.12 A**, available to post-shunt V5 loads. After the current peak fixed loads of 10.26 W, no more than **30.34 W** remains for display during a guaranteed peak. The existing 34.79 W display peak allocation is therefore denied.

Do not silently change the USB-PD contract to resolve this. Bounded resolution options, none selected here, are:

1. Reduce the panel/display short peak so total post-shunt V5 load does not exceed 40.60 W under this pessimistic screen.
...
The rail-budget 85 percent value is an allocation, not measured TPS56A37 efficiency. It allows 7.05 W continuous and 7.95 W during the short screen across the complete buck stage. It does not say how much is in the IC, inductor, shunt, capacitors, copper, or connector. The TPS56A37 4-layer EVM effective junction-to-ambient figure is 30 C/W. At 50 C ambient, attributing even the full continuous allowance to the IC would imply a 211 C rise, which is not a valid thermal model. Fabrication release needs an actual loss split, copper/thermal-via layout, and measurements rather than a thermal calculation based only on the power envelope.

Required evidence before lifting DENY:

- PCB PI layout review against the TPS56A37 EVM/layout guidance: VIN capacitor loop and PGND, SW copper keepout, BOOT loop, AGND/PGND single point, separate post-shunt feedback sense, Kelvin shunt routes, and output return to the HUB75 connector. The circuit-model coordinates are nonphysical placeholders, not a placement or clearance proof; the former `L_V5_BUCK` / `L_APP_REGULATOR` coordinate collision is prevented by a circuit assertion only.
- Scope VIN at the regulator, SW, V5 at the capacitors and panel end, EN, PG, eFuse output, and V5 current for attach, detach, PD hard reset, eFuse retry, 0 A to maximum load steps, maximum audio, Wi-Fi/Ethernet traffic, and full-white panel content.
...
- Perform thermal-camera and thermocouple tests at 50 C ambient with blocked vents. Record IC junction proxy/case, inductor, shunt, input/output MLCCs, eFuse, USB-C connector, copper, and panel-end cable temperatures at continuous load and the repeated transient profile.
...
- [Bourns CRE2512 data sheet](https://www.bourns.com/docs/product-datasheets/cre.pdf): 2 mOhm order code, 1 percent, and 3 W option.
