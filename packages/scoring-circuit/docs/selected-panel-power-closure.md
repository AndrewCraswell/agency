...
The eFuse-bounded V5 ceiling is **40.73 W**. It is derived from
the worst-low approximately 2.40 A eFuse current limit, the 20 V input, the
0.85 V5 conversion floor. The calculation does not treat the 2.99 A
worst-high eFuse limit as usable capacity. P0 omits permanent V5 telemetry and
uses the removable current links with external instruments.
...
| Total V5 load | **26.96 W** | **30.46 W** |
| V5 load current | 5.39 A | 6.09 A |
| V5 buck conversion loss at 0.85 floor | 4.76 W | 5.38 W |
| USB-PD/eFuse path-loss allowance | 1.00 W | 1.00 W |
| USB-PD source demand | **32.72 W / 1.64 A** | **36.84 W / 1.84 A** |
| Worst-low eFuse current limit | 2.40 A | 2.40 A |
| eFuse current headroom | 0.76 A | 0.55 A |
| Guaranteed V5 ceiling | 40.73 W | 40.73 W |
| V5 power headroom | 13.77 W | 10.27 W |
| Arithmetic selected-configuration fit | **PASS** | **PASS** |

The selected panel therefore fits the current guaranteed V5 ceiling in
...
allowance; the buck loss is shown separately so that no loss is silently
hidden in the panel allocation.
...
but the eFuse-bounded maximum display allocation is only **30.47 W** after the
...
4. Confirm the repeated blocked-vent 50 C thermal test for the TPS56A37,
   inductor, eFuse, USB-C connector, application regulator, panel
