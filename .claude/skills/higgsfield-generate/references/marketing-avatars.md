...
`type` is `preset` for curated, `custom` for user-created. `--avatars` expects a JSON array, so pass it via
`@/path/to/file.json`.

For UGC modes, an avatar is optional if the brief clearly mentions a person and no specific presenter was requested; the
backend can synthesize a Soul Character automatically.
