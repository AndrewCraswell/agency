...
Show every Brandkit review directly in chat when the client can render local images. Always provide the editable local
file path as well.
...
![<name> board](/absolute/path/to/board.png) [Open editable HTML](/absolute/path/to/board.html)
```

For 2–3 options, stack the complete blocks in one response. Never send filenames without enough context to identify the
option. Ask for ordinary chat feedback and stop.
...
![Logo candidate 1](<Recraft SVG URL>) [Download SVG](<Recraft SVG URL>)
```

If the client cannot render the SVG URL, download the exact SVG locally and rasterize a review-only PNG with
`rsvg-convert`. Keep the original SVG untouched as the downloadable asset.
