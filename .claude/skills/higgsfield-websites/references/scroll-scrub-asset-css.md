...
Read `references/scroll-scrub.md` first. Copy the fenced source into `app/src/components/scroll-scrub/scroll-scrub.css`
and replace composition values through the design brief.
...
@media (max-width: 860px) {
  ...
  .scroll-scrub__chapter[data-align="right"] .scroll-scrub__chapter-pin {
    ...
    justify-content: flex-start;
    padding: 6.5rem max(1.25rem, env(safe-area-inset-right)) calc(4rem + env(safe-area-inset-bottom))
