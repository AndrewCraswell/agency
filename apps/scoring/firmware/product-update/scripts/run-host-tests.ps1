...
if ([string]::IsNullOrWhiteSpace($BuildDirectory)) {
  $suffix = if ($Sanitizers) { "-$($Configuration.ToLowerInvariant())-sanitizers" } else { "-$($Configuration.ToLowerInvariant())" }
  $BuildDirectory = Join-Path $sourceDirectory "out\host-clang$suffix"
}
...
if (-not (Test-Path -LiteralPath $ninja) -or -not (Test-Path -LiteralPath $compiler)) {
  throw "The product release host test requires Ninja and the pinned x86_64 MinGW C compiler."
}
...
if ($Sanitizers) {
  $configureArguments += "-DSCORING_PRODUCT_UPDATE_SANITIZERS=ON"
}
