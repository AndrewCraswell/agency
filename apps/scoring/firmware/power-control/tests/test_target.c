...
static void reset(void) {
    ...
    assert(power_register_read(0xe000e014u)==5999u);
    assert(power_register_read(0xe000e010u)==7u); /* Clock, interrupt and counter enabled. */
}
int main(void) {
    ...
    source(&pd,false,2);assert(!power_target_poll());assert((gpio&0x830u)==0x820u);
    ...
    pd.reg[0x0e]=0;assert(!power_target_poll());assert((gpio&0x830u)==0x820u);
    ...
    reset();force_busy=true;assert(!power_target_poll());assert((gpio&0x830u)==0x820u);
