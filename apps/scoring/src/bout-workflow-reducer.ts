 ...
 *
 * This module deliberately does not create or mutate scoring-core DecisionRecords.
 * New-bout completion is represented as a pending request until a separately
 * authenticated scoring-core response supplies its immutable correlation id.
 */
import {
  isRemoteCommand,
...
type ScoringCoreWorkflowOperation = "safe-idle-sleep" | "scoring-rearm" | "weapon-request"

export type PendingScoringCoreWorkflowAction = Readonly<{
  command: RemoteCommand
  nextAutoRearm: BoutWorkflowSnapshot["autoRearm"]
  nextWeapon: Weapon
  operation: ScoringCoreWorkflowOperation
}>
...
export type BoutWorkflowReducerState = Readonly<{
  ...
  pendingScoringCoreWorkflowAction: PendingScoringCoreWorkflowAction | null
...
export type BoutWorkflowAction = BoutWorkflowCommandAction | ScoringCoreBoutResetResult | ScoringCoreWorkflowResult

/** Schema-v1 result tokens and `stm32RecordId` field names remain wire-compatible. */
export type ScoringCoreBoutResetResult =
  | Readonly<{
...
export type ScoringCoreWorkflowResult =
  | Readonly<{
      authority: "stm32-scoring"
      operation: ScoringCoreWorkflowOperation
      requestId: string
...
export type BoutWorkflowReduction = Readonly<{
  ...
  reason: RemoteCommandRejectionReason | "malformed-action" | "unknown-scoring-core-request" | null
...
/** Verifies descriptors before any untrusted scoring-core field is read. */
...
function freezeState(
  ...
  return freeze({
    ...
    pendingScoringCoreWorkflowAction:
      pendingScoringCoreWorkflowAction === null ? null : freeze(structuredClone(pendingScoringCoreWorkflowAction)),
...
function remember(state: BoutWorkflowReducerState, event: BoutStateEvent): BoutWorkflowReducerState {
  ...
  return freezeState(
    ...
    state.pendingScoringCoreWorkflowAction,
...
/** Strictly authenticates the shape and authority asserted by a new-bout scoring-core result. */
export function parseScoringCoreBoutResetResult(value: unknown): ScoringCoreBoutResetResult {
  if (
    !isStrictPlainRecord(value) ||
    ...
    throw new TypeError("Scoring-core bout-reset results have an invalid shape, authority, or correlation")
  }
...
/** Strictly authenticates a correlated scoring-core rearm, weapon, or safe-idle response. */
export function parseScoringCoreWorkflowResult(value: unknown): ScoringCoreWorkflowResult {
  if (
    !isStrictPlainRecord(value) ||
    ...
    throw new TypeError("Scoring-core workflow results have an invalid shape, authority, or correlation")
  }
  if (value.result === "accepted") {
    if (!isIdentifier(value.stm32RecordId))
      throw new TypeError("Scoring-core workflow results have an invalid shape, authority, or correlation")
 ...
 * revisions, and any scoring-core correlation remain owned by the compensating event.
 */
...
function requestScoringCoreWorkflowAction(
  state: BoutWorkflowReducerState,
  command: RemoteCommand,
  operation: ScoringCoreWorkflowOperation
): BoutWorkflowReduction {
  const pending: PendingScoringCoreWorkflowAction = freeze({
...
/**
 * Reduces one already authenticated command or one scoring-core new-bout response.
...
export function reduceBoutWorkflow(state: BoutWorkflowReducerState, action: BoutWorkflowAction): BoutWorkflowReduction {
  ...
  if (safeCommandAction !== null) {
    ...
    try {
      safeAction = parseScoringCoreBoutResetResult(action)
    } catch {
      try {
        safeAction = parseScoringCoreWorkflowResult(action)
      } catch {
  ...
  if (
    ...
    (state.pendingNewBout?.command.commandId === safeAction.command.commandId ||
      state.pendingScoringCoreWorkflowAction?.command.commandId === safeAction.command.commandId)
  ) {
  ...
  if (
    ...
    state.completedCommandIds.length >=
      (state.pendingNewBout === null && state.pendingScoringCoreWorkflowAction === null
  ...
  if (safeAction.type === "stm32-bout-reset-result") {
    ...
    if (pending === null || pending.command.commandId !== safeAction.requestId) {
      return { event: null, outcome: "ignored", reason: "unknown-scoring-core-request", state }
    }
  ...
  if (safeAction.type === "stm32-workflow-result") {
    const pending = state.pendingScoringCoreWorkflowAction
    if (
      pending === null ||
    ...
    // An accepted scoring-core result is an irreversible authority boundary. Retaining
  ...
  if (state.pendingNewBout !== null || state.pendingScoringCoreWorkflowAction !== null)
  ...
  if (command.command === "scoring.rearm" || command.command === "scoring.autoRearm.advance") {
    if (!isBoutClock(state.snapshot) || state.snapshot.clock.status === "running" || state.snapshot.medical !== null)
      return reject(state, command, "invalid-mode")
    return requestScoringCoreWorkflowAction(state, command, "scoring-rearm")
  }

  if (command.command === "weapon.showOrAdvance") {
    if (!isBoutClock(state.snapshot) || state.snapshot.clock.status === "running" || state.snapshot.medical !== null)
      return reject(state, command, "invalid-mode")
    return requestScoringCoreWorkflowAction(state, command, "weapon-request")
  }

  if (command.command === "device.sleep.request") {
    if (!safeIdle(state.snapshot)) return reject(state, command, "invalid-mode")
    return requestScoringCoreWorkflowAction(state, command, "safe-idle-sleep")
  }
