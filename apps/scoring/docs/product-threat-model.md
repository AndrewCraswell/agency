...
| Z5 venue/network and optical input | Ethernet, BLE/Wi-Fi peers, field-serial peers, update servers, remote controllers, IR receivers, and ambient or deliberately injected IR | Fully untrusted; authentication, freshness, replay protection, and authorization are required for control and update. |
...
| Encrypted IR remote input | A copied, guessed, replayed, reordered, wrong-apparatus, malformed, or flooded optical command changes bout state; counter rollback reuses a nonce; a factory default or leaked pairing key controls multiple units. | Treat every optical frame as hostile until authenticated; per-pair credentials, authenticated encryption, persistent freshness/counter state, duplicate command IDs, physical pairing, revocation, bounded queues/rates, no plaintext fallback, and no universal production key. Applied bout events retain command identity; remote input never creates an STM32 decision. | [Remote contract](encrypted-ir-remote-control-contract.md); BP-121/BP-126/BP-145/BP-146/BP-500/BP-506/BP-509/BP-510; M7-08; M8-02/M8-03. |
