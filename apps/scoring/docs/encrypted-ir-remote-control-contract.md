...
| `RC-03` | in-progress | Root-approved implementation is ready to commit: AES-256-GCM wire profile, pairing/key-custody boundary, anti-replay/counter contract, canonical AAD codec, NIST known-answer gate, and no-fallback rules. Commit is temporarily blocked by the unrelated legislation package/lock mismatch; target adapters remain separately denied under `RC-13` through `RC-15`. | Select encrypted IR protocol, pairing, anti-replay, counter persistence, and key custody. | `RC-01` | Independent security review approves the suite, threat response, provisioning boundary, rotation/revocation, reset recovery, and no-fallback rules. |
