...
| `CW-02` | backlog | The stale whole-tree fingerprint checker was removed during the scoring cleanup because it treated every presentation and adapter edit as a scoring change and no longer matched the committed source tree. Replace it with a concise reviewed ownership inventory limited to scoring decisions, timing, state, and their runtime consumers. | Inventory every scoring-affecting TypeScript branch and consumer. | `CW-01` | A reviewed ownership table identifies every module that can decide scoring, timing, or authoritative state plus its direct runtime consumers; adapters are classified by boundary without per-file source-digest churn. |
| `CW-03` | blocked | Root-approved logical schema and 8 focused tests are ready; closure waits for the simplified `CW-02` ownership inventory. | Freeze language-neutral normalized input, state, result, and error schemas. | `CW-02`, normative rule review | Schemas cover all three weapons, simultaneous events, unavailable and indeterminate inputs, faults, diagnostics, reset, overflow, and exhaustion without JavaScript-shaped semantics. |
