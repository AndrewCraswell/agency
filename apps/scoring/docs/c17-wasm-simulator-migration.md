...
| --- | --- | --- | --- | --- | --- |
| `CW-00` | blocked | Blocked: The regenerated oracle is green only with the broad uncommitted simulator migration; review and commit that coherent simulator unit before activating this baseline. | Repair the current behavior-oracle baseline. | Current repository state | The existing TypeScript-era manifest is regenerated only after review of every changed input, binds all behavior-affecting simulator sources/tests, names the simulator test command, and passes `--check`; this maintenance grants no C/WebAssembly parity credit. |
...
| `CW-02` | in-progress | Root review pending: every ownership row is fingerprinted; the clean-checkout boundary remains to be reviewed. | Inventory every scoring-affecting TypeScript branch and consumer. | `CW-01` | [`c17-scoring-semantic-inventory.json`](c17-scoring-semantic-inventory.json) must bind source digests and AST-derived decision signatures for every production-source ownership row, and check the transitive runtime-consumer closure. Focused and adversarial source-change, branch, adapter-mutation, standalone-scorer, and indirect-consumer checks must pass from a coherent clean checkout before independent approval/commit evidence may be reviewed. |
| `CW-03` | backlog | Next after `CW-02`, normative rule review: Freeze language-neutral normalized input, state, result, and error schemas. | Freeze language-neutral normalized input, state, result, and error schemas. | `CW-02`, normative rule review | Schemas cover all three weapons, simultaneous events, unavailable and indeterminate inputs, faults, diagnostics, reset, overflow, and exhaustion without JavaScript-shaped semantics. |
...
| `CW-09` | backlog | Next after `CW-04`, `CW-07`, `CW-08`: Add native C conformance, sanitizer, property, and fuzz gates. | Add native C conformance, sanitizer, property, and fuzz gates. | `CW-04`, `CW-07`, `CW-08` | GCC and Clang builds pass warnings-as-errors, AddressSanitizer, UndefinedBehaviorSanitizer, static analysis, malformed-input fuzzing, deterministic replay, and full-corpus byte comparison. |
