...
| M3-03 | in-progress | Active: completing the native STM32 host-build scaffold and exact substitutable clock, ADC, comparator, DMA, flash, watchdog, and transport interface evidence after M3-01/M3-02 closure. | STM32 host-build scaffold with hardware interfaces | M3-01, M3-02 | Builds without STM32 hardware and substitutes clock, ADC, comparator, DMA, flash, watchdog, and transport interfaces |
...
| M3-08 | in-progress | Active: auditing and completing the native ESP32 host-build scaffold with substitutable storage, clock, identity, network, display, audio, update, and scoring-link dependencies while retaining STM32-only scoring authority. | ESP-IDF host-build scaffold with service interfaces | M3-01, M3-02 | Storage, clock, identity, network, display, audio, update, and scoring-link dependencies are substitutable |
...
| M4-04 | blocked | Static ERC and the exact 45-reference/26-MPN review queue remain root-reviewed; Molex 43650-0300 is now truthfully identified as an unacquired/unhashed series-drawing lead, while exact drawing/CAD bytes, artwork overlays, and every independent footprint/orientation review remain open. | Single-channel sensing coupon schematic and verified footprints | M4-02, M4-03 | ERC passes; footprints are checked against manufacturer drawings by a second reviewer |
...
| M4-10 | blocked | Primary Stäubli item and catalogue PDFs are acquired and hash-bound for exact red/blue socket selections; their 30.5 mm versus 30.7 mm overall-length conflict now explicitly blocks enclosure/footprint release alongside received-sample plug-fit, retention, cycle, resistance, and salt evidence. | Exact reel-socket selection and physical plug-fit study | None | Color suffixes, retention, contact resistance, sweat/salt exposure plan, harness termination, and cycle target are recorded |
| M4-11 | blocked | Samtec artifacts remain hash-bound; corrected exact Molex drawing URLs and exact Amphenol USB-C page/drawing/spec/3D URLs are manufacturer-listed, but acquisition is blocked by timeout/HTTP 403, so CAD, geometry, enclosure, strain-relief, and release evidence remain DENY. | Communications and power connector CAD/footprint verification | None | RJ45, USB-C, locking power, shield tabs, fasteners, service access, and strain relief match manufacturer drawings |
