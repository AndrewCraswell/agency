...
| M2-03 | blocked | Delivered and root-reviewed: deterministic bounded virtual STM32 shell validates M2-02 snapshots, resolves the reviewed weapon profile before scorer invocation, ignores wrong-profile or untrusted inputs, and emits only immutable scorer-origin outcomes; completion waits for M1-06. | Virtual STM32 device shell independent of epee-specific types | M1-06, M2-01, M2-02 | Selects weapon tables and emits no application-owned decisions |
...
| M4-04 | blocked | Static ERC and an exact 45-reference/26-MPN evidence queue are delivered and root-reviewed; the Molex 43650-0300 series drawing is recorded without releasing geometry, while exact drawings/CAD and generated artwork overlays for the remaining MPNs plus every independent footprint/orientation review remain open. | Single-channel sensing coupon schematic and verified footprints | M4-02, M4-03 | ERC passes; footprints are checked against manufacturer drawings by a second reviewer |
