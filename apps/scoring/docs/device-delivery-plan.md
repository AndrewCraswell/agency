...
| M3-14 | blocked | STM32 7/7 and ESP32 3/3 native suites pass, and the safe seam is specified: publish one bounded STM32 frame into the ESP32 receiver/journal/replay path with duplicate, corruption, ordering, backpressure, reset, and power-loss checks while listing every platform fake. Implementation waits for the required M3-12 Renode disposition so the runner cannot prejudge or conceal target support. | Dual-virtual-firmware integration runner | M3-05, M3-09, accepted M3-12/M3-13 results | Runs the highest-value supported firmware paths together; unsupported peripherals remain host fakes, not hidden omissions |
