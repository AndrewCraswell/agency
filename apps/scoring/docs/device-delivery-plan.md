...
| M3-04 | done | Delivered and root-approved at the firmware-foundation boundary: the heapless C17 core passes the complete checked 54-vector three-weapon corpus, matches exported decision-record fields, and retains 100% line/function/branch coverage. Fix `aadc87c` additionally rejects negative public weapon enums atomically in init and advance; all seven host tests and the STM32G474RE target build/static checks pass. Full fault, uncertainty, generated-profile, portable-ABI, and WebAssembly parity remain owned by CW-03 onward. | STM32 scoring core implementation | M3-03 | Passes the complete three-weapon golden corpus and matches decision records field-for-field |
...
| BT-03 | blocked | Root-reviewed partial limits contract binds BT-02 source ceilings, M4-05 calibration inputs, and the BP-104 isolation screen without claiming DUT performance. Closure remains blocked until calibrated connector measurements allocate line state, leakage, isolation/no-back-power, capacitance, delay/skew, output observation, guarded-fault recovery, and a minimum justified switch topology. | Switch-matrix and programmable-impedance architecture | M4-01, M4-05, BT-02 | Coverage proof chooses the minimum justified topology; resistance, leakage, capacitance, switching time, skew, and fault energy fit allocated limits |
| BT-04 | done | Delivered: Root-reviewed canonical-scenario compiler emits detached, deeply frozen integer-microsecond stimulus and authored expectation timelines without importing a scorer or copying timing constants. Causality fix `853dbf4` additionally rejects every source-linked expectation timestamp that precedes any referenced stimulus while preserving valid post-stimulus and no-decision windows; 13 BT-04/BT-05 tests plus app types, lint, and format pass. | Tester sequence language and canonical-scenario compiler | M0-07, M2-12, BT-01 | Scenarios compile deterministically without copied timing constants; invalid, unsupported, or unsafe steps fail closed |
| BT-05 | done | Delivered and root-approved in `984fc50`: deterministic integer-microsecond command, explicitly virtual measurement, authored expectation, actual-output, and evaluation lanes replay together in the bout observatory. Passed, failed, skipped, indeterminate, and infrastructure-error outcomes remain distinct; infrastructure failures are never reported as DUT failures. Root verification passed the scoring build, 14 focused tests, TypeScript, lint, format, desktop playback synchronization, and a 390 px responsive check with no horizontal overflow. | Virtual tester and bout-observatory integration | M2-12, BT-04 | Commands, measured transitions, expected/actual outputs, evaluations, and pass/fail/skipped/indeterminate/infrastructure-error states replay on one timeline |
