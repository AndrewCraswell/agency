...
| M0-02 | backlog | Next after M0-01: Glossary and units contract for line names, sides, timestamps, resistance, and timing boundaries. | Glossary and units contract for line names, sides, timestamps, resistance, and timing boundaries | M0-01 | No overloaded names; all internal time uses integer microseconds; all electrical units are explicit |
| M0-03 | backlog | Next after M0-01, M0-02: Seven-conductor logical signal contract, weapon excitation phases, and safe inactive state. | Seven-conductor logical signal contract, weapon excitation phases, and safe inactive state | M0-01, M0-02 | Reviewed by software and electrical owners; every illegal or indeterminate state has a diagnostic outcome |
| M0-04 | backlog | Next after M0-02: STM32/ESP32 responsibility and fault-containment contract. | STM32/ESP32 responsibility and fault-containment contract | M0-02 | Scoring authority, reset ownership, watchdog behavior, degraded modes, and forbidden ESP32 decisions are explicit |
| M0-05 | backlog | Next after M0-01, M0-04: Versioned decision-record schema covering hits, rejections, line faults, calibration, and uncertainty. | Versioned decision-record schema covering hits, rejections, line faults, calibration, and uncertainty | M0-01, M0-04 | Schema examples round-trip and reject unknown incompatible versions |
| M0-06 | backlog | Next after M0-04, M0-05: Binary transport frame specification with sequence, length, CRC-32C, message type, and compatibility policy. | Binary transport frame specification with sequence, length, CRC-32C, message type, and compatibility policy | M0-04, M0-05 | Golden encoded frames exist; corruption, truncation, duplication, reordering, and version mismatch outcomes are defined |
| M0-07 | backlog | Next after M0-01, M0-02, M0-05: Golden scenario file format and corpus manifest. | Golden scenario file format and corpus manifest | M0-01, M0-02, M0-05 | A scenario can express line transitions, expected events, expected non-events, and boundary uncertainty without code |
| M0-08 | review | Candidate map and tests exist, but CubeMX/package alternate-function proof for GPIO, ADC, comparator, timer, and DMA conflicts is still missing. | Candidate STM32 GPIO/ADC/comparator/timer/DMA allocation | M0-03 | Every used signal maps to a real MCU function; conflicts and alternate-function constraints are checked |
| M0-09 | review | Candidate N16R2 map and tests exist; schematic/RF/antenna, boot electrical, and physical interface reviews remain before root closure. | Candidate ESP32 GPIO and peripheral allocation | M0-04, M0-06 | Strapping, boot, USB, Ethernet SPI, display, audio, I2C, debug, and antenna constraints are reviewed |
| M0-10 | backlog | Next after M0-04: Power-state and reset-state contract. | Power-state and reset-state contract | M0-04 | Cold boot, brownout, independent reset, watchdog reset, update, and power-loss persistence behavior are defined |
| M0-11 | backlog | Next after M0-04, M0-06, M0-10: Product threat model and firmware trust boundaries. | Product threat model and firmware trust boundaries | M0-04, M0-06, M0-10 | Covers signed updates, rollback, device identity, debug access, network isolation, malformed frames, and recovery |
...
| M1-03 | backlog | Next after M0-01, M0-03, M0-07: Foil contact-break and on/off-target state machine. | Foil contact-break and on/off-target state machine | M0-01, M0-03, M0-07 | Tests cover 13/14/15 ms boundaries, target grounding, lame/weapon faults, and lockout |
...
| ID | Status | Latest state | Deliverable | Depends on | Acceptance |
| --- | --- | --- | --- | --- | --- |
| M2-01 | backlog | Next after M0-02: Explicit virtual microsecond clock and scheduler. | Explicit virtual microsecond clock and scheduler | M0-02 | No simulator behavior reads wall-clock time; equal seeds produce byte-identical output |
| M2-02 | backlog | Next after M0-03, M2-01: Seven-line virtual front-end reading model. | Seven-line virtual front-end reading model | M0-03, M2-01 | Expresses open, closed, grounded, cross-line, resistance bucket, and indeterminate/fault readings |
| M2-03 | backlog | Next after M1-06, M2-01, M2-02: Virtual STM32 device shell independent of epee-specific types. | Virtual STM32 device shell independent of epee-specific types | M1-06, M2-01, M2-02 | Selects weapon tables and emits no application-owned decisions |
| M2-04 | backlog | Next after M0-05, M2-03: Hit, rejection, short, late-hit, parry, whipover, and line-fault event capture. | Hit, rejection, short, late-hit, parry, whipover, and line-fault event capture | M0-05, M2-03 | Each record contains bounded pre/post samples, reason, rule revision, firmware identity, boot ID, and sequence range |
| M2-05 | backlog | Next after M0-06: Canonical binary encoder and decoder. | Canonical binary encoder and decoder | M0-06 | Golden frames match byte-for-byte; malformed length, CRC, type, and version are rejected |
| M2-06 | backlog | Next after M2-05: Fault-injectable virtual processor link. | Fault-injectable virtual processor link | M2-05 | Supports delay, loss, duplication, reordering, corruption, disconnect, and bounded backpressure |
...
| M2-08 | backlog | Next after M0-05, M0-10, M2-07: Event journal and power-fail transaction model. | Event journal and power-fail transaction model | M0-05, M0-10, M2-07 | Power loss at every write boundary yields either the old or new valid state, never a partial record |
| M2-09 | backlog | Next after M0-05, M2-08: Application boot ID, RTC uncertainty, and network-time metadata. | Application boot ID, RTC uncertainty, and network-time metadata | M0-05, M2-08 | Offline and resynchronized timelines remain ordered and explicitly uncertain where required |
...
| M3-05 | backlog | Next after M2-05, M3-03: STM32 binary transport implementation. | STM32 binary transport implementation | M2-05, M3-03 | Passes golden frames, fragmentation, corruption, sequence, and backpressure tests |
| M3-06 | backlog | Next after M0-08, M0-10, M3-03: STM32 target startup, clocks, MPU, watchdog, supervisor, and safe outputs. | STM32 target startup, clocks, MPU, watchdog, supervisor, and safe outputs | M0-08, M0-10, M3-03 | Target build and static checks pass; reset defaults cannot indicate a hit |
| M3-07 | backlog | Next after M0-08, M3-04, M4-08: STM32 acquisition scheduler and DMA buffer adapter. | STM32 acquisition scheduler and DMA buffer adapter | M0-08, M3-04, M4-08 | Host timing tests pass; target timing budget is instrumented for later board measurement |
| M3-08 | backlog | Next after M3-01, M3-02: ESP-IDF host-build scaffold with service interfaces. | ESP-IDF host-build scaffold with service interfaces | M3-01, M3-02 | Storage, clock, identity, network, display, audio, update, and scoring-link dependencies are substitutable |
| M3-09 | backlog | Next after M2-07 through M2-11, M3-08: ESP32 receiver, journal, and replay implementation. | ESP32 receiver, journal, and replay implementation | M2-07 through M2-11, M3-08 | Passes virtual power-loss, duplicate, corruption, reset, and replay scenarios |
| M3-10 | backlog | Next after M0-11, M3-08: ESP32 identity, secure boot, signed update, rollback, and recovery design. | ESP32 identity, secure boot, signed update, rollback, and recovery design | M0-11, M3-08 | Threat-model tests and documented provisioning/recovery flow pass review |
| M3-11 | backlog | Next after M0-04, M3-08: Display/audio/network load-isolation tests. | Display/audio/network load-isolation tests | M0-04, M3-08 | Maximum simulated application load cannot modify scoring records or STM32 timestamps |
| M3-12 | backlog | Next after M3-06, M3-07: STM32 Renode feasibility spike. | STM32 Renode feasibility spike | M3-06, M3-07 | A bounded report identifies supported peripherals and value beyond host tests; no permanent dependency without evidence |
| M3-13 | backlog | Next after M3-08 through M3-10: ESP32 QEMU feasibility spike. | ESP32 QEMU feasibility spike | M3-08 through M3-10 | A bounded report identifies supported ESP-IDF behavior and value beyond host tests |
| M3-14 | backlog | Next after M3-05, M3-09, accepted M3-12/M3-13 results: Dual-virtual-firmware integration runner. | Dual-virtual-firmware integration runner | M3-05, M3-09, accepted M3-12/M3-13 results | Runs the highest-value supported firmware paths together; unsupported peripherals remain host fakes, not hidden omissions |
| M3-15 | backlog | Next after M3-01 through M3-14: Firmware-foundation release. | Firmware-foundation release | M3-01 through M3-14 | Host verification passes; target builds are reproducible; remaining board-only tests are listed explicitly |
...
| M4-05 | backlog | Next after M0-07, M4-01: Socketed resistance, capacitance, and pulse fixture design. | Socketed resistance, capacitance, and pulse fixture design | M0-07, M4-01 | Covers the matrix in `packages/scoring-circuit/docs/analog-front-end.md` with calibrated uncertainty |
...
| M4-10 | backlog | Next unassigned unit: Exact reel-socket selection and physical plug-fit study. | Exact reel-socket selection and physical plug-fit study | None | Color suffixes, retention, contact resistance, sweat/salt exposure plan, harness termination, and cycle target are recorded |
| M4-11 | backlog | Next unassigned unit: Communications and power connector CAD/footprint verification. | Communications and power connector CAD/footprint verification | None | RJ45, USB-C, locking power, shield tabs, fasteners, service access, and strain relief match manufacturer drawings |
