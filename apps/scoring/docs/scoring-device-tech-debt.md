...
| 47 | SC-020 | P1 | ready | Retire the obsolete multi-board executable model after P0 footprint evidence migration |
| 48 | SC-021 | P1 | ready | Cache current-P0 circuit renders so applicable integration tests remain reliable |
| 49 | SC-022 | P2 | blocked | Replace parallel retired BOM/readiness ledgers with the accepted P0 inventory |
...
## SC-020: retire the obsolete multi-board executable model

- Priority: `P1`
- State: `ready`
- Affected files: the retired carrier, scoring-I/O, STM32, isolation, interboard, and legacy BP executable source/test families under `packages/scoring-circuit/src`.
- Description: the single-ESP32 P0 contract is canonical, but 111 old-board and M4-era test files remain discoverable. Large executable models such as `communications-module.circuit.tsx`, `application-display-carrier.circuit.tsx`, `scoring-io-board.circuit.tsx`, and `interboard-interface.ts` still assert the retired carrier, STM32, isolator, and harness architecture.
- Impact: the package suite reports architecture contradictions as failures, so it cannot serve as a trustworthy P0 verification gate.
- Bounded remediation: after P0-06 transfers selected manufacturer and footprint facts into the current inventory, remove obsolete executable sources and tests together. Retain useful manufacturer evidence and review documents without keeping the retired board model executable.
- Dependencies: P0-06 must first preserve every selected footprint and source-evidence binding used by the clean-sheet board.
- Non-goals: do not delete the specifications folder, discard selected manufacturer evidence, or weaken current-P0 assertions to accept both architectures.
- Acceptance checks: normal package test discovery contains only the current architecture and reusable evidence utilities; no production source imports retired STM32, isolator, carrier, or interboard models; all current P0 tests and the integrated PCB gate pass.

## SC-021: cache current-P0 circuit renders in tests

- Priority: `P1`
- State: `ready`
- Affected file: `packages/scoring-circuit/src/p0-digital-peripherals.circuit.test.tsx` and any other current-P0 suite proven to repeat the same expensive render.
- Description: the digital-peripheral suite renders the non-PCB circuit five times and the PCB circuit twice.
- Impact: a valid Ethernet and HUB75 regression suite can exceed Vitest's default timeout, creating noise and wasting verification time.
- Bounded remediation: cache one non-PCB and one PCB Circuit JSON result per suite while preserving every existing net, shield-boundary, and footprint assertion.
- Dependencies: coordinate with the active P0-06 digital-footprint edit to avoid concurrent changes.
- Non-goals: do not weaken assertions, globally raise all test timeouts, or add a generic render-cache framework.
- Acceptance checks: the focused suite passes under the default timeout and still checks the same schematic and PCB properties; package types, lint, and format pass.

## SC-022: replace parallel retired BOM/readiness ledgers

- Priority: `P2`
- State: `blocked`
- Affected files: `packages/scoring-circuit/src/bench-prototype-application-footprints.ts`, `bench-prototype-bom.ts`, `part-readiness.ts`, `fabrication-footprint-gates.ts`, and their historical tests.
- Description: several large architecture-specific registries independently encode old assembly ownership, DNP counts, part selections, and release state.
- Impact: an exact part or reference change requires reconciling multiple stale ledgers while the clean-sheet board needs one selected component and footprint inventory.
- Bounded remediation: after P0-06 accepts the integrated-board inventory, make it the sole executable selection and footprint authority. Preserve historical records only as non-executable evidence when they retain useful provenance.
- Dependencies: blocked on root acceptance and commit of P0-06.
- Non-goals: do not create a monorepo-wide parts framework, delete specifications, or duplicate the 256-component schematic in a manually maintained shadow BOM.
- Acceptance checks: one current inventory derives from the integrated schematic; selected non-generic parts bind reviewed footprint evidence; retired ledgers are no longer executable authorities; current P0 tests and the package verification gate pass.
