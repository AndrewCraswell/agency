...
const useToolbarNavigation = (toolbarRef: React.RefObject<HTMLDivElement | null>) => {
  ...
  const initialContainerTabIndexRef = useRef<number | null | undefined>(undefined)
  ...
  useEffect(() => {
    ...
    if (initialContainerTabIndexRef.current === undefined) {
      initialContainerTabIndexRef.current =
        toolbar.hasAttribute("tabindex") && toolbar.tabIndex >= 0 ? toolbar.tabIndex : null
    }
  ...
  useEffect(() => {
    ...
    const handleBlur = (e: FocusEvent) => {
      ...
      const leavesToolbar =
        !nextTarget || !NodeConstructor || !(nextTarget instanceof NodeConstructor) || !toolbar.contains(nextTarget)
  ...
  useEffect(() => {
    ...
    const activeIndex =
      currentIndex >= 0 && items[currentIndex] ? currentIndex : existingTabStop >= 0 ? existingTabStop : 0
