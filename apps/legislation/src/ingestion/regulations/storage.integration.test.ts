...
import { createLegalEditionSearch } from "../../api/legal-edition-search.js"
...
suite.sequential("regulatory edition storage on real PostgreSQL", () => {
  ...
  it.skipIf(!process.env.REGULATORY_SEARCH_TEST_DATABASE_URL)(
    ...
    async () => {
      ...
      try {
        ...
        const editionSearch = createLegalEditionSearch(pool, target, ["org-regulatory-test"])
        const searchEdition = () =>
          runWithRequestContext(context, () =>
            editionSearch({
              editionIds: [first.editionId],
              query: "Original"
            })
          )
        expect((await searchEdition()).hits).toHaveLength(1)
        ...
        try {
          await expect(searchEdition()).rejects.toMatchObject({ category: "dependency_unavailable" })
  ...
  it("serves exact provision text through HTTP and the typed client, with account-bound continuation and live rights", async () => {
    const data = await materialized(
      await input({ body: `<DIV8 N="1.1" TYPE="SECTION"><HEAD>Reader</HEAD><P>${"a".repeat(40_000)}</P></DIV8>` })
    )
    await publishRegulatoryEdition(pool, data.lease, null)
    const rows = await pool.query(
      "SELECT v.id,v.body FROM legislation.legal_provision_versions v JOIN legislation.legal_edition_provisions m ON m.version_id=v.id WHERE m.edition_id=$1 ORDER BY length(v.body) DESC LIMIT 1",
      [data.editionId]
    )
    const source = z.object({ id: z.uuid(), body: z.string() }).parse(rows.rows[0])
    ...
    const first = await api.getLegalText(source.id, { editionId: data.editionId, limit: 1 })
    ...
    while (cursor) {
      const page = await api.getLegalText(source.id, { editionId: data.editionId, limit: 1, cursor })
    ...
    await expect(
      api.getLegalText(source.id, { editionId: data.editionId, limit: 1, cursor: first.data.nextCursor })
    ...
    await expect(api.getLegalText(source.id, { editionId: data.editionId })).rejects.toMatchObject({
      category: "forbidden"
    })
  })
