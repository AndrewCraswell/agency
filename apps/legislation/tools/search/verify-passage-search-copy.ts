...
try {
  ...
  if (process.argv[2] === "--initialize") {
    ...
    await target.query(await readFile(new URL("../../infra/passage-search/schema.sql", import.meta.url), "utf8"))
  ...
  const query = dialect.sqlToQuery(rankedSectionPageQuery({ query: "the OR section OR act", limit: 20 }))
  await target.query("begin read only")
  await target.query("set local statement_timeout='5s'")
  const started = performance.now()
  const hits = await target.query(query.sql, query.params)
  assert.ok(hits.rows.length > 0)
  const elapsedMs = Math.round(performance.now() - started)
  await target.query("commit")
  process.stdout.write(
    `${JSON.stringify({ documents: documents.length, sections: copied, replay: "passed", hits: hits.rows.length, elapsedMs, scope: "copy canary, not full-corpus acceptance" })}\n`
  )
} finally {
