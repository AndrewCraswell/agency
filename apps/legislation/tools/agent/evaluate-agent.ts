...
import { z } from "zod"
import { createResearchModel, researchAgentLimits } from "../../app/chat/agent"
import { getResearchPrompt } from "../../app/chat/prompt"
import { assertSafeArtifact, datasetSchema, digest, experimentSchema, type EvalScore } from "../../app/evals/contracts"
import { evaluationInputContract } from "../../app/evals/evaluationInput"
import { criticInstructions, evaluateResult, judgeInstructions } from "../../app/evals/evaluators"
import {
  CheckpointMismatch,
  createStageJournal,
  durableCallCounter,
  withEvaluationLock,
  writeArtifact
} from "../../app/evals/journal"
import { createEvalLangfuse, observeEval } from "../../app/evals/langfuse"
import { createCallBudget, executeCase } from "../../app/evals/runner"
import { caseResultSchema } from "../../app/evals/runner"
import { smokeDataset } from "../../app/evals/smoke"
...
async function main() {
  ...
  if (
    values.resume &&
    (values.dataset || values.case || values.judge || values.config !== "evals/agent-experiment.json")
  ) {
  ...
  const langfuse = createEvalLangfuse(process.env)
  await langfuse.verifyProject()
  const synced = frozen?.synced ?? (await langfuse.syncDataset(dataset.name, cases))
  if (values.sync) {
  ...
  const trackedDiff = execFileSync("git", ["-C", root, "diff", "HEAD", "--", "apps/legislation"], {
    encoding: "utf8",
  ...
  const sourceFiles = execFileSync(
    ...
    [
      ...
      "--exclude-standard",
      "--",
      "apps/legislation/app/chat",
      "apps/legislation/app/evals",
      "apps/legislation/app/lib",
      "apps/legislation/src/mcp",
      "apps/legislation/tools/agent/evaluate-agent.ts"
    ],
    { encoding: "utf8" }
  )
    .trim()
