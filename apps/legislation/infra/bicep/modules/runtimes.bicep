  ...
  properties: {
    ...
    template: {
      containers: [{
        ...
        args: [
          '-c'
          'n8n import:workflow --separate --input=/opt/legislation/workflows --activeState=false && n8n update:workflow --id=legislation-congress-sync --active=false && n8n update:workflow --id=legislation-congress-events-refresh --active=false && n8n update:workflow --id=legislation-congress-votes-refresh --active=false && n8n update:workflow --id=legislation-congress-amend-refresh --active=false && n8n update:workflow --id=legislation-coverage-report --active=false && n8n update:workflow --id=legislation-document-processing --active=false && n8n update:workflow --id=legislation-embedding-refresh --active=false && n8n update:workflow --id=legislation-govinfo-bootstrap --active=false && n8n update:workflow --id=legislation-openstates-refresh --active=false'
        ]
        env: [
          { name: 'DB_TYPE', value: 'postgresdb' }
          { name: 'DB_POSTGRESDB_DATABASE', value: n8nDatabaseName }
