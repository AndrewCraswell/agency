...
`apps/legislation/app/api`. Route migration and staged Railway release readiness are tracked in the
...
`apps/legislation/app/ready/route.ts`; documented `/api/**` operations remain under the `app/api` boundary. All
