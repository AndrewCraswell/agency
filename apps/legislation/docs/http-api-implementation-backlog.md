...
2026-08-25 through reviewed commit `d64236e`. Bill detail, batch detail, bill amendments, bill votes, the canonical bill
timeline, supporting-material section pages, person bill activity, and subscription mutations now have focused endpoint
and query evidence plus reviewed commits. Current totals are 0 **In progress**, 37 **Blocked**, 0 **Ready**, and 50
**Done** across 87 endpoints.
...
| API-04 | Subscriptions and webhooks | In progress | Subscription reads and mutations and webhook reads have complete endpoint evidence and are **Done**. Webhook mutations remain blocked on durable secret protection, exact replay transactions, and the pinned outbound verification executor. Delivery workers, matching, and event materialization are separate background-processing deliverables, not gates for the subscription resource endpoints. |
...
matrix is 0 **In progress**, 37 **Blocked**, 0 **Ready**, and 50 **Done** routes.
