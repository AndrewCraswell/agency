...
| ADR-015 | Use the existing Next.js 16 App Router application in `apps/legislation-web` as the legislation application and public API runtime, with one explicit Route Handler under `apps/legislation-web/app/api` per documented operation. | One framework owns the future application and HTTP boundary. Existing standalone Node handlers and domain code in `apps/legislation` remain reusable migration input, but the former Railway `legislation-api` service is deleted and is not a rollback target. A route is complete only after its Next.js handler passes a staged `legislation-web` deployment and remote smoke. WorkOS auth follows all route migrations, distributed rate limiting follows auth, and MCP HTTP cutover is last after a live Next.js MCP route exists. |
