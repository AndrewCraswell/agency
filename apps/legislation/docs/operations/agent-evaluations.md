...
accepts additional cases validated by the canonical schemas in `app/evals/contracts.ts`; keep a reviewed cumulative
...
Old runs without this execution contract cannot be resumed in place. `eval:agent:regrade` reads frozen candidate outputs
...
upload files; retry those with `eval:agent --upload GRADING_DIRECTORY` if necessary.
...
start in `app/evals/evaluators.ts`. Runs with judging enabled resolve the Langfuse `evaluation` label once at startup,
