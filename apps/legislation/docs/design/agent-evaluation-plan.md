...
- `app/chat/route.ts` hardcodes a Luna model and low reasoning effort. It uses eight model steps, 4096 output tokens,
  and a 120-second timeout; the research wrapper permits 24 attempted research executions.
- `app/chat/prompt.ts` loads `legislative-research` using the moving `production` label. Experiments must resolve and pin
