...
| --- | --- |
| [C data and versions](../../../../packages/legislation-core/docs/regulations/data-contract.md) | Canonical identities, schema, editions, provenance, temporal semantics, rights and persisted publication invariants |
...
| [I search and embeddings](search-indexing.md) | Passage generation, copy/index operations and benchmark evidence; shared routing comes from C |
...
commercial alternatives. This I page is the program map, not a duplicate shared data or serving specification. The owning
contracts above govern their respective boundaries. Revalidate source details against actual responses at implementation.
...
| --- | --- | --- |
| Canonical storage | C `src/database/`; I `src/persistence/`; W query repositories | C schema/primitives, I writes and W reads, without sibling imports |
...
| Embeddings | C `src/embeddings/`; I `src/ingestion/embeddings/`; W retrieval runtime | Shared routes/client/tokenizer, I generation and W query/model execution |
| Text search | I queue/replication; C `infra/passage-search/`; W serving | Regulatory outbox, isolated generation and separately accepted reader |
| HTTP | W explicit `app/api/.../route.ts` and application services; C typed client | Authenticated operations, shared envelopes and wire validation |
| MCP | M `src/mcp/http-query-adapter.ts` and registration | Calls W over HTTPS with exact API parity; no DB/model/provider runtime |
| Events | I canonical change publication; W subscriptions and webhooks; C shapes | Historical suppression with W-owned customer delivery |
