...
- [ ] **PASS-09 Prepare full selected partitions.** Run the deployed preparation worker on validated current/FR/history
  ...
  275,138 pending. The planner now requires the audit's model/tokenizer/catalog/owner descriptor, persists it in the
  immutable wave identity and checks every database lookahead against the admitted ordered owners before rights or
  dispatch writes; exhaustion must exactly account for every owner. Candidate selection, deployed execution and complete
  partition accounting remain open.
...
- [x] **INDEX-01 Design cross-version filter projections.** Add source/rights, jurisdiction, corpus, code, agency,
...
- [ ] **EVAL-04 Complete blind relevance review.** Use existing rank-blind packets; record grades, rationales, multiple
  ...
  The review contract now supports multiple independent judgments per candidate and a separate adjudication. Automated
