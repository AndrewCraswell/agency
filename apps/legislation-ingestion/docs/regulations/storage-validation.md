...
I owns producer/recovery procedures and the dated source evidence here. C owns
[schema/rights](../../../../packages/legislation-core/docs/regulations/data-contract.md) and
[reader invariants](../../../../packages/legislation-core/docs/regulations/reader-contract.md); W owns
[query/reader acceptance](../../../legislation-web/docs/regulations/legal-search-serving.md).
Run package-local `pnpm tool` examples from I. Historical artifact paths still refer to the retained original checkout
unless explicitly relocated; do not recreate or overwrite evidence to make a relative path resolve.
...
The [C projection baseline](../../../../packages/legislation-core/docs/engineering/search-projections.md) owns passage
generation/schema invariants. `passage-storage.ts` prepares outside the write transaction, then locks and rechecks the published source
...
Use [C's constraints](../../../../packages/legislation-core/docs/regulations/data-contract.md#indexes-and-constraints).
Short row locks serialize lease/head updates; see [PostgreSQL locking](https://www.postgresql.org/docs/current/explicit-locking.html).

## Execution and checks

### Scope publication, current reads and revocation

W owns [internal reader and external-use authorization](../../../legislation-web/docs/regulations/legal-search-serving.md#internal-reader-boundary).
An internal local-search permission is not API entitlement. The producer handoff below supplies its acknowledged scope.
...
W owns current-head selection and failure when a newer head lacks a completed copy. Annual history and FR scoped
acknowledgements do not create a current eCFR head or latest regulatory-action graph.
...
W owns the [internal lexical reader contract](../../../legislation-web/docs/regulations/legal-search-serving.md#internal-reader-boundary).
Real-source parity evidence here compares IDs, ordering, scores and content; it does not establish public readiness.
...
```powershell
# Run from apps/legislation-ingestion after W releases C migrations and the disposable database has its canonical US jurisdiction.
