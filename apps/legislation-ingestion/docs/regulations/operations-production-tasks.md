...
- [ ] **SYNC-02 Implement eCFR change discovery.** Poll title metadata under the configured hourly discovery policy,
  ...
  changed acquisition units. Its concurrency is one and no recurring schedule is registered. Pending units can now be
...
- [ ] **SYNC-03 Implement FR modification discovery.** Query source modification time with overlap, bounded pagination
  ...
  replay-stable self-continuation. Old issue dates remain distinct from modification time. Pages with new units now start
  the bounded shared controller. Parsed issues use a source-specific adapter that freezes exact-day publisher metadata,
  ...
  deployed durable-volume verification and recurring activation remain open. A bounded manual recovery controller can
  now resubmit expired PDF work and ready finalizers using durable row revisions; it has no schedule. No recurring source
