...
- [ ] Production event-read closure remains open. The database contains 2,849 Alaska and one North Carolina Open States event rows with complete transport provenance, dates, classifications and source URLs, but all have `canonical_facts_complete=false`; no AK/NC meeting is currently public through the API. The verified Alaska scraper inventory and conservative normalization exist, but its 2,846 non-conflicting session-34 occurrences have not been promoted through hosted orchestration. Do not count a successful empty meeting page as event-data acceptance.
