...
- [x] Ran an isolated HB1 journal extraction from the retained, tested Alaska scraper image at upstream revision `d43f853796ceeeb49205f7d144790647764ce105`. Archived the seven-file result through the normal Azure state-source store under manifest `openstates/scrapers/d43f853796ceeeb49205f7d144790647764ce105/ak/bills/ak-vote-replay-20260917/retained.json`; the manifest SHA-256 is `a9f44a4a3700ba6cf4b350e81d5cffdc937a65eecc3527a6b28a52d49249ba7e`, and checksum read-back passed.
- [x] Added a fail-closed, bill-scoped reconciliation command. It verifies the immutable manifest and approved scraper build, requires a complete one-to-one vote inventory, rejects contradictory counts/dates/provenance or ambiguous matches, preserves canonical vote IDs and previously resolved person links only for the same source observation, and never infers people from names. Dry-run is the default; apply locks the one parent bill and changes only the matched votes, their source positions and canonical observations in one transaction.
...
- [x] Deployed the frozen Alaska event planner and sequential cloud-promotion controller from clean commit `74a277e` as Trigger `20260917.5` after correcting the packaged Python asset path. Production planner `run_06gatakhl0c71f5oqhklt7hke1` completed in 8.1 seconds and immutably retained official inventory `24b43a38d3ced425946df56399242e20c2044080fb55456d72042af4ae3f3763`: 2,847 accepted occurrences in 285 ten-or-fewer-event batches, with both conflicting occurrences retained in quarantine. Dispatch `run_06gatam6vpdtvskpidkt21m301` selected the first pending batch without resetting prior receipts.
- [x] Alaska event promotion and relationship/readiness reconciliation completed for the frozen inventory. Production has all 285 immutable promotion receipts for inventory `24b43a38d3ced425946df56399242e20c2044080fb55456d72042af4ae3f3763`. The reconciliation chain ended at batch index 284 in Trigger run `run_06gb131m7dsa5vlr881nljdn01` with `status=cycle_reconciled`, seven final-batch events and three organization links. A later batch-zero invocation on `20260917.13` reused the existing global continuation idempotency keys rather than duplicating the completed chain. This closes ingestion relationship readiness; public meeting API/MCP acceptance remains a separate serving check.
- [x] Closed the Alaska HB10 extraction failure without a bill-specific exception. The publisher splits the 37-name effective-date vote across a printed journal page header and blank text nodes. The shared parser now ignores recognized split page-header fragments only while a voter group remains short of its declared tally; exact counts, valid names, disjoint groups and unique bill/tally selection remain fail-closed. Six parser tests and the source-policy/runner suites passed. An offline rebuild from the original digest-verified archive produced build-input fingerprint `b9ba2dbe321b86084243119f0f36c83d304abca89e1061457453114629c60314` and local image `sha256:6ba4912cb173d4a7807661b8300c36036f702691b726673d8a0ad3c56eac96b2` from verified base `sha256:b1fc3596b68a071b480ead1569bdc571f631dc1fdcfe7908098e78aaa2b82e53` with networking disabled.
- [x] Closed the next Alaska HB13 extraction failure without a bill-specific exception. One official journal page contains several different amendments with identical 15-23-2 tallies, so tally plus bill identity is insufficient. The scraper now preserves the official action URL's named journal anchor, bounds parsing to that anchored action while ignoring embedded bill and printed-page anchors, and still requires one exact tally, complete disjoint voter groups and valid source names. HB13's complete bounded extraction now succeeds with nine source-backed votes and 340 named positions. Nine parser tests and source-policy verification passed; the offline rebuilt inputs have fingerprint `52e131d0814a3ad7965a2ee0387f7da307313bdbfc717562c79ec4d0fa0efd02`, and local image `sha256:98eadf29bdceb580c180fa0d17f186458e8ef72d1a1636ebab32ba176f5b61eb` passed startup verification and a runner-isolated HB13 canary. That exact digest is staged under the unique non-active ACR tag `openstates-scraper:20260917-journal-anchor-fix-67c0d4d`; the active Azure job remains unchanged until the older Alaska event chain drains.
- [x] The extended Alaska batch then exposed a second source-wide anchor variant on HB16: the publisher repeats the same `AM11` anchor for an amendment and its amended motion. The parser now evaluates every bounded occurrence of the official anchor and accepts only one occurrence that independently matches bill identity and the complete tally; zero or multiple matching roll calls still fail closed. A runner-isolated HB16 extraction succeeded with all seventeen votes after this change. Ten parser tests and source-policy verification passed; current build-input fingerprint is `8a840aee67bd6c21f3f1239aa2d1ee86d6575ccb9c88368ec80c654c29a177af` and local image digest is `sha256:a250e439f68b92f680ce4e8909c1d5c16c7530ba74277c13853b0f354f25cc96`. This image is not active and has not replaced the draining Azure event worker.
- [x] HB14 exposed the complementary printed-page case: its bill heading precedes the numeric page anchor targeted by the action URL. Candidate validation now admits only the bounded 1,500-character heading context before that page marker, then still requires the nearest bill reference and exact tally. A runner-isolated HB14 extraction retained all four roll calls. Eleven parser tests and source-policy verification passed; current build-input fingerprint is `77472ea67ca34927bbb8eb779066c7dff768e946e660565ad56b47c4d6ccfd06` and local image digest is `sha256:eadd03cfdbcd56907d463468b6555c1368dd0b77a8f67980e8e240bdb89d0e74`. The complete HB11-HB20 extraction then retained all ten bills and 33 votes, and the shared controller promoted the second frozen batch atomically against isolated `legislation_test`; the cycle now has 20/856 bills and 2/87 batches promoted. The exact image is staged under unique non-active ACR tag `openstates-scraper:20260917-journal-anchor-fix-7077fb6`; it remains inactive pending the old event-chain drain.
- [x] HB25 exposed an invalid publisher fragment: the action URL names `#AM8`, but the journal only has the printed page anchor `2198`. The scraper now derives a numeric fallback solely from the same official URL's validated `Page` query and uses it only when the named fragment is absent; a present-but-ambiguous fragment never falls back. Numeric page actions may span embedded amendment and later page anchors, but bill plus exact tally must be unique in the resulting suffix. A runner-isolated HB25 extraction retained all fifteen votes. Fourteen parser tests and source-policy verification passed; current build-input fingerprint is `d8914ee780f0bb9d17e818d7d29b9ca4606b36c6ed91fc4b114fe067ab75b280` and local image digest is `sha256:515382285d55aeb17ab767dcf77884c9c36d00db1b9d832e97d4704cdcd081ac`. The full HB21-HB30 extraction retained all ten bills and 32 votes, and the controller promoted the third frozen batch atomically; isolated acceptance is now 30/856 bills across 3/87 batches. The exact image is staged under unique non-active ACR tag `openstates-scraper:20260917-journal-anchor-fix-2283a32`; it remains inactive pending the old event-chain drain.
- [x] Re-ran both state bill paths through the shared controller against isolated `legislation_test`. Alaska frozen cycle `ak-bills-99943f0de8856a045197f1de9436035f` promoted HB1-HB10 atomically; HB10 retained six votes and 220 source-name positions, including the exact 37-3 page-spanning vote. North Carolina frozen cycle `nc-bills-deab20e75843fa5aa7812075037ca7a1` independently promoted H1-H10. Each cycle committed exactly one immutable promotion receipt and released its runtime-confirmed ownership hold. These local canaries prove shared orchestration and parser acceptance, not full-cycle production completion or person resolution.
- [x] Pushed the corrected image under the unique, non-active ACR tag `openstates-scraper:20260917-journal-page-fix-22cd905`; registry digest read-back is `sha256:6ba4912cb173d4a7807661b8300c36036f702691b726673d8a0ad3c56eac96b2`. The Azure job remains pinned to the prior digest while the Alaska event inventory drains. The next Trigger deployment approves the corrected build-input fingerprint, but neither the active job image nor the event controller is replaced before all existing receipts settle.
- [x] Hardened the next Trigger cutover so direct event-cloud payloads cannot select an arbitrary syntactically valid build fingerprint. New extraction accepts only the current reviewed build. Relationship reconciliation additionally accepts the exact prior build solely so the already committed Alaska event receipts remain replayable after image cutover; any other digest fails before archive preparation or database writes.
...
- [x] Activation flags can be previewed without `--apply`. A production dry-run with quota acknowledgement and allowlist `ak,nc` selected the 156 OpenStates schedules only: 150 unchanged and exactly six activate actions, with zero federal schedules in scope. Applying remains a separate explicit operation.
- [x] Current-source acceptance passed on Trigger `20260917.1` for all six AK/NC lanes. Bills updated 49 AK and four NC records, entities updated 392 AK and 2,025 NC records, NC events updated one record, and AK events updated 2,849 records; every lane reported zero failures. The AK/NC-only schedule gates were then uploaded, the same six-action plan was reviewed and applied, and repeat reconciliation returned 156 unchanged schedules with no actions. All federal schedules and the other 50 state/territory schedules remained inactive.
- [x] Alaska content completed a full AK34 scan and a separate freshness audit: 857 bill embeddings and 66,722 section embeddings had zero missing or stale records. The final bounded continuation reported one complete scan round, not global search/API readiness. Seventeen reviewed unsupported formats, two publisher 404s, one malformed source and the unavailable HB9050B source remain explicit source limitations rather than fabricated content.
- [x] The Alaska legislature intermittently returned HTTP 500 for its official `Bill/Text` route while the matching `Bill/Plaintext` route remained available. Added a source-family fallback guarded to the exact official host/path/query grammar, plus a validation-first retry mode for exhausted transient downloads. Forty downloader tests, nine retry-tool tests, ingestion type-check, lint and formatting passed. Clean snapshot `e8983ed` deployed as Trigger `20260917.3`. HB9050A was independently downloaded/extracted, guardedly requeued and processed to 225 characters with a fresh section embedding; HB9050B still returns HTTP 500 from both official routes and was not reclassified as successful.
- [x] North Carolina continuation `run_06gas9rts116b8hdepjfam5k01` exhausted its bounded 100-continuation budget without losing its checkpoint or overlapping the state/session lease. After confirming that its Trigger status was terminal and that no database lease remained, replacement controller `run_06gasibm3l5mpjahjkot4glg01` resumed the retained cursor on Trigger `20260917.3` with bill concurrency two. At 07:48Z the cursor crossed the final `sr` record, reset to the beginning and began the next round, proving one complete NC2025 scan without a reset or overlapping worker. A subsequent repeatable-read audit found all 2,338 bill embeddings and all 36,187 document-section embeddings present with matching input hashes, contracts and dimensions. The replacement completed at 07:54:57Z after its fixed 100-continuation budget with one complete scan round; `ingestionComplete=false` remains the controller's conservative bounded result, not evidence of missing vectors.
- [x] Repeated the Alaska freshness audit after the guarded HB9050A recovery. All 857 AK34 bill embeddings and 66,723 document-section embeddings were present with matching input hashes, contracts and dimensions. The one-section increase from the earlier audit is the recovered official HB9050A text; HB9050B remains an explicit publisher failure.
- [x] Dedicated passage-search parity was established from indexed processed-document inventories rather than a collation-dependent range. Alaska matched exactly at 61,390 documents, 259,075 sections, 785,343,784 text bytes and XOR fingerprint 8,517,708,952,800,530,311. North Carolina matched exactly at 30,315 documents, 151,997 sections, 511,155,724 text bytes and XOR fingerprint 959,898,321,125,554,698. The backfill checkpoint was complete with no pending or failed work, the BM25 index was valid and ready, and no index blocker was present.
- [x] Production web cutover enabled the dedicated passage-search database with ranking generation `ak-nc-full-corpus-2026-09-17`, a 10-second statement timeout and five connections. Railway deployment `93b7d531-c52d-4fc5-9831-213b32e2533b` reached SUCCESS. Authenticated lexical `education` searches returned ten canonical Alaska results in 900 ms and ten canonical North Carolina results in 242 ms; `/health` and `/ready` both returned 200. This proves machine-token API acceptance, not the separate user-consent MCP gate.
- [x] Recovered the Alaska HB 9050 second-session text route through the shared official-PDF fallback. Canonical document `bill:ak:34:hb:9050:document:9fb3099400719282cac03eec` is processed with source hash `dc03433ff06bcb4c52df738af53881fe105b7569a83af730f1c55245d29fd41b`; its 448-character section has one routed `openai/text-embedding-3-small` embedding. The isolated passage-search row has the matching section hash, and a live ParadeDB lexical query for `test` returned that exact section with score 21.033371. The separate direct-PDF source record remains an explicit unsupported/not-found observation rather than being rewritten.
- [ ] Alaska frozen bill replay inventory `aabfb2518ee7c6934f4aeb35cedbc350659c397e0edd1dcc77ab1b576278f2a9` is actively draining on Trigger `20260917.13`. The previously failing batch `3a876a649968ad6560aa726b71f4402b692778ac89079a54a108f7bbefb95743` completed as `run_06gb1lvuavsrcvdh9sertqks01` after monotonic sponsor reconciliation was deployed, promoted ten bills and launched its successor. The next three observed batches also promoted successfully. Completion still requires all 87 receipts, final action/vote provenance audits and downstream serving acceptance.
- [x] Restored the standalone MCP deployment after the monorepo split. Root `.railwayignore` now admits the three legislation apps and shared core package while continuing to exclude credentials, dependencies, outputs and artifacts. Clean commit `2ab6a85` produced Railway image `sha256:1ed1e6777860967918f82173fff6e7ea0f80d25af2efade392c4d2f46f576bf6`; deployment `08e94031-e73e-4460-9ed2-ee0b91d9f67c` reached SUCCESS after adding the required `AUTH_MODE=workos` setting. The standalone origin returns 200 for `/health`, `/ready` and canonical protected-resource metadata. Anonymous and API-audience tokens both receive the expected 401 challenge for the exact standalone `/mcp` resource.
- [x] Reworked the production foundation smoke to take an explicit jurisdiction/session pair and scope bills, amendments, votes, people, organizations and meetings to it. Machine-authenticated AK34 and NC2025 runs passed health, readiness, route envelopes, conditional reads, bill/timeline/document/section/vote/person/term and resource-batch checks. AK amendment detail also passed; NC has no configured amendment fixture. A later production smoke used canonical Alaska meeting `event:openstates:ak-meeting-34-c31118df483064ff313111887f14414e07f9012fe52196ce1954557f81e57fe6`: meeting detail, its six-item agenda list and configured agenda-item detail, participant list and participant detail all returned valid authenticated resources. Meeting-document checks remain conditional because this fixture has no documents.
- [x] Rehearsed both immutable foundation archive pairs against isolated `legislation_test`, including the real people, term, committee and membership writes. Alaska produced 167 accepted people, 274 source-backed terms, all 33 committee identities and all 233 current memberships; North Carolina produced 506 people, 581 terms, all 94 committee identities and all 1,678 current memberships. The Alaska rehearsal still records four held historical timelines, but a complete validated current directory now establishes those current identities and current terms without accepting the contradictory older assertions. This is a provider-neutral rule, not a named-person exception; a partial current directory cannot use it.
- [x] Confirmed the North Carolina district-1 gap is upstream rather than an importer omission. The pinned people revision remains the current Open States `main` HEAD, but its current snapshot contains no Senate district 1. The official NCGA 2025-2026 member list records Bobby Hanig as resigned August 24, 2026 and Jerry Tillett as appointed September 3, 2026. The observed Open States archive therefore remains explicitly partial; the importer does not invent the replacement.
- [x] Diagnosed the 49 Alaska action-provenance holds without weakening the exact-timeline guard. The retained AK34 session archive is still the same 27,522,771-byte August edition at its recorded Open States URL, while recurring API synchronization has since imported newer action timelines. The recurring path previously retained only a database content hash, not the provider response bytes, so stale evidence correctly could not authorize those URL fills.
- [x] Closed that durability gap in the shared current-state pipeline. Every non-empty Open States bill response is now retained before canonical writes, partitioned by canonical session, with immutable bytes plus jurisdiction, session, query window, page, retrieval time and provider-endpoint metadata in the state-source store. The import checkpoint uses the same archive stream and SHA-256, making later exact provenance replay discoverable and deterministic. Archive failure aborts before database mutation; mixed-session pages are separated rather than silently filtered. Ten focused archive, synchronization and provenance tests, scoped lint, formatting and ingestion type-check passed. Deployment waits for the active Alaska event chain to drain.
- [x] Added the bounded recovery path for responses imported before byte retention existed. Manual task `openstates-action-provenance-capture` accepts only Alaska or North Carolina, one canonical session and an ISO retrieval floor no more than 120 days old. It reacquires the included provider records, archives each exact session/page before doing anything else, and queues the existing serialized exact-timeline reconciler with separate inspect/apply idempotency. It never imports or changes bill facts, and a request returning no target-session evidence fails rather than reporting success. Nine focused task/archive/reconciliation tests, scoped lint, formatting, ingestion type-check and a Trigger dry build passed; no production task was started.
- [ ] Deploy the committed current-identity/history separation only after the active Alaska event chain drains, then replay both exact foundation archive pairs through `openstates-foundation-import`. Verify publisher committee identifiers and memberships before starting Alaska relationship reconciliation or North Carolina meeting promotion.
- [ ] Production event-read closure remains open. The active frozen-inventory chain is adding Alaska rows with complete transport provenance, dates, classifications and source URLs, but publication remains fail-closed until the exact receipts are relationship-reconciled after foundation replay. No AK/NC meeting is currently public through the API. Do not count a successful empty meeting page or raw promoted rows as event-data acceptance.
- [ ] Positive WorkOS MCP-resource consent and authenticated tool calls remain open. A one-time device authorization accepted the exact new resource indicator without creating a client, but the operator consent window expired before a token was issued. No token was printed, persisted or added to service variables.
...
- [x] The prior post-deployment AK HB1 422 was traced to incomplete vote provenance. The source-verified, bill-scoped reconciliation above closed the two known HB1 vote rows without weakening the readers. Live endpoint acceptance is tracked separately above.
- [x] Corrected hosted web service configuration left behind by the monorepo split: `configFile=/apps/legislation-web/railway.json`, matching Dockerfile and watch paths including core. Read-back matches committed configuration. Credentials, database configuration and activation flags unchanged. Initial snapshot release `33266a48-afc7-4a74-ad86-da7df3e93aa9` failed before build because the old `/apps/legislation/railway.json` was absent; this was not an application runtime failure.
- [ ] Clean committed snapshot `f313474` reader release `5b87b778-218d-4699-8212-ed94c30ef450` is BUILDING, with dependencies installed and Next production compilation started. The intervening config-triggered redeploy `c1a97121-4c50-4281-bef1-ea48dedc2996` succeeded using the previous image; it is not proof of new reader deployment. Poll the exact new release and smoke test after SUCCESS; do not re-upload merely because observation times out.
- [ ] Subsequent release evidence: `5b87b778-218d-4699-8212-ed94c30ef450` FAILED on missing tracked source `legislation/coverage/report.ts`. Re-uploading the same clean tracked snapshot with `--no-gitignore` resolved that omission. Release `39a77d28-2cbf-4ecd-96ff-0ee0c57f191d` then FAILED on TS2883 inferred type portability in two conversation Storybook fixtures. Added explicit public type annotations only; focused web type-check/lint passed. Full verification now stops on concurrent legal-code route/test lint errors. New reader release still requires SUCCESS and live smoke evidence.
...
- [x] Added bounded session action-provenance orchestration. Each continuation verifies the immutable archive hash, normalizes the entire fixed inventory, selects an exact cursor-bound batch, locks parents/actions in two indexed queries, fills only null URLs with one guarded set update, and quarantines changed or missing timelines without stopping independent matches. The task is serialized, idempotent, resumable, manual-only and defaults to read-only inspection.
- [x] Full production read-only audits completed against the retained current-session archives. Alaska matched 808 of 857 bills and planned 18,711 URL fills; 49 changed or insufficient timelines were held with explicit reasons. North Carolina matched all 2,338 bills with zero holds and planned 19,887 URL fills. No production row changed. An isolated PostgreSQL apply test updated two exact timelines atomically and repeated as a zero-update no-op.
- [ ] The session reconciler is deployed. NC2025 apply completed across all 2,338 matched bills with zero held timelines; direct production readback now shows 19,976 actions, zero missing source URLs and zero bills with gaps. Alaska remains deliberately deferred until its active 856-bill replay drains; then apply the exact AK34 inventory, verify repeat no-op readback and bill-detail acceptance, and investigate its 49 held timelines from newer retained source evidence rather than weakening timeline equality.
...
| [ ] | People and committee dependency closure | Four Alaska historical timelines remain quarantined, but isolated replay now admits all 33 current committees and 233 memberships from the independently complete current directory. Production replay remains pending. NC admits all 94 committees and 1,678 memberships; official Senate district 1 is still absent from Open States HEAD. |
