...
From `apps/legislation-ingestion`, prepare a **new** output directory. Existing ignored bundles remain under
`apps/legislation/artifacts/openstates-runtime`; explicitly relocate only required non-secret inputs locally before
reuse, or configure the chosen build context. Do not overwrite retained historical bundles.
...
After downloading the original wheel through the approved feed, from `apps/legislation-ingestion`:
...
The local acceptance Dockerfile is `python/Dockerfile`, with build context `apps/legislation-ingestion`. It requires an explicitly
...
Run `pnpm --filter legislation-ingestion test:python` for build-input and runner tests. Real process-group and symlink tests require Linux;
