...
| `CDIR` Congressional Directory | Congress/member/committee directory volumes and member/state granules | Committee, subcommittee, and membership rosters are ingested from official plain-text editions; member identities must match an existing Congress.gov person and term. | Daily `govinfo-committee-directory-sync` at 09:30 UTC; manual `govinfo:committees` for validated explicit ranges. | Newly dated editions are applied chronologically as complete, Congress-scoped GovInfo organization snapshots. Continuous assignments retain one tenure; later omission records a detected end; return creates another tenure. Accepted historical editions and remaining source quarantine are recorded in [membership history](committee-membership-history.md). |
