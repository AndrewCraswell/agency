...
if (databaseUrl !== undefined && new URL(databaseUrl).pathname !== "/legislation_ingestion_test") {
  throw new Error("LEGISLATION_INGESTION_TEST_DATABASE_URL must target the legislation_ingestion_test database")
}

describePostgres.sequential("legislation PostgreSQL ingestion", () => {
  const pool = new pg.Pool({ connectionString: databaseUrl, max: 2 })
  const database = drizzle(pool, { schema })

  async function seedBillPrerequisites() {
    await database
    ...
    await database
      ...
      .values({
        ...
        id: "bill:us:119:hr:1234:document:introduced",
  ...
  async function seedCaliforniaBill() {
    await database
      .insert(schema.jurisdictions)
      .values({
        classification: "state",
  ...
  it("upserts aggregates idempotently and rolls back a failed child replacement", async () => {
    ...
    await expect(database.query.bills.findFirst({ where: eq(schema.bills.id, billId) })).resolves.toMatchObject({
      title: "A stable aggregate"
    })
  ...
  it("processes, indexes, and embeds supporting-material sections", async () => {
    await seedBillPrerequisites()
  ...
  it("keeps a GovInfo checkpoint before a transient failure and safely replays later packages", async () => {
    const xml = await readFile(
      new URL("../../tests/fixtures/govinfo/BILLSTATUS-119hr1234.xml", import.meta.url),
      "utf8"
    )
    let currentXml = xml
  ...
  it("validates the assembled ingestion corpus", async () => {
    await seedBillPrerequisites()
    await database.insert(schema.organizations).values({
      classification: "committee",
      id: "organization:openstates:wa-data-committee",
      jurisdictionId: "jurisdiction:wa",
