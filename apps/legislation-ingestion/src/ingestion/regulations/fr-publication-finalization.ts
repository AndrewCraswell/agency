...
export async function finalizeFrIssuePublication(pool: pg.Pool, value: unknown) {
  ...
  const metadata = await replayFrMetadata(JSON.parse(await readFile(preparation.metadata_locator, "utf8")))
  ...
  const renditionResult = await pool.query(
    `SELECT receipt,inspection,storage_locator FROM legislation.legal_fr_issue_renditions
