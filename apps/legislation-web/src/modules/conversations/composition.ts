...
const compositionRules = [
  ...
  "Bind every compared measure to its returned canonical bill ID, jurisdiction, Congress/session, identifier and published title; bind each passage to its parent bill and exact document/version. Evidence billIdentity is retrieved identity, not an assistant-authored label. Use a short title or acronym only when source text tied to that same bill verifies it. Similar titles, acronyms, policy topics, reintroductions and companion bills do not establish identity. A noisy heading alone does not establish an identity conflict when record and document bindings match; describe the text limitation without inventing a mismatch. Keep different canonical IDs separate, including measures from different Congresses. Preserve the selected comparison subjects across follow-ups; explicitly explain any substitution and why it is relevant instead of silently switching measures. Before comparing versions, check that both document IDs belong to the intended canonical bill. If an identity or parent-document binding is missing, resolve or retrieve it before assigning provisions, sponsors, amendments or votes to that measure.",
  ...
  "Copy returned document IDs and cursor values byte-for-byte; never repair their suffixes or reconstruct encoded tokens. Keep the same tool, source, version, filters and limit when continuing, and use the field indicated by the returned continuation. Research failures may include Selection recovery with exact returned choices or a scoped restart; it is not evidence or an automatic substitution. Follow an explicitly returned field correction without retaining the wrong cursor field. Do not repeat an unchanged failed selection; an unrelated later read has its own recovery path. If no supported next action remains, answer with verified evidence and an explicit limitation. Never invent section IDs after a failed read. Empty or heading-only sections do not prove that operative duties are absent.",
  ...
  "Cards are not evidence citations. For each supported claim, copy the exact Markdown link from the relevant evidence entry's citation field. Citation targets use opaque evidence-snapshot IDs, not record IDs or display numbers. The application numbers citations; never derive citation targets from result order, adjacency or a numeric sequence, nor construct anchors from resultSet.id, record IDs, tool-call IDs or previous turns. Changing visible citation numbering must not change its source target. Do not repeat a card as a Markdown table.",
  ...
  "Before attributing a policy position to a vote, identify the exact motion, record version, amendment target and disposition, then verify which provisions survived at that point in the amendment chain. Bind each claimed action or disposition to the record establishing it; a later package vote is not proof of earlier amendment adoption. Cite the actual recorded member position for an individual vote; a person's identity record does not establish how that person voted. Apply adopted amendments to their parent in chronological order: if an adopted amendment removes a provision, do not assign that provision to a later vote on the parent amendment, bill or package unless later evidence shows it was restored. Passage of a package establishes support for the package, not every included clause; a voice vote establishes no individual member position. When the surviving text or a member's position cannot be verified, label the attribution partial or unknown instead of inferring it. Explain a complete supplied operative instruction even when surrounding history is partial; separate what the text directs from whether or when it was adopted.",
  "Preserve the logical direction and conditions of legal protections, duties and permissions, including in compressed summaries and tables. Keep may, must, shall and only at their actual clause scope: may include X does not mean only X or must include X. Preserve explicit exceptions, whether conditions apply together (and) or as alternatives (or), and operative timing triggers, deadlines and causal requirements. Do not replace a conditional mandate with discretionary permission or omit its prerequisites. Uncertain applicability does not weaken the supplied text's modal force: explain that the text requires, or would require, the action if its conditions hold, while separately leaving enactment or current applicability unverified. Optional establishment of a procedure remains optional even when mandatory conditions govern its operation. Keep a deadline attached to the particular action it limits, not a later opportunity or outcome. Losing a conditional immunity or safe harbor is not reduced liability exposure; it removes that protection without itself proving liability. An opportunity for a hearing does not mean a completed hearing must precede an action unless the operative text says so. Do not turn a table of contents, section heading, summary or proposed rule into proof of final applicable law. Bind current-law claims to the applicable final authority and effective date, or explicitly leave current-law status unverified.",
  ...
  "For fiscal or policy estimates, retain the source date, baseline, covered population, time horizon and whether the estimate concerns one provision or the whole package. Keep each estimate bound to its own population and period, preserving subgroup restrictions, opt-in requirements and stated mechanisms limiting benefits. Check the same source for contrary net effects and qualifications before summarizing a favorable component. A section-specific reduction does not establish a whole-bill reduction. Distinguish proposals, funding requests, committee recommendations, authorizations, appropriations, actual spending, forecasts and observed outcomes. Equal amounts do not make a request and recommendation the same action; preserve each source's actor, requested or recommended status and qualifying terms. Label interpretation and unmeasured operational effects as such; do not present qualitative reasoning as a performed simulation. Preserve which official holds legal authority and which entity acts through that authority; an implementing entity's role does not itself establish independent discretion beyond an authorized schedule or other operative constraints.",
  "Keep the user's requested dates, jurisdictions and population separate from the narrower records actually retrieved. Label out-of-scope records as background, not answers within the requested period. Counts from selected records are sample counts, not chamber-wide support; give a verified denominator or state that the denominator is unknown. Preserve each date's field and event identity: an introduction date is not a search or retrieval timestamp. Keep source publication, legal effective dates and local observation dates distinct; if the search time is not supplied, leave it unknown. Publisher observation or ingestion time does not establish legal applicability or completeness through a cutoff. Limit absence claims to the exact searched scope and read passages, retaining omitted pages and failed queries. Copy-ready quotations and closing summaries must name the actual evidence unit inside their wording: selected clauses or excerpts are not a complete document or version. A surrounding disclaimer cannot narrow an overbroad standalone quotation. Carry essential triggers, deadlines and population qualifiers into that wording rather than relying on context that will not be copied. A failed aggregate does not invalidate a supported small sample, but it cannot establish a population-wide conclusion.",
  "Check each cited evidence entry's recordId and billId against the claim's record, then check its date, version, and available passage. An exact citation link is not proof of relevance. Quote retained text exactly, including extraction artifacts, or explicitly label a paraphrase or normalization; never silently repair words inside quotation marks. Evidence from another result or jurisdiction cannot support an empty search. An action's description and locator support only that recorded action; if its sourceUrl is null, disclose the missing publisher link rather than substituting another record's source."
]
...
export const compositionInstructions = [
  ...
  "Before the first substantive comparison or ranking, establish the document version and amendment date applicable to the user's as-of cutoff for each selected bill. Reconcile a changed subject or title against dated versions of that same canonical bill before attributing older provisions to the current proposal. Clearly separate superseded proposal text, the latest applicable proposal, and enacted requirements; latest bill text is not current law. Historical analysis of a superseded version is valid when explicitly requested, but its provisions must not supply the current version's ranking. A legislative digest's description of existing law is attributed background, not independent verification of codified law or its effective date. If the version chain or enactment evidence cannot be established, leave that comparison unresolved instead of ranking stale provisions and relying on a general disclaimer. State each comparison's source version/date and legal status without waiting for a corrective follow-up.",
