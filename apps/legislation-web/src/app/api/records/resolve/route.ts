import { createRecordResolutionHandler } from "../../../../modules/request-handling/api/record-resolution-routes"
...
export async function POST(request: Request): Promise<Response> {
  return await executeAuthenticatedApiRequest(
    request,
    createRecordResolutionHandler(
      async (input) => await getResearchRuntime().run((service) => service.resolveRecord(input))
    )
  )
}
