...
| --- | --- | --- |
| [Homepage](../../src/app/page.tsx) | Existing Next.js route and runtime | Replace the foundation content with the working conversation. |
...
| [MCP registry](../../../../packages/legislation-core/src/research/tools.ts) | Registered inputs, read-tool behavior, structured results and response bounds | Connect and validate; do not implement a second legislative query layer. |
| [API-backed MCP runtime](../../../legislation-mcp/src/application.ts) | Existing authenticated transport and API adapter | API/MCP audiences remain separate; a public chat session is not an API bearer credential. |
| [Research answers](../../src/modules/request-handling/api/research-answers.ts) | Evidence identity and claim/citation validation patterns | Single-answer generation is not a complete conversational agent or UI stream. Avoid nested model generation when tools can return evidence directly. |
| [Tool contract](../../../legislation-mcp/docs/engineering/tool-contracts.md) | Existing research operations and documented limitations | The legal-text pilot is gated and locally implemented; local availability is not public production acceptance. |
...
- Work: run the smallest relevant completed-slice checks, repair related defects, then `pnpm --filter legislation-web build`
  and the [single legislation verification gate](../operations/testing.md#full-verification). Confirm frozen dependency installation and the release Docker build.
...
- Work: retain repository-root Docker context, `apps/legislation-web/Dockerfile`, Config File Path
  `/apps/legislation-web/railway.json`, `PORT` binding on `0.0.0.0`, non-root runtime and `/ready` health check. Check watched paths.
...
- [Public API contract](../engineering/api/README.md) and [MCP contract](../../../legislation-mcp/docs/engineering/tool-contracts.md).
...
- [Authentication](../operations/authentication.md), [runtime/deployment](../operations/development.md), and
  [database pooling](../../../../packages/legislation-core/docs/operations/database-connection-pooling.md).
