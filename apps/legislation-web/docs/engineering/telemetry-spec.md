...
| Node and edge | Originally skipped OpenTelemetry setup; now owned by the [shared Node runtime](node-telemetry.md) and [edge configuration](../../src/services/sentry/sentry.edge.config.ts) | Validate context isolation, propagation and span export instead of merely increasing the sample rate |
...
| AI telemetry | [Conversation telemetry](../../src/modules/conversations/telemetry.ts) previously initialized a separate Langfuse NodeSDK; it now delegates to the shared Node owner | Coexist with Sentry without two global providers, duplicate spans or exporting content to both destinations |
