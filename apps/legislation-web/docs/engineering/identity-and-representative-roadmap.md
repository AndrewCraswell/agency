...
Status: federal identity hydration has recorded range acceptance; state and residual identity coverage remain open.
See the [retained coverage decisions](#retained-coverage-decisions).
...
| --- | --- | --- |
| Canonical legislation ingestion | Foundations implemented; scope acceptance separate | Persisted record families do not establish current nationwide entity/history coverage; see [state rollout](../../../legislation-ingestion/docs/operations/openstates-rollout-checklist.md). |
...
See [C's identity invariants](../../../../packages/legislation-core/docs/engineering/identity.md#identity-rules).

## Target data model

See [C's target civic model](../../../../packages/legislation-core/docs/engineering/identity.md#target-data-model).
Planned schema expansion is distinct from the partial implementation recorded in IDN-103/104 below.

## Entity taxonomy

See [C's entity taxonomy](../../../../packages/legislation-core/docs/engineering/identity.md#entity-taxonomy).

## Maintenance

I owns [source maintenance and recovery](../../../legislation-ingestion/docs/engineering/identity-sources.md#maintenance).
The task tables below remain one program sequencing record, not a second source specification. OFF and ENR execution
belongs to I; C owns shared schema and W owns customer/query acceptance.
...
owned by [embedding rollout](../../../legislation-ingestion/docs/engineering/embedding-rollout-plan.md); the Complete rows below are not current integrity certification.
...
| Task | Status | Deliverable and acceptance gate |
| --- | --- | --- |
| IDN-101 | Inventory complete; source gaps open | Retain stable-ID and term checks. The Payne father/son pair is distinct; the sixteen other normalized-name groups were unverified state vote stubs, not approved merges. See the retained coverage decisions below. |
...
| IDN-103 | Partial | Reuse `person_external_identifiers`, `people.source_id` and `upstream_ids`; do not create a parallel identifier table. Federal aliases and identifiers have recorded population acceptance; residual and state coverage remain open. Provider-scoped uniqueness and accepted-source population remain open; this row does not approve a new provider. |
| IDN-104 | Partial | Reuse `person_aliases` and OpenStates detail normalization. Federal aliases have recorded population acceptance; residual and state coverage remain open. Source-backed population, validity dates and language remain open; names alone cannot merge people. |
...
| OFF-204 | Partial | Shared NC/AK people/history import and local replay exist; [people quarantine](../../../legislation-ingestion/docs/operations/openstates-people-quarantine.md) and [state rollout](../../../legislation-ingestion/docs/operations/openstates-rollout-checklist.md) own acceptance. Complete remaining source, office/district and hosted lanes without recreating the importer. |
...
The unchanged September 11 counts, unresolved identities and source-URL decisions now live in
[I's retained source evidence](../../../legislation-ingestion/docs/engineering/identity-sources.md#retained-coverage-decisions).
