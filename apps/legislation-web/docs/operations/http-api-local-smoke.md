...
The W harness is `pnpm --filter legislation-web smoke:api`. Set `LEGISLATION_SMOKE_BASE_URL` to an explicitly started
W Next.js instance, local or remote. The former combined `src/cli/main.ts serve` is not the separated runtime's entrypoint.
The value must be a credential-free HTTP(S) origin at its root, without a query or fragment. The harness always
...
- [ ] Confirm removed operations return `404`. An endpoint awaiting production-data acceptance is not necessarily absent; verify its documented behavior and record missing fixture evidence separately.
...
- [ ] Exercise the retained relationship, agenda, participant and document operations with real linked fixtures. Confirm
      standalone calendar, meeting-outcome and representative-lookup operations remain absent from the public contract.
