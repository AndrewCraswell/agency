...
A checked item means the described implementation is complete and has passed its focused executable check. Partial
implementation remains unchecked. Browser acceptance and repository-wide verification remain tracked by the owning
workstream test item and ENG-124.

### Workstream 0: Stabilize the verification baseline

- [x] **ENG-001 — Repair the web coverage harness.** Reproduce the missing
...
- [x] **ENG-002 — Complete API mock coverage for shell-owned requests.** Add handlers for
...
- [x] **ENG-003 — Restore the workflow-editor model test.** Isolate and fix the failing “authors catalog-backed model
...
- [x] **ENG-010 — Define the canonical journal run-detail contract.** Expose one versioned run resource under
  `apps/agentic/src/workflows/` with normalized outcome, current or failed step, workflow version, summary, available
  actions, and persisted evidence. Journal records are the sole product truth; no source variant is retained.
- [x] **ENG-011 — Use one canonical run route.** Resolve `GET /api/workflow-runs/:runId` directly from the workflow
  journal and return its typed not-found result. Do not probe or fall back to a control-plane run endpoint.
- [x] **ENG-012 — Remove legacy run detail.** Delete the control-plane run-detail endpoint, contracts, static topology,
  tests, web client, and Operations links that treated orchestration records as journal workflow runs.
- [x] **ENG-013 — Project journal recovery state.** Project activations, waits, effects, retries, cancellation, and
  evidence into the canonical contract and derive normalized available actions in the service layer.
- [x] **ENG-014 — Normalize failure summaries in the service layer.** Produce a customer-safe failed-step label, concise
...
- [x] **ENG-015 — Replace the web run API clients.** Replace `getWorkflowRunDetail` and
...
- [x] **ENG-016 — Refactor `RunDetailPage` around a view model.** Remove the dual `detail` and `journalDetail` state paths,
...
- [x] **ENG-017 — Add the failed-run action surface.** Render a persistent Fluent `MessageBar` containing failed step,
...
- [x] **ENG-018 — Add run-contract and page tests.** Cover adapter projections for failed, cancelled, waiting, running,
...
- [x] **ENG-020 — Add an explicit active-version field.** Introduce `activePublishedVersion` in the workflow persistence
...
- [x] **ENG-021 — Update workflow store operations.** Make publish atomically create an immutable version and set it
...
- [x] **ENG-022 — Make run creation version-specific.** Require `POST /api/workflows/:workflowId/runs` to receive or
...
- [x] **ENG-023 — Version the workflow response schemas.** Update `WorkflowSummarySchema`, `WorkflowDraftViewSchema`, and
...
- [x] **ENG-024 — Build an editor command toolbar.** Replace the free-form action row in `WorkflowEditorPage` with a
...
- [x] **ENG-025 — Implement lifecycle copy and disabled reasons.** Render “Editing draft,” “All changes saved,” “Publish
...
- [x] **ENG-026 — Add lifecycle service and editor tests.** Cover editing while an older version remains active,
...
- [x] **ENG-030 — Stamp validation responses with draft revision.** Add `draftRevision` to the validation contract and
...
- [x] **ENG-031 — Model editor result state explicitly.** Replace loose `validation` and `simulation` state with typed
...
- [x] **ENG-032 — Extract the contextual properties inspector.** Move workflow, step, and connection configuration out of
  the monolithic `WorkflowEditorPage.tsx` into focused inspector components without validation or simulation sections.
- [x] **ENG-033 — Add Problems and Test results panels.** Build persistent, labeled panels with counts and revision
...
- [x] **ENG-034 — Link results to editable targets.** Extend validation issues and test-step results with field or port
...
- [x] **ENG-035 — Add result-panel tests.** Cover fresh and stale transitions, repeated validation, a failed test beside
...
- [x] **ENG-040 — Split editor styles and presentation components.** Move the toolbar, canvas surface, outline,
  inspector, and result-panel styles into focused files following the repository component-layout convention before the
  responsive rewrite.
- [x] **ENG-041 — Build an outline projection from graph data.** Add a tested graph-to-outline utility that derives
...
- [x] **ENG-042 — Replace the flat Outline.** Render the topology projection with semantic nested lists or tree behavior,
...
- [x] **ENG-043 — Implement mobile editor composition.** Replace wrapped desktop controls with a compact identity row,
...
- [x] **ENG-044 — Implement editor focus coordination.** Define focus movement and return among canvas, Outline,
...
- [x] **ENG-045 — Add responsive and accessibility test coverage.** Add browser-rendered editor stories for desktop and
...
- [x] **ENG-050 — Reconcile the create-workflow contract.** Align `CreateWorkflowRequestSchema`, the server service, and
...
- [x] **ENG-051 — Replace implicit dialog defaults.** Reset create state whenever the dialog opens, initialize the name
  and repository as empty, and require an explicit repository selection. Do not select the first inventory result in
  `openCreate`.
- [x] **ENG-052 — Add prerequisite navigation.** Add a typed app link from the no-repository state to Integrations and
  return to workflow creation after a connection is completed. Preserve only intentional form selections during that
  round trip.
- [x] **ENG-053 — Explain binding scope in the form.** Show repository owner and name, selected Linear team and model when
  required by the contract, and concise text describing which workflow agents, data, and actions inherit those bindings.
- [x] **ENG-054 — Add creation-flow tests.** Cover empty required fields, one and many repositories, no connected
...
- [x] **ENG-060 — Define Operations query contracts.** Add schema-validated search parameters for view, text query,
...
- [x] **ENG-061 — Add server-side work-item querying.** Replace the all-items control-plane snapshot dependency for the
...
- [x] **ENG-062 — Split the Operations route by job.** Extract Overview, Runs, and Work queue components from
  `apps/web/src/App.tsx`. Keep summary exceptions on Overview, execution status on Runs, and assignment and triage on Work
  queue.
- [x] **ENG-063 — Build the compact work queue.** Use a Fluent `DataGrid` when the finalized task model needs column
...
- [x] **ENG-064 — Persist and restore queue state.** Store filters, sorting, and selected view in typed URL search state so
  links are shareable and browser navigation restores the same queue.
- [x] **ENG-065 — Correct active-work terminology.** Rename “Active agents” to “Active runs” in metrics, headings, empty
...
- [x] **ENG-066 — Add Operations service and UI tests.** Cover filter and cursor contracts, stable sorting, 200-plus item
...
- [x] **ENG-070 — Add UI metadata to step definitions.** Extend the authoritative step catalog with field labels,
...
- [x] **ENG-071 — Build reusable schema-field renderers.** Implement focused Fluent controls for strings, numbers,
...
- [x] **ENG-072 — Build typed mapping and condition editors.** Replace routine mapping JSON, switch-case JSON, expression
  paths, and comparison values with row-based editors that source available ports and fields from adjacent step schemas.
- [x] **ENG-073 — Build a typed test-input editor.** Generate draft test inputs from the workflow input schema, retain an
  Advanced JSON mode, and use the same parsed value for Test draft and Run published version where their schemas match.
- [x] **ENG-074 — Add an Advanced configuration disclosure.** Move digests, raw schemas, IDs, immutable references, and
...
- [x] **ENG-075 — Migrate step inspectors incrementally.** Convert the existing `StepInspector` branches by category,
...
- [x] **ENG-076 — Add renderer and migration tests.** Test schema constraints, invalid intermediate input, mode switching,
...
- [x] **ENG-080 — Rename the route and navigation.** Change the primary navigation item and typed route from Settings to
  Integrations and update customer-facing loading and error copy. Remove `/settings`; this unshipped product does not
  retain a compatibility redirect.
- [x] **ENG-081 — Extract integration connection components.** Split catalog, connection summary, resource inventory, and
  connection actions from `SettingsPage.tsx` into focused components and rename the page module to match the route.
- [x] **ENG-082 — Add connection summary fields.** Expose provider account, connection status, last successful sync,
  latest error, resource counts, and capability scope in the integration service contract and render those fields before
  resource details.
- [x] **ENG-083 — Add resource search and disclosure.** Collapse resource names by default, add per-connection search and
...
- [x] **ENG-084 — Move disconnect impact to service data.** Return affected workflow counts or references from a dry-run
  impact query and use them in the disconnect confirmation instead of a generic warning.
- [x] **ENG-085 — Add integration route tests.** Cover route removal, summary projection, resource disclosure and search,
...
- [x] **ENG-090 — Inventory consequential actions and authorization enforcement.** Trace assign, publish, run, retry,
...
- [x] **ENG-091 — Define action-availability metadata.** Add a shared contract for action key, allowed state, disabled
...
- [x] **ENG-092 — Render action consequences consistently.** Update command bars, menus, and dialogs to consume
...
- [x] **ENG-093 — Add a shared asynchronous status surface.** Add a small app-level live-status component or hook for
...
- [x] **ENG-094 — Normalize busy and retry state.** Give each mutation its own pending state, prevent duplicate requests,
  preserve retryable input after failure, and avoid globally disabling unrelated connection or editor actions.
- [x] **ENG-095 — Add authorization and async tests.** Cover allowed and denied action projections, approval-required
...
- [x] **ENG-100 — Define durable schedule persistence.** Add schedule identity, workflow version, trigger node, enabled
...
- [x] **ENG-101 — Implement a leased schedule dispatcher.** Add a control-plane worker loop that claims due schedules,
...
- [x] **ENG-102 — Add schedule lifecycle APIs.** Return server-owned schedule health and last and next run from workflow
  APIs, and add enable, disable, and update operations with optimistic concurrency.
- [x] **ENG-103 — Remove browser-owned execution.** Delete `WorkflowScheduleCoordinator`, its shell registration, and
  `listPublishedWorkflowSchedules` as an execution mechanism. Keep only UI queries for server-reported schedule status.
- [x] **ENG-104 — Add scheduler tests.** Use controlled time to test one dispatcher, competing dispatchers, expired
...
- [x] **ENG-105 — Add deployment ownership.** Start the scheduler worker in the agentic runtime or a dedicated process,
...
- [x] **ENG-110 — Remove orphan routes.** Delete `/about` and `/contact` route registrations, page modules, tests, and dead
...
- [x] **ENG-111 — Apply the approved lifecycle terminology.** Update editor, run, Operations, workflow creation, and
...
- [x] **ENG-112 — Revalidate changed copy and primitives with Fluent Agent.** Review every changed customer-facing string
...
- [x] **ENG-113 — Update product and architecture documentation.** Document the canonical run model, draft and active
...
- [x] **ENG-120 — Add browser fixtures for reviewed scale and failure states.** Create deterministic development and test
...
- [x] **ENG-121 — Run integrated browser journeys.** Exercise create, configure, resolve
  problems, test, publish, run, diagnose, recover, filter Operations, manage an integration, and inspect a schedule across
  desktop and mobile viewports.
- [x] **ENG-122 — Add accessibility automation and manual checkpoints.** Run automated checks on every route and add
  repeatable keyboard, screen-reader, high-contrast, touch-target, and 400 percent zoom scripts or checklists to the
  browser verification workflow.
- [x] **ENG-123 — Define the direct migration rollout.** Register and verify the database migrations, deploy the API and
  client against the new contracts, and remove obsolete fields, endpoints, routes, and adapters. This unshipped product
  does not retain dual reads, compatibility redirects, fallback adapters, or deprecated contracts.
- [x] **ENG-124 — Run repository gates per coherent slice.** For each workstream, add focused service and UI tests, run
